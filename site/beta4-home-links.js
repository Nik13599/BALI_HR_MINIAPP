(() => {
  ...
  document.addEventListener("click", event => {
    ...
    if (telegram && tg?.openTelegramLink) { event.preventDefault(); tg.openTelegramLink(telegram.href); return; }
    const external = event.target.closest("[data-open-link]");
    if (external && tg?.openLink) { event.preventDefault(); tg.openLink(external.href); }
  });
  let attempts = 0;
  const timer = setInterval(() => { attempts += 1; if (mount() || attempts > 30) clearInterval(timer); }, 100);
})();