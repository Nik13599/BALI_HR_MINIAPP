(() => {
  ...
  function install() {
    ...
    const people = [
      { id:"bali-user-nikolay", name:"Николай", status:"party", statusLabel:"Ищу компанию на вечеринку", bio:"Организую лучшие ночи BALI. Собираю компанию на ближайшее событие.", active:true, gender:"male", birthDate:"1994-12-19", phone:"375296700300", sharePhone:false, vipPlan:"black", level:"BALI Insider", visits:14, xp:5350, photo:avatar("Николай","#c8ff3d","#184d35"), cropX:50, cropY:42, updatedAt:iso(0) },
      { id:"bali-user-anna", name:"Анна Мороз", status:"table", statusLabel:"Ищу компанию за столик", bio:"Люблю house, красивые вечеринки и новые знакомства.", active:true, gender:"female", birthDate:"1998-05-14", phone:"375291112233", sharePhone:false, vipPlan:"legend", level:"Club Legend", visits:18, xp:6680, photo:avatar("Анна Мороз","#ff6fb1","#5d1b67"), cropX:50, cropY:42, updatedAt:iso(0) },
      { id:"bali-user-maxim", name:"Максим Орлов", status:"chat", statusLabel:"Открыт к общению", bio:"DJ, музыка и новые люди. Всегда на танцполе.", active:true, gender:"male", birthDate:"1996-09-02", phone:"375333456789", sharePhone:true, vipPlan:"vip", level:"Night Regular", visits:9, xp:2940, photo:avatar("Максим Орлов","#4dd4ff","#123e7a"), cropX:50, cropY:42, updatedAt:iso(0) },
      { id:"bali-user-sofia", name:"София Волкова", status:"party", statusLabel:"Ищу компанию на вечеринку", bio:"Танцую до утра. Всегда за яркие события BALI.", active:true, gender:"female", birthDate:"2000-02-28", phone:"375447778899", sharePhone:false, vipPlan:"", level:"BALI Insider", visits:12, xp:4270, photo:avatar("София Волкова","#f4cf5d","#9f3f28"), cropX:50, cropY:42, updatedAt:iso(0) },
      { id:"bali-user-artem", name:"Артём Левин", status:"table", statusLabel:"Ищу компанию за столик", bio:"Собираю компанию на ближайшую пятницу.", active:true, gender:"male", birthDate:"1997-07-11", phone:"375259998877", sharePhone:false, vipPlan:"", level:"Night Regular", visits:6, xp:1780, photo:avatar("Артём Левин","#a783ff","#32205e"), cropX:50, cropY:42, updatedAt:iso(0) },
      { id:"bali-user-daria", name:"Дарья Ночь", status:"chat", statusLabel:"Открыта к общению", bio:"Коктейли, танцы и новые знакомства ✨", active:true, gender:"female", birthDate:"1999-11-08", phone:"375295554433", sharePhone:false, vipPlan:"", level:"Night Regular", visits:10, xp:3560, photo:avatar("Дарья Ночь","#ff8f55","#721c45"), cropX:50, cropY:42, updatedAt:iso(0) },
      { id:"bali-user-alex", name:"Алекс Грин", status:"party", statusLabel:"Ищу компанию на вечеринку", bio:"Люблю techno и большие клубные события.", active:true, gender:"male", birthDate:"1995-03-22", phone:"375291010101", sharePhone:false, vipPlan:"black", level:"BALI Insider", visits:16, xp:4920, photo:avatar("Алекс Грин","#51ff93","#0d515f"), cropX:50, cropY:42, updatedAt:iso(0) },
      { id:"bali-user-mila", name:"Мила Рэй", status:"closed", statusLabel:"Не знакомлюсь", bio:"Пришла слушать музыку и отдыхать с друзьями.", active:true, gender:"female", birthDate:"2001-06-18", phone:"375292020202", sharePhone:false, vipPlan:"", level:"Party Starter", visits:4, xp:980, photo:avatar("Мила Рэй","#ff9c5a","#4a174f"), cropX:50, cropY:42, updatedAt:iso(0) }
    ];

    const rewards = [
      { id:"reward-demo-regular", title:"Постоянный гость", description:"Посетить BALI минимум 5 раз", icon:"🎟", image:"", xp:300, conditionType:"visits", threshold:5, active:true, sort_order:1, createdAt:iso(-180), updatedAt:iso(0) },
      { id:"reward-demo-crown", title:"Участник Neon Crown", description:"Принять участие в конкурсе Король и Королева BALI", icon:"👑", image:"", xp:500, conditionType:"event", eventId:"event-demo-crown", eventTitle:"BALI Neon Crown", threshold:1, active:true, sort_order:2, createdAt:iso(-30), updatedAt:iso(0) },
      { id:"reward-demo-dance", title:"Легенда танцпола", description:"Эксклюзивная награда активному гостю", icon:"🪩", image:"", xp:1000, conditionType:"manual", threshold:1, active:true, sort_order:3, createdAt:iso(-90), updatedAt:iso(0) },
      { id:"reward-demo-company", title:"Большая компания", description:"Забронировать стол на 6 и более гостей", icon:"🥂", image:"", xp:350, conditionType:"manual", threshold:1, active:true, sort_order:4, createdAt:iso(-60), updatedAt:iso(0) },
      { id:"reward-demo-vip", title:"VIP-гость", description:"Получить любой VIP-статус BALI", icon:"💎", image:"", xp:600, conditionType:"manual", threshold:1, active:true, sort_order:5, createdAt:iso(-45), updatedAt:iso(0) },
      { id:"reward-demo-social", title:"Душа BALI People", description:"Активно знакомиться и участвовать в событиях", icon:"✨", image:"", xp:450, conditionType:"manual", threshold:1, active:true, sort_order:6, createdAt:iso(-20), updatedAt:iso(0) }
    ];
    ...
    const appUsers = Object.fromEntries(people.map((person, index) => [person.id, {
      ...
      gender:person.gender,
      status:person.status,
      status_label:person.statusLabel,
