  ...
  try {
    ...
    if (action === "sync") {
      ...
      await db.from("app_users").upsert({
        ...
        avatar: String(patch.photo || user.photo_url || "").slice(0, 2_500_000),
      ...
      const profilePayload = {
        ...
        phone: "",
        photo: String(patch.photo || user.photo_url || "").slice(0, 2_500_000),
    ...
    if (action === "list") {
      ...
      const [appUsers, pointUsers, profiles, memberships, plans] = await Promise.all([
        safeRows(db.from("app_users").select("*").order("last_seen_at", { ascending: false }).limit(1000)),
        safeRows(db.from("points_accounts").select("*").order("updated_at", { ascending: false }).limit(1000)),
        safeRows(db.from("social_profiles").select("*").limit(1000)),
        safeRows(db.from("vip_memberships").select("*").lte("starts_at", now).gt("expires_at", now).order("expires_at", { ascending: false }).limit(2000)),
