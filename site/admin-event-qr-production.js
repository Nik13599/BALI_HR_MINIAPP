(() => {
  ...
  function checkinUrl(event){const bot=String(cfg.telegramUsername||"BaliMinskAppBot").replace(/^@/,"");const payload=encode({v:2,eventId:event.id,token:event.qr_token,title:event.title,date:event.event_date,time:event.event_time||"23:00",issuedAt:new Date().toISOString()});return`https://t.me/${bot}?startapp=checkin_${payload}`}
