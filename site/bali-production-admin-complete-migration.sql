...
create table if not exists public.loyalty_gift_grants (
  id uuid primary key default gen_random_uuid(),
  gift_id text not null references public.loyalty_gifts(id) on delete cascade,
  user_key text not null references public.app_users(user_key) on delete cascade,
  user_name text not null default 'Гость BALI',
...
-- Reviews are submitted only from the Telegram-gated Mini App. Administrators retain full control.
drop policy if exists "telegram app submit reviews" on public.reviews;
create policy "telegram app submit reviews" on public.reviews for insert to anon with check(
...
do $$ declare t text; begin
  foreach t in array array[
    ...
    'loyalty_gifts','loyalty_gift_grants','vip_gifts'
  ] loop
...
grant select,insert,update,delete on public.venue_content,public.reviews,public.loyalty_settings,
  public.loyalty_rewards,public.loyalty_reward_grants,public.loyalty_gifts,public.loyalty_gift_grants,public.vip_gifts to authenticated;

-- Keep PostgREST schema cache current after the new tables are created.
