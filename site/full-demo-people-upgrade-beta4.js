(() => {
  ...
  function styles() {
    ...
    style.textContent = `
      ...
      .people-public-badges{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px}.people-public-badges span{display:inline-flex;align-items:center;min-height:25px;padding:0 8px;border:1px solid rgba(200,255,61,.25);border-radius:999px;background:rgba(200,255,61,.07);color:var(--lime);font-size:8px;font-weight:900}
      .people-public-badges .vip{border-color:rgba(242,205,102,.42);background:rgba(242,205,102,.1);color:#f2cd66}.people-public-badges .crown{border-color:rgba(255,121,198,.4);color:#ff9bd4}
  ...
  function privacyContacts(person) {
    const values = [];
    if (canSee(person,"telegram") && person.username) values.push(`<span>${esc(String(person.username).startsWith("@") ? person.username : `@${person.username}`)}</span>`);
    if (canSee(person,"phone") && person.phone) values.push(`<span>${esc(person.phone)}</span>`);
    const age = ageFor(person);
    if (age && canSee(person,"age")) values.push(`<span>${age} лет</span>`);
    return values.join("");
  }
  ...
  function decorateCard(card, person) {
    ...
    let contacts = body.querySelector(".people-private-contact");
    const contactsHtml = privacyContacts(person);
    if (contactsHtml) {
      if (!contacts) { contacts = document.createElement("div"); contacts.className = "people-private-contact"; body.querySelector(".person-v2-actions")?.insertAdjacentElement("beforebegin", contacts); }
      contacts.innerHTML = contactsHtml;
    } else contacts?.remove();
    ...
    badges.innerHTML = `${vip ? `<span class="vip">${esc(vip.plan?.name || vip.planId || "VIP")}</span>` : ""}<span>${Number(account.balance || 0)} баллов</span>${rewards.length ? `<span>🏆 ${rewards.length}</span>` : ""}`;
  ...
  function decorateCards() {
    ...
    const note = document.querySelector("#baliPeopleSearch small");
    if (note) note.textContent = viewerHasVip() ? "VIP-доступ: поиск по имени, возрасту, Telegram и телефону." : "Telegram, телефон и возраст участвуют в поиске только при разрешении пользователя.";
  }
  ...
  async function decorateDialog(personId) {
    ...
    body.insertAdjacentHTML("beforeend", `<div id="fullPeopleDetails">
      <section class="people-detail-section"><h3>О пользователе</h3><div class="people-detail-list">
        <div class="people-detail-row"><span>Статус</span><strong>${esc(vip?.plan?.name || game.levelFor(Number(account.xp || 0)).current.name)}</strong></div>
        ${canSee(person,"age") && ageFor(person) ? `<div class="people-detail-row"><span>Возраст</span><strong>${ageFor(person)} лет</strong></div>` : ""}
        ...
        <div class="people-detail-row"><span>BALI-баллы</span><strong>${Number(account.balance || 0)}</strong></div>
      </div></section>
      <section class="people-detail-section"><h3>Награды и победы</h3><div class="people-detail-list">
        ${rewards.map(row => `<div class="people-detail-row"><span>${esc(row.reward?.icon || "🏆")} ${esc(row.reward?.title || "Награда BALI")}</span><strong>${esc(fmt(row.earnedAt))}</strong></div>`).join("") || '<div class="people-detail-row"><span>Награды</span><strong>Пока нет</strong></div>'}
        ${crown.miss ? `<div class="people-detail-row"><span>👑 Королева BALI</span><strong>${crown.miss}×</strong></div>` : ""}
        ${crown.mister ? `<div class="people-detail-row"><span>👑 Король BALI</span><strong>${crown.mister}×</strong></div>` : ""}
      </div></section>
      <section class="people-detail-section"><h3>История посещений</h3><div class="people-detail-list">${visits.slice(0,20).map(row => `<div class="people-detail-row"><span>${esc(row.event_title || "Мероприятие BALI")}</span><strong>${esc(fmt(row.checked_in_at))}</strong></div>`).join("") || '<div class="people-detail-row"><span>Посещений пока нет</span><strong>—</strong></div>'}</div></section>
    </div>`);
  }
  ...
  function activeRequests() {
    return social.requests().filter(row => social.isRequestActive?.(row) !== false);
  }
  function inviteCard(row, incoming) {
    return `<article class="profile-invite-card"><header><div><h3>${esc(row.eventTitle || "Мероприятие BALI")}</h3><p>${incoming ? `От: <strong>${esc(row.fromName || "Пользователь BALI")}</strong>` : `Кому: <strong>${esc(row.toName || "Пользователь BALI")}</strong>`}</p></div><span class="${esc(row.status || "pending")}">${requestStatus(row.status)}</span></header><p>${esc(row.eventDate || "")} · ${esc(row.eventTime || "23:00")}</p>${incoming ? `<div class="profile-invite-actions"><button type="button" class="secondary accept ${row.status === "accepted" ? "active" : ""}" data-profile-invite-response="${esc(row.id)}:accepted">Принять</button><button type="button" class="secondary decline ${row.status === "declined" ? "active" : ""}" data-profile-invite-response="${esc(row.id)}:declined">Отклонить</button></div>` : ""}</article>`;
  }
  function renderInviteCenter() {
    const root = document.getElementById("profileInvitationsBody");
    if (!root) return;
    const me = String(social.myId()), rows = activeRequests();
    const incoming = rows.filter(row => String(row.toId) === me);
    const outgoing = rows.filter(row => String(row.fromId) === me);
    document.getElementById("profileInvitationsTitle")?.replaceChildren(document.createTextNode("Приглашения"));
    root.innerHTML = `<div class="profile-invite-columns"><section class="profile-invite-column"><h3>Кто пригласил меня · ${incoming.length}</h3>${incoming.map(row => inviteCard(row,true)).join("") || '<div class="empty">Входящих приглашений нет</div>'}</section><section class="profile-invite-column"><h3>Кого пригласил я · ${outgoing.length}</h3>${outgoing.map(row => inviteCard(row,false)).join("") || '<div class="empty">Вы ещё никого не приглашали</div>'}</section></div>`;
    const tile = document.querySelector("[data-open-profile-invitations]");
    tile?.querySelector("small")?.replaceChildren(document.createTextNode("ПРИГЛАШЕНИЯ"));
    tile?.querySelector("strong")?.replaceChildren(document.createTextNode(`${incoming.length} входящих · ${outgoing.length} исходящих`));
  }
  ...
  function filterByExtendedSearch() {
    ...
    document.querySelectorAll("[data-open-social-person]").forEach(card => {
      const matches = !query || String(card.dataset.peopleSearch || "").includes(query);
      ...
      const ageFiltered = min > 18 || max < 99;
      const genderMatches = gender === "all" || card.dataset.peopleGender === gender;
      const ageMatches = !ageFiltered || (age && age >= Math.min(min,max) && age <= Math.max(min,max));
      card.hidden = !(matches && genderMatches && ageMatches);
    });
  }
  ...
  document.addEventListener("click", event => {
    ...
    const current = event.target.closest("[data-open-current-event]");
    if (current) document.querySelector('[data-social-v2-tab="inside"]')?.click();
  ...
  document.addEventListener("submit", event => {
    ...
    social.saveProfile({ shareAge:Boolean(form.elements.shareAge?.checked), showPhoto:Boolean(form.elements.showPhoto?.checked) });
  }, true);
  document.addEventListener("input", event => {
    if (["baliPeopleNameSearch","baliPeopleAgeMin","baliPeopleAgeMax"].includes(event.target.id)) setTimeout(filterByExtendedSearch, 0);
  });
  document.addEventListener("change", event => {
    if (event.target.id === "baliPeopleGender") setTimeout(filterByExtendedSearch, 0);
  });
  ...
  ["bali:social-changed","bali:points-changed","bali:beta4-changed","bali:loyalty-changed","bali:data-changed","bali:checkin-complete","bali:checkin-left"].forEach(name => window.addEventListener(name, refresh));
