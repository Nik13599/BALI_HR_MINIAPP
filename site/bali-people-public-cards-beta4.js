(() => {
  ...
  function accountRows() {
    return Object.values(points?.accounts?.() || {});
  }
  ...
  function rankingData(person) {
    ...
    return {
      account,
      place: Number(row?.position || (index >= 0 ? index + 1 : 0)),
      xp: Number(row?.xp || account.xp || 0),
      visits: Number(row?.visits || account.visits || 0),
      level: row?.level?.name || row?.levelName || row?.level || game?.levelFor?.(Number(row?.xp || account.xp || 0))?.current?.name || "BALI Guest"
    };
  }
  ...
  function rewardData(person, account) {
    ...
    return grants.map(grant => ({
      ...rewards.find(reward => String(reward.id) === String(grant.rewardId)),
      ...grant
    }));
  }
  ...
  function styles() {
    ...
    style.textContent = `
      ...
      .bali-people-public-stats article{padding:10px 7px;border:1px solid var(--line);border-radius:13px;background:#ffffff05;text-align:center}
      .bali-people-public-stats span{display:block;color:var(--muted);font-size:7px}.bali-people-public-stats strong{display:block;margin-top:5px;color:var(--lime);font:600 14px Unbounded}
      .bali-people-public-level{margin-top:9px;padding:10px;border:1px solid rgba(200,255,61,.2);border-radius:13px;background:rgba(200,255,61,.06);font-size:9px}
      .bali-people-rewards{display:grid;gap:7px;margin-top:13px}.bali-people-rewards h3{margin:0 0 2px;font-size:12px}.bali-people-reward{display:grid;grid-template-columns:38px 1fr;gap:9px;align-items:center;padding:9px;border:1px solid var(--line);border-radius:13px;background:#ffffff04}.bali-people-reward i{width:38px;height:38px;display:grid;place-items:center;border-radius:11px;background:#c8ff3d12;color:var(--lime);font-style:normal}.bali-people-reward strong{font-size:9px}.bali-people-reward small{display:block;margin-top:3px;color:var(--muted);font-size:7px}
      @media(max-width:360px){.bali-people-public-stats{grid-template-columns:1fr}}
    `;
  ...
  function enhancePublicCard(id) {
    ...
    body.insertAdjacentHTML("beforeend", `<section class="bali-people-public-details"><div class="bali-people-public-note">Контактный номер и Telegram пользователя скрыты. В публичной карточке доступны только достижения и рейтинг.</div><div class="bali-people-public-stats"><article><span>МЕСТО</span><strong>${data.place ? `#${data.place}` : "—"}</strong></article><article><span>ПОСЕЩЕНИЯ</span><strong>${data.visits}</strong></article><article><span>XP</span><strong>${data.xp}</strong></article></div><div class="bali-people-public-level">Текущий уровень: <b>${esc(data.level)}</b></div><div class="bali-people-rewards"><h3>Награды пользователя</h3>${rewards.length ? rewards.map(reward => `<article class="bali-people-reward"><i>${reward.image ? `<img src="${esc(reward.image)}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:10px">` : "🏆"}</i><div><strong>${esc(reward.title || reward.rewardTitle || "Награда BALI")}</strong><small>${esc(reward.description || reward.source || "Получена в BALI")}</small></div></article>`).join("") : '<div class="social-v2-empty">Наград пока нет</div>'}</div></section>`);
  }
