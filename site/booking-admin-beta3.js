(() => {
  ...
  function bookingCard(row) {
    return `<article class="booking-admin-card">
      <div class="booking-admin-time"><strong>${esc(row.booking_time || "23:00")}</strong><span>${esc(row.table_name || row.table_id || "Стол не выбран")}</span></div>
      <div class="booking-admin-person"><strong>${esc(row.customer_name || row.name || "Гость")}</strong><span>${esc(row.phone || "—")} · ${Number(row.guests || 0)} гостей</span>${row.comment ? `<small>${esc(row.comment)}</small>` : ""}</div>
      <span class="status ${esc(row.status || "pending")}">${esc(labels[row.status] || row.status || "Ожидает")}</span>
      <div class="booking-admin-actions"><button class="icon-btn" data-edit-booking="${row.id}" title="Редактировать">✎</button>${row.status !== "cancelled" ? `<button class="icon-btn danger-soft" data-cancel-booking="${row.id}" title="Отменить">×</button>` : ""}</div>
    </article>`;
  }
  ...
  function dayGroups(rows) {
    ...
    return dates.map((date) => {
      const dayRows = groups[date].sort(byDateTime);
      const totals = summary(dayRows);
      return `<section class="booking-day-group"><header><div><span>${formatDate(date)}</span><strong>${totals.guests} гостей</strong></div><div><b>${totals.bookings} броней</b>${totals.cancelled ? `<small>${totals.cancelled} отменено</small>` : ""}</div></header><div class="booking-admin-list">${dayRows.map(bookingCard).join("")}</div></section>`;
    }).join("");
  }
  ...
  async function renderBookingManager(root) {
    ...
    state.bookingRange ||= { from: today, to: today };
    ...
    root.innerHTML = `<section class="panel"><div class="panel-head"><div><h3>Бронирования по дням</h3><small>По умолчанию показан текущий день. Для анализа выберите период.</small></div></div><div class="panel-body booking-period-body">
      <div class="booking-period-toolbar"><div class="booking-period-presets"><button class="ghost" data-booking-preset="today">Сегодня</button><button class="ghost" data-booking-preset="1">Месяц</button><button class="ghost" data-booking-preset="3">3 месяца</button><button class="ghost" data-booking-preset="6">6 месяцев</button></div><div class="booking-period-dates"><label><span>С</span><input id="bookingFrom" type="date" value="${state.bookingRange.from}"/></label><label><span>По</span><input id="bookingTo" type="date" value="${state.bookingRange.to}"/></label><button class="primary compact" id="applyBookingRange" type="button">Показать</button></div></div>
      <div class="booking-period-summary"><article><span>БРОНЕЙ</span><strong>${totals.bookings}</strong></article><article><span>ГОСТЕЙ</span><strong>${totals.guests}</strong></article><article><span>ОТМЕНЕНО</span><strong>${totals.cancelled}</strong></article></div>
      <div id="bookingDayGroups">${dayGroups(filtered)}</div>
    </div></section>`;

    root.querySelectorAll("[data-booking-preset]").forEach((button) => button.addEventListener("click", () => {
      const value = button.dataset.bookingPreset;
      const from = todayIso();
      state.bookingRange = value === "today" ? { from, to: from } : { from, to: plusMonths(from, Number(value)) };
      render();
    }));
    $("#applyBookingRange").addEventListener("click", () => { state.bookingRange = { from: $("#bookingFrom").value, to: $("#bookingTo").value }; render(); });
    ...
    root.querySelectorAll("[data-cancel-booking]").forEach((button) => button.addEventListener("click", async () => {
      if (!confirm("Отменить это бронирование?")) return;
      const all = await store.list("bookings");
      const row = all.find((item) => item.id === button.dataset.cancelBooking);
      if (!row) return;
      await store.save("bookings", { ...row, status: "cancelled", cancelled_at: new Date().toISOString(), cancelled_by: "admin" });
      toast("Бронирование отменено"); render();
    }));
  }
  ...
  render = async function() {
    if (state.view !== "bookings") return baseRender();
    $("#pageTitle").textContent = "Бронирования";
    $("#primaryAction").style.display = "inline-flex";
    $("#primaryAction").textContent = "Новая бронь";
    await renderBookingManager($("#content"));
  };
})();