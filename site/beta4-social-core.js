(() => {
  ...
  function profile(){const p=base(),saved=read(KEYS.profile,{});const status=saved.status&&saved.status!=="closed"?saved.status:"chat";return{id:myId(),name:p.name||"Гость BALI",username:p.username||"",photo:saved.photo||p.avatar||"",cropX:Number(saved.cropX??50),cropY:Number(saved.cropY??40),status,bio:saved.bio||"",active:true,shareTelegram:false,...saved,status,active:true}}
  function cleanPeople(rows){return(rows||[]).filter(item=>item&&item.id&&!String(item.id).startsWith("demo-"))}
  function saveProfile(patch={}){const normalized={...patch,active:true,status:patch.status&&patch.status!=="closed"?patch.status:"chat"};const next={...profile(),...normalized,id:myId(),updatedAt:now()};write(KEYS.profile,next);const rows=cleanPeople(read(KEYS.people,[])).filter(x=>x.id!==next.id);rows.unshift(next);write(KEYS.people,rows);return next}
  function people(){const me=profile(),rows=cleanPeople(read(KEYS.people,[]));return[me,...rows.filter(x=>x.id!==me.id)]}
  function visiblePeople(){return people().filter(x=>x.id!==myId())}
