(() => {
  const hallConfig = () => { try { return JSON.parse(localStorage.getItem("bali_hall_layout_config_v1")) || {}; } catch { return {}; } };
  ...
  function setHallBackground(node) {
    const image = hallConfig().image;
  ...
  async function openEvent(eventId) {
    ...
    if (!event) return;
    ensureDialog();
    const availability = (await store.getAvailability(event.event_date)).sort((a, b) => String(a.name).localeCompare(String(b.name), "ru", { numeric: true }));
    const free = availability.filter((table) => table.available);
    const imageStyle = event.image_url ? `background-image:linear-gradient(180deg,rgba(0,0,0,.08),rgba(7,9,9,.96)),url('${esc(event.image_url)}')` : "";
    ...
    sheet.innerHTML = `<button class="event-details-close" type="button" data-event-close>×</button>
      <section class="event-details-hero" style="${imageStyle}"><div><span class="eyebrow">BALI · ${formatDate(event.event_date)} · ${esc(event.event_time || "23:00")}</span><h2>${esc(event.title)}</h2><p>${free.length ? `${free.length} свободных столов из ${availability.length}` : "Свободных столов сейчас нет"}</p></div></section>
      <div class="event-details-body">
        ...
        <section class="event-availability"><div class="event-availability-head"><div><span class="eyebrow">РАССАДКА НА ${formatDate(event.event_date).toUpperCase()}</span><h3>Свободные места</h3></div><span>${free.length}/${availability.length} свободно</span></div>
          ${availability.length ? `<div class="event-hall-map" id="eventHallMap">${availability.map((table) => `<button type="button" class="event-table ${esc(table.shape || "round")} ${table.available ? "free" : "booked"}" style="left:${Number(table.x)}%;top:${Number(table.y)}%" data-event-table="${table.id}" ${table.available ? "" : "disabled"}><b>${esc(tableLabel(table.name))}</b><small>${table.available ? "свободен" : "занят"}</small></button>`).join("")}</div>` : '<div class="event-no-tables">Схема зала пока не настроена.</div>'}
    ...
    setHallBackground(document.getElementById("eventHallMap"));
