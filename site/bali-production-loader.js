(async () => {
  ...
  const load = (name, limit = 9000) => {
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      script.src = name.startsWith("http") ? name : `./${name}?v=${version}`;
      script.async = false;
  ...
  const optional = async (name, limit = 6500) => {
    try {
      return await load(name, limit);
    } catch (error) {
      console.warn("[BALI optional]", error.message);
      return null;
    }
  };

  const batchOptional = names => Promise.allSettled(names.map(name => optional(name)));
