var ja=Object.defineProperty;var D=(t,i)=>()=>(t&&(i=t(t=0)),i);var ot=(t,i)=>{for(var e in i)ja(t,e,{get:i[e],enumerable:!0})};var xt={};ot(xt,{createPool:()=>Sa,many:()=>w,one:()=>d,transaction:()=>S});import xa from"pg";function Sa(t,i){if(!t)throw new Error("DATABASE_URL is required");let e=Number.parseInt(process.env.DB_POOL_MAX||"",10),a=Number.isFinite(i)&&Number(i)>0?Number(i):Number.isFinite(e)&&e>0?e:process.env.VERCEL?1:10;return new xa.Pool({connectionString:t,max:a,idleTimeoutMillis:3e4,connectionTimeoutMillis:1e4,ssl:/sslmode=require/i.test(t)?{rejectUnauthorized:!1}:void 0})}async function d(t,i,e=[]){return(await t.query(i,e)).rows[0]||null}async function w(t,i,e=[]){return(await t.query(i,e)).rows}async function S(t,i){if(!t.connect)return i(t);let e=await t.connect();try{await e.query("begin");let a=await i(e);return await e.query("commit"),a}catch(a){throw await e.query("rollback"),a}finally{e.release()}}var B=D(()=>{"use strict"});function p(t){return(i,e,a)=>void t(i,e,a).catch(a)}var _,St,Pt,L=D(()=>{"use strict";_=class extends Error{constructor(e,a,n="request_failed",s){super(a);this.status=e;this.code=n;this.details=s}};St=(t,i,e)=>{e(new _(404,`Route ${t.method} ${t.path} was not found`,"not_found"))},Pt=(t,i,e,a)=>{let n=Number(t?.status||500);n>=500&&console.error(`[${i.requestId}]`,t);let s=Number(t?.details?.retryAfter);n===429&&Number.isFinite(s)&&s>0&&e.setHeader("Retry-After",String(Math.ceil(s))),e.status(n).json({error:{code:t?.code||(n>=500?"internal_error":"request_failed"),message:n>=500?"Internal server error":String(t?.message||"Request failed"),details:n>=500?void 0:t?.details,requestId:i.requestId}})}});import{createHash as Pa,createHmac as it,randomBytes as Nt,scrypt as Ta,timingSafeEqual as rt,webcrypto as Na}from"node:crypto";import{promisify as Aa}from"node:util";function Ma(){let t=globalThis.crypto?.subtle||Na.subtle;if(!t)throw new Error("Web Crypto API is unavailable");return t}async function Kt(t,i,e,a){let n=Ma(),s=await n.importKey("raw",Da.encode(t),"PBKDF2",!1,["deriveBits"]),o=await n.deriveBits({name:"PBKDF2",hash:"SHA-256",salt:i,iterations:e},s,a*8);return new Uint8Array(o)}function ge(t,i){return it("sha256",i).update(t).digest("hex")}function ie(){return Nt(32).toString("base64url")}function Rt(t,i,e,a=Math.floor(Date.now()/1e3)){if(!t||!i)throw new le("Telegram authentication is unavailable");let n=new URLSearchParams(t),s=n.get("hash")||"";if(n.delete("hash"),n.delete("signature"),!/^[a-f0-9]{64}$/i.test(s))throw new le("Invalid Telegram signature");let o=[...n.entries()].sort(([b],[v])=>b.localeCompare(v)).map(([b,v])=>`${b}=${v}`).join(`
`),r=it("sha256","WebAppData").update(i).digest(),c=it("sha256",r).update(o).digest(),u=Buffer.from(s,"hex");if(u.length!==c.length||!rt(u,c))throw new le("Invalid Telegram signature");let l=Number(n.get("auth_date")||0);if(!Number.isInteger(l)||l<=0)throw new le("Invalid Telegram auth date");let m=a-l;if(m<-30||m>e)throw new le("Telegram authentication expired");let y;try{y=JSON.parse(n.get("user")||"")}catch{throw new le("Invalid Telegram user payload")}if(!Number.isSafeInteger(y?.id)||y.id<=0||!String(y.first_name||"").trim())throw new le("Invalid Telegram user payload");return{user:y,authDate:l,queryId:n.get("query_id")||void 0,startParam:n.get("start_param")||void 0}}async function Le(t){if(t.length<12)throw new Error("Administrator password must contain at least 12 characters");let i=Nt(16),e=await Kt(t,i,Tt,Ra);return`${At}:${Tt}:${i.toString("base64url")}:${Buffer.from(e).toString("base64url")}`}async function xe(t,i){let e=String(i||"").split(":");if(e[0]===At){let o=Number(e[1]),r=e[2],c=e[3];if(!Number.isInteger(o)||o<1e5||o>2e6||!r||!c)return!1;let u=Buffer.from(c,"base64url");if(!u.length)return!1;let l=await Kt(t,Buffer.from(r,"base64url"),o,u.length),m=Buffer.from(l);return u.length===m.length&&rt(u,m)}let[a,n,s]=e;if(a!=="scrypt"||!n||!s)return!1;try{let o=Buffer.from(s,"base64url"),r=await Ka(t,Buffer.from(n,"base64url"),o.length);return o.length===r.length&&rt(o,r)}catch{return!1}}function Y(t){return Pa("sha256").update(t).digest("hex")}var Ka,At,Tt,Ra,Da,le,me=D(()=>{"use strict";Ka=Aa(Ta),At="pbkdf2-sha256",Tt=6e5,Ra=32,Da=new TextEncoder;le=class extends Error{status=401}});function Oa(t){let e=String(t.get("authorization")||"").trim().match(/^Bearer\s+(.+)$/i);return e?String(e[1]||"").trim():""}function Dt(t,i){return String(t.cookies?.[i]||Oa(t)||"").trim()}function Mt(t,i){return p(async(e,a,n)=>{let s=Dt(e,he);if(!s)return n();let o=await d(t,`select s.id as session_id, s.app_user_key, s.last_seen_at, s.auth_method,
        ...
        where s.token_hash = $1 and s.revoked_at is null and s.expires_at > now()
          and u.account_status = 'active' and u.blocked_at is null`,[ge(s,i.sessionSecret)]);if(o){let r=String(o.telegram_user_id||""),c=o.auth_method==="mobile"||o.auth_method==="telegram"?o.auth_method:r?"telegram":"mobile",u=!1;c==="mobile"&&(u=!!(await d(t,"select must_change_password from public.mobile_credentials where app_user_key = $1",[o.app_user_key]))?.must_change_password),e.userPrincipal={kind:"user",userKey:o.app_user_key,telegramUserId:r,sessionId:String(o.session_id),name:o.name,username:o.username,status:o.account_status,authMethod:c,mustChangePassword:u},Date.now()-new Date(o.last_seen_at).getTime()>3e5&&await t.query(`update public.user_sessions set last_seen_at = now()
            where id = $1 and last_seen_at < now() - interval '5 minutes'`,[o.session_id])}n()})}function Ot(t,i){return p(async(e,a,n)=>{let s=Dt(e,He);if(!s)return n();let o=await d(t,`select s.id as session_id, s.last_seen_at, a.id as admin_id, a.email, a.role, a.status
     ...
     returning request_count`,[e,a,y.toISOString(),m,b.toISOString()]);if(Number(v?.request_count||1)>r){let h=Math.max(1,Math.ceil((b.getTime()-u)/1e3));throw new _(429,"Too many requests","rate_limit_exceeded",{bucket:e,retryAfter:h,requestId:i.requestId})}}var Ca,ye=D(()=>{"use strict";B();L();Ca={"auth.telegram":{limit:10,windowSeconds:60},"auth.admin":{limit:8,windowSeconds:300},"message.create":{limit:20,windowSeconds:60},"message.repeat":{limit:3,windowSeconds:300},"message.mentions":{limit:8,windowSeconds:60},"message.links":{limit:5,windowSeconds:60},"poll.create":{limit:5,windowSeconds:3600},"poll.vote":{limit:30,windowSeconds:60},"event.attach":{limit:10,windowSeconds:3600},"report.create":{limit:5,windowSeconds:3600},"notification.broadcast":{limit:3,windowSeconds:3600},"connection.create":{limit:10,windowSeconds:86400},"invitation.create":{limit:20,windowSeconds:86400},"event_invitation.create":{limit:20,windowSeconds:86400},"direct_message.create":{limit:60,windowSeconds:60},"user_report.create":{limit:5,windowSeconds:86400},"gift.create":{limit:20,windowSeconds:3600},"booking.hold":{limit:10,windowSeconds:60},"game.session":{limit:30,windowSeconds:3600},"content.upload":{limit:30,windowSeconds:3600}}});function f(t,i,e,a=1){let n=String(t??"").trim();if(n.length<a||n.length>e)throw new _(400,`${i} must contain ${a}-${e} characters`,"validation_error");return n}function g(t,i){let e=String(t??"").trim();if(e.length>i)throw new _(400,`Text is longer than ${i} characters`,"validation_error");return e}function P(t,i="id"){let e=String(t||"");if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(e))throw new _(400,`${i} is invalid`,"validation_error");return e}function $(t,i="id"){let e=String(t||"").trim();if(!/^[a-zA-Z0-9:_-]{1,160}$/.test(e))throw new _(400,`${i} is invalid`,"validation_error");return e}function k(t,i,e,a){if(t==null||t==="")return i;let n=Number(t);if(!Number.isInteger(n)||n<e||n>a)throw new _(400,`Value must be an integer from ${e} to ${a}`,"validation_error");return n}function A(t,i=!1){if(t===void 0)return i;if(t===!0||t===!1)return t;throw new _(400,"Value must be a boolean","validation_error")}function K(t,i,e){let a=String(t??"").trim();if(!e.includes(a))throw new _(400,`${i} must be one of: ${e.join(", ")}`,"validation_error");return a}function Q(t,i,e,a){if(t==null||t==="")return i;let n=Number(t);if(!Number.isFinite(n)||n<e||n>a)throw new _(400,`Value must be a number from ${e} to ${a}`,"validation_error");return n}function $e(t,i,e,a,n=200){if(!Array.isArray(t))throw new _(400,`${i} must be an array`,"validation_error");let s=[...new Set(t.map(o=>f(o,i,n)))];if(s.length<e||s.length>a)throw new _(400,`${i} must contain ${e}-${a} unique values`,"validation_error");return s}function E(t){if(!t)return null;let i=new Date(String(t));if(Number.isNaN(i.getTime()))throw new _(400,"Invalid date","validation_error");return i.toISOString()}var Z=D(()=>{"use strict";L()});import{Router as Ea}from"express";function Ct(t){return{ipHash:Y(String(t.ip||"")),userAgent:String(t.get("user-agent")||"").slice(0,500)}}function Et(t,i){let e=Ea();return e.post("/telegram",p(async(a,n)=>{await O(t,a,"auth.telegram",M(a));let s;try{s=Rt(String(a.body?.initData||""),i.telegramBotToken,i.telegramAuthMaxAgeSeconds)}catch(h){throw h instanceof le?new _(401,h.message,"telegram_auth_failed"):h}let o=s.user,r=`tg:${o.id}`,c=`${o.first_name||""} ${o.last_name||""}`.trim()||"\u0413\u043E\u0441\u0442\u044C BALI",u=await d(t,"select app_user_key from public.telegram_accounts where telegram_user_id = $1",[o.id]);if(!u){let h=await d(t,`select user_key
          ...
          limit 1`,[String(o.id),r]);if(h)throw await t.query(`insert into public.data_merge_review(
           ...
           on conflict (entity_type, legacy_id) do update
             set candidate_user_key = excluded.candidate_user_key,
                 ...
                 end`,[String(o.id),h.user_key,"Legacy Telegram ID requires administrator review before account binding",JSON.stringify({signedTelegramUser:o,canonicalUserKey:r})]),new _(409,"Account binding requires administrator review","identity_merge_review_required")}let l=u?.app_user_key||r,m=ie(),y=new Date(Date.now()+i.sessionTtlSeconds*1e3),b=Ct(a),v=await S(t,async h=>{let j=await d(h,`insert into public.app_users(
         ...
         ) values ($1,'app_open','telegram','user',$1,$2::jsonb)`,[l,JSON.stringify({firstOpen:!u})]),j});n.cookie(he,m,Ke(i,i.sessionTtlSeconds*1e3)),n.status(201).json({user:{id:v.user_key,name:v.name,username:v.username,avatar:v.avatar},environment:i.environment,expiresAt:y.toISOString()})})),e.get("/session",H,p(async(a,n)=>{n.json({user:{id:a.userPrincipal.userKey,name:a.userPrincipal.name,username:a.userPrincipal.username},environment:i.environment})})),e.post("/logout",H,p(async(a,n)=>{await t.query("update public.user_sessions set revoked_at = now() where id = $1",[a.userPrincipal.sessionId]),n.clearCookie(he,{path:"/"}),n.status(204).end()})),e.post("/admin/login",p(async(a,n)=>{await O(t,a,"auth.admin",M(a));let s=f(a.body?.email,"email",320).toLowerCase(),o=f(a.body?.password,"password",1e3),r=await d(t,"select id, email, password_hash, role, status from public.admin_users where email = $1",[s]);if(!r||r.status!=="active"||!await xe(o,r.password_hash))throw new _(401,"Invalid administrator credentials","admin_login_failed");let c=ie(),u=new Date(Date.now()+720*60*1e3),l=Ct(a);await t.query(`insert into public.admin_sessions(
         admin_user_id, token_hash, expires_at, ip_hash, user_agent
       ) values ($1,$2,$3,$4,$5)`,[r.id,ge(c,i.sessionSecret),u.toISOString(),l.ipHash,l.userAgent]),n.cookie(He,c,Ke(i,720*60*1e3)),n.json({admin:{id:r.id,email:r.email,role:r.role},expiresAt:u})})),e.get("/admin/session",X,p(async(a,n)=>{n.json({admin:a.adminPrincipal})})),e.post("/admin/logout",X,p(async(a,n)=>{await t.query("update public.admin_sessions set revoked_at = now() where id = $1",[a.adminPrincipal.sessionId]),n.clearCookie(He,{path:"/"}),n.status(204).end()})),e}var Ut=D(()=>{"use strict";B();L();z();ye();me();Z()});import{Router as Ua}from"express";function ct(t){let e=f(t,"phone",40).replace(/\D/g,"");if(e.length<9||e.length>15)throw new _(400,"\u0412\u0432\u0435\u0434\u0438\u0442\u0435 \u043A\u043E\u0440\u0440\u0435\u043A\u0442\u043D\u044B\u0439 \u043D\u043E\u043C\u0435\u0440 \u0442\u0435\u043B\u0435\u0444\u043E\u043D\u0430","invalid_phone");return`+${e}`}function Bt(t){let i=f(t,"telegramUsername",64).replace(/^@+/,"");if(!/^[A-Za-z0-9_]{5,32}$/.test(i))throw new _(400,"\u0412\u0432\u0435\u0434\u0438\u0442\u0435 Telegram username \u0432 \u0444\u043E\u0440\u043C\u0430\u0442\u0435 @username","invalid_telegram_username");return i}function Ba(t){return{ipHash:Y(String(t.ip||"")),userAgent:String(t.get("user-agent")||"").slice(0,500)}}async function La(t,i,e,a,n){let s=ie(),o=new Date(Date.now()+i.sessionTtlSeconds*1e3),r=Ba(e);return await t.query(`insert into public.user_sessions(
       app_user_key, token_hash, telegram_auth_date, expires_at, ip_hash, user_agent, auth_method
     ) values ($1,$2,now(),$3,$4,$5,'mobile')`,[n.user_key,ge(s,i.sessionSecret),o.toISOString(),r.ipHash,r.userAgent]),a.cookie(he,s,Ke(i,i.sessionTtlSeconds*1e3)),{expiresAt:o,sessionToken:s}}function Lt(t,i){let e=Ua();return e.get("/mobile/session",H,p(async(a,n)=>{n.json({user:{id:a.userPrincipal.userKey,name:a.userPrincipal.name,username:a.userPrincipal.username},authMethod:a.userPrincipal.authMethod,mustChangePassword:a.userPrincipal.mustChangePassword})})),e.post("/mobile/register-request",p(async(a,n)=>{await O(t,a,"auth.mobile_request",M(a));let s=ct(a.body?.phone),o=Bt(a.body?.telegramUsername),r=f(a.body?.displayName,"displayName",120,2);if(await d(t,"select app_user_key from public.mobile_credentials where phone = $1",[s]))throw new _(409,"\u0410\u043A\u043A\u0430\u0443\u043D\u0442 \u0441 \u044D\u0442\u0438\u043C \u043D\u043E\u043C\u0435\u0440\u043E\u043C \u0443\u0436\u0435 \u0441\u0443\u0449\u0435\u0441\u0442\u0432\u0443\u0435\u0442","account_exists");let u=await d(t,`insert into public.mobile_access_requests(
       ...
       do update set telegram_username = excluded.telegram_username,
                     ...
                     requested_at = now(),
                     updated_at = now()
       returning id, status, requested_at`,[s,o,r]);n.status(202).json({request:u,message:"\u0417\u0430\u044F\u0432\u043A\u0430 \u043E\u0442\u043F\u0440\u0430\u0432\u043B\u0435\u043D\u0430 \u0430\u0434\u043C\u0438\u043D\u0438\u0441\u0442\u0440\u0430\u0442\u043E\u0440\u0443"})})),e.post("/mobile/reset-request",p(async(a,n)=>{await O(t,a,"auth.mobile_reset",M(a));let s=ct(a.body?.phone),o=Bt(a.body?.telegramUsername),r=await d(t,`select app_user_key from public.mobile_credentials
        where phone = $1 and lower(telegram_username) = lower($2)`,[s,o]);r&&await t.query(`insert into public.mobile_access_requests(
         ...
         do update set telegram_username = excluded.telegram_username,
                       ...
                       requested_at = now(),
                       updated_at = now()`,[s,o,r.app_user_key]),n.status(202).json({message:"\u0415\u0441\u043B\u0438 \u0434\u0430\u043D\u043D\u044B\u0435 \u0441\u043E\u0432\u043F\u0430\u043B\u0438, \u0430\u0434\u043C\u0438\u043D\u0438\u0441\u0442\u0440\u0430\u0442\u043E\u0440 \u043F\u043E\u043B\u0443\u0447\u0438\u043B \u0437\u0430\u043F\u0440\u043E\u0441 \u043D\u0430 \u0432\u043E\u0441\u0441\u0442\u0430\u043D\u043E\u0432\u043B\u0435\u043D\u0438\u0435"})})),e.post("/mobile/login",p(async(a,n)=>{await O(t,a,"auth.mobile_login",M(a));let s=ct(a.body?.phone),o=f(a.body?.password,"password",256),r=await d(t,`select c.app_user_key, c.password_hash, c.must_change_password, c.locked_until,
        ...
        where user_key = $1`,[r.app_user_key]);let{expiresAt:c,sessionToken:u}=await La(t,i,a,n,r);await t.query(`insert into public.analytics_events(user_key, event_name, source, entity_type, entity_id, properties)
       values ($1,'app_open','mobile','user',$1,$2::jsonb)`,[r.app_user_key,JSON.stringify({authMethod:"mobile"})]),n.json({user:{id:r.user_key,name:r.name,username:r.username,avatar:r.avatar},mustChangePassword:!!r.must_change_password,expiresAt:c.toISOString(),accessToken:u,tokenType:"Bearer"})})),e.post("/mobile/change-password",H,p(async(a,n)=>{await O(t,a,"auth.mobile_password",M(a));let s=f(a.body?.currentPassword,"currentPassword",256),o=f(a.body?.newPassword,"newPassword",128,12),r=await d(t,"select password_hash from public.mobile_credentials where app_user_key = $1",[a.userPrincipal.userKey]);if(!r||!await xe(s,r.password_hash))throw new _(401,"\u0422\u0435\u043A\u0443\u0449\u0438\u0439 \u043F\u0430\u0440\u043E\u043B\u044C \u0432\u0432\u0435\u0434\u0451\u043D \u043D\u0435\u0432\u0435\u0440\u043D\u043E","current_password_invalid");if(await xe(o,r.password_hash))throw new _(400,"\u041D\u043E\u0432\u044B\u0439 \u043F\u0430\u0440\u043E\u043B\u044C \u0434\u043E\u043B\u0436\u0435\u043D \u043E\u0442\u043B\u0438\u0447\u0430\u0442\u044C\u0441\u044F \u043E\u0442 \u0432\u0440\u0435\u043C\u0435\u043D\u043D\u043E\u0433\u043E","password_not_changed");let c=await Le(o);await t.query(`update public.mobile_credentials
        ...
        where app_user_key = $1 and id <> $2 and revoked_at is null`,[a.userPrincipal.userKey,a.userPrincipal.sessionId]),n.json({ok:!0,mustChangePassword:!1})})),e}var Ht=D(()=>{"use strict";B();L();z();ye();me();Z()});async function ke(t,i,e,a){let n=await d(t,`select
     ...
     where m.clan_id = $1 and m.user_key = $2`,[e,i.userKey]);if(!n||n.membership_status!=="active"||n.clan_status!=="active")return{allowed:!1,source:"none"};let s=await d(t,`select * from public.clan_chat_restrictions
      ...
      where clan_id = $1 and user_key = $2 and revoked_at is null
        and (expires_at is null or expires_at > now())
      order by created_at asc`,[e,i.userKey])).rows,s=new Set(a.leader_user_key===i.userKey||a.role==="leader"?Qt:ut);for(let o of n)o.effect==="deny"?s.delete(o.permission_key):s.add(o.permission_key);if(!a.enabled)for(let o of[...s])["chat.read","message.read"].includes(o)||s.delete(o);if(a.read_only)for(let o of Jt)s.delete(o);return[...s].sort()}var ut,Qt,Jt,Vt=D(()=>{"use strict";B();L();ut=new Set(["chat.read","chat.write","chat.reply","message.read","message.create","message.reply","message.delete_own","poll.read","poll.vote","event.read","report.create"]),Qt=new Set([...ut,"chat.enable","chat.disable","chat.set_read_only","chat.settings.update","message.delete_any","message.pin","poll.create","poll.finish","poll.cancel","poll.delete","poll.pin","event.attach","event.detach","event.set_primary","event.link_poll","event.pin","announcement.create","notification.broadcast","member.restrict_chat","member.unrestrict_chat","audit.read"]),Jt=new Set(["chat.write","chat.reply","message.create","message.reply"])});async function ee(t,i,e){let a=e.chatId||i.permissionDecision?.chat?.chat_id||null;!a&&e.clanId&&(a=(await d(t,"select id from public.clan_chats where clan_id = $1",[e.clanId]))?.id||null);let n=i.userPrincipal?.userKey||(["user","leader","delegate"].includes(e.actorType)?e.actorId:null);await t.query(`insert into public.clan_chat_audit_log(
     ...
     ) values ($1,$2,$3,$4,$5,$6,$7,$8::jsonb,$9::jsonb,$10,$11)`,[i.adminPrincipal.adminId,i.adminPrincipal.email,e.action,e.targetType,e.targetId||"",i.requestId,e.reason||"",e.before===void 0?null:JSON.stringify(e.before),e.after===void 0?null:JSON.stringify(e.after),Y(String(i.ip||"")),String(i.get("user-agent")||"").slice(0,500)])}var ve=D(()=>{"use strict";B();me()});function Ft(t,i){let e=["avatar","status","clan"].includes(i)?"public":i==="events"?"clan":"private",a=String(t?.[i]||e);return dt.has(a)?a:"private"}async function lt(t,i,e){let a=await d(t,`select user_row.user_key,
      ...
      limit 1`,[i,e]))throw new _(404,"BALI profile was not found","not_found");if(!a.discoverable&&!n&&!c&&!u)throw new _(404,"BALI profile was not found","not_found");let m=b=>{if(n)return!0;let v=Ft(a.profile_privacy,b);return v==="public"?!0:v==="clan"?c:!1},y={id:a.user_key,name:a.name,bio:a.bio||"",interests:a.interests||[],gender:a.gender||"unspecified",actions:{canConnect:!n&&!!a.allow_connections,canInvite:!n&&!!a.allow_event_invites,canGift:!n&&!!a.allow_gifts},privacy:n?a.profile_privacy:void 0};return m("avatar")&&a.avatar&&(y.avatar=a.avatar),m("username")&&a.username&&(y.username=a.username),m("phone")&&a.phone&&(y.phone=a.phone),m("birth_date")&&a.birth_date&&(y.birthDate=a.birth_date),m("status")&&a.status_text&&(y.status=a.status_text),m("clan")&&(y.clans=await(async()=>(await t.query(`select clan.id, clan.name, clan.clan_type,
        ...
        limit 1`,[e])),y}async function Qe(t,i,e){let a=[...new Set(e)].filter(Boolean).slice(0,100);if(!a.length)return[];if(!await d(t,`select user_key from public.app_users
      where user_key = $1 and account_status = 'active'`,[i]))throw new _(401,"Viewer account is unavailable","authentication_required");let[s,o,r,c,u,l]=await Promise.all([w(t,`select user_row.user_key,
        ...
        order by attendance.user_key,
                 coalesce(runtime.starts_at, event.event_date::timestamptz)`,[a])]),m=new Set(o.map(I=>I.target_user_key)),y=new Set(r.map(I=>I.target_user_key)),b=new Set(c.map(I=>I.target_user_key)),v=new Map;for(let I of u){let x=v.get(I.target_user_key)||[];x.push({id:I.id,name:I.name,clan_type:I.clan_type,logo_url:I.logo_url,description:I.description}),v.set(I.target_user_key,x)}let h=new Map(l.map(I=>[I.target_user_key,{id:I.id,title:I.title,event_date:I.event_date,event_time:I.event_time,status:I.status}])),j=new Map(s.map(I=>[I.user_key,I])),N=[];for(let I of a){let x=j.get(I);if(!x||b.has(I))continue;let R=m.has(I);if(!x.discoverable&&!R&&!y.has(I))continue;let J=C=>{let W=Ft(x.profile_privacy,C);return W==="public"||W==="clan"&&R},U={id:x.user_key,user_key:x.user_key,name:x.name,bio:x.bio||"",interests:x.interests||[],gender:x.gender||"unspecified",actions:{canConnect:!!x.allow_connections,canInvite:!!x.allow_event_invites,canGift:!!x.allow_gifts}};J("avatar")&&x.avatar&&(U.avatar=x.avatar),J("username")&&x.username&&(U.username=x.username),J("phone")&&x.phone&&(U.phone=x.phone),J("birth_date")&&x.birth_date&&(U.birthDate=x.birth_date),J("status")&&x.status_text&&(U.status=x.status_text),J("clan")&&(U.clans=v.get(I)||[]),J("events")&&(U.upcomingEvent=h.get(I)||null),N.push(U)}return N}var dt,Gt,_t=D(()=>{"use strict";B();L();dt=new Set(["public","clan","private"]),Gt=["avatar","username","phone","birth_date","status","events","clan"]});import{Router as Ha}from"express";function F(t){let i=t.permissionDecision?.chat?.chat_id;if(!i)throw new _(403,"Clan chat is unavailable","chat_unavailable");return String(i)}function Qa(t){return t.deleted_at?t.deleted_by_type==="admin"?"\u0421\u043E\u043E\u0431\u0449\u0435\u043D\u0438\u0435 \u0443\u0434\u0430\u043B\u0435\u043D\u043E \u0430\u0434\u043C\u0438\u043D\u0438\u0441\u0442\u0440\u0430\u0442\u043E\u0440\u043E\u043C BALI":["leader","delegate"].includes(t.deleted_by_type)?"\u0421\u043E\u043E\u0431\u0449\u0435\u043D\u0438\u0435 \u0443\u0434\u0430\u043B\u0435\u043D\u043E \u0440\u0443\u043A\u043E\u0432\u043E\u0434\u0438\u0442\u0435\u043B\u0435\u043C \u043A\u043B\u0430\u043D\u0430":"\u0421\u043E\u043E\u0431\u0449\u0435\u043D\u0438\u0435 \u0443\u0434\u0430\u043B\u0435\u043D\u043E \u0430\u0432\u0442\u043E\u0440\u043E\u043C":t.body}function Yt(t){return{id:t.id,body:Qa(t),messageType:t.message_type,author:t.author_user_key?{id:t.author_user_key,name:t.author_name||"\u0423\u0447\u0430\u0441\u0442\u043D\u0438\u043A BALI"}:null,reply:t.reply_to_message_id?{id:t.reply_to_message_id,body:t.reply_deleted_at?"\u0421\u043E\u043E\u0431\u0449\u0435\u043D\u0438\u0435 \u0443\u0434\u0430\u043B\u0435\u043D\u043E":t.reply_body,authorName:t.reply_author_name||"\u0423\u0447\u0430\u0441\u0442\u043D\u0438\u043A BALI"}:null,deleted:!!t.deleted_at,createdAt:t.created_at,updatedAt:t.updated_at}}async function Wt(t,i,e,a){return(await w(t,`select m.*, u.name as author_name,
      ...
      limit $3`,[i,e,a])).reverse().map(Yt)}async function Ja(t,i,e){let a=await w(t,`select * from public.clan_chat_polls
      ...
      order by created_at desc limit 30`,[i]);if(!a.length)return[];let n=[];for(let s of a){let o=await w(t,`select o.id, o.label, o.sort_order, count(v.id)::integer as votes
        ...
        where poll_id = $1 and voter_user_key = $2`,[s.id,e]);n.push({...s,options:o,myOptionIds:r.map(c=>c.option_id),responseCreatesCheckin:!1})}return n}async function za(t,i,e,a){let s={message:"clan_chat_messages",poll:"clan_chat_polls",event:"clan_chat_events",announcement:"clan_chat_announcements"}[i];if(!s)throw new _(400,"Unsupported pin target","validation_error");if(!await d(t,`select id from public.${s} where id = $1 and chat_id = $2`,[e,a]))throw new _(404,"Pin target was not found","not_found")}function Zt(t){let i=Ha();i.use(H),i.get("/",p(async(e,a)=>{let n=await w(t,`select c.id, c.name, c.clan_type, m.role, ch.id as chat_id, ch.enabled, ch.read_only
         from public.clan_memberships m
        ...
        where clan.id = $1
          ...
          limit 1`,[n,o.clan_type])]);if(!r)throw new _(404,"Invitee was not found","not_found");if(c)throw new _(409,"This user already belongs to a clan in the same category","clan_category_membership_conflict",{clanId:c.id,clanName:c.name,clanType:o.clan_type});try{let u=await S(t,async l=>{let m=await d(l,`insert into public.clan_invitations(
           ...
           on conflict (idempotency_key) do nothing`,[n,"\u041F\u0440\u0438\u0433\u043B\u0430\u0448\u0435\u043D\u0438\u0435 \u0432 \u043A\u043B\u0430\u043D",`${e.userPrincipal.name} \u043F\u0440\u0438\u0433\u043B\u0430\u0448\u0430\u0435\u0442 \u0432\u0430\u0441 \u0432 \xAB${o.name}\xBB.`,JSON.stringify({invitationId:m.id,clanId:o.id}),`clan-invitation:${m.id}`]),m});a.status(201).json({invitation:u})}catch(u){throw u?.code==="23505"?new _(409,"A pending invitation already exists","clan_invitation_pending"):u}})),i.get("/:clanId/chat",G(t,"chat.read"),p(async(e,a)=>{let n=F(e),s=k(e.query.limit,50,1,100),o=e.query.before?E(e.query.before):null,[r,c,u,l,m,y,b]=await Promise.all([Wt(t,n,o,s),Ja(t,n,e.userPrincipal.userKey),w(t,`select ce.id, ce.is_primary, ce.created_at,
        ...
        order by event_date, event_time limit 100`);a.json({events:n})})),i.post("/:clanId/messages",G(t,"message.create"),p(async(e,a)=>{await O(t,e,"message.create",M(e,e.params.clanId));let n=f(e.body?.body,"body",4e3),s=e.body?.replyToId?P(e.body.replyToId,"replyToId"):null,o=F(e);if(s){if(!(await ke(t,e.userPrincipal,e.params.clanId,"message.reply")).allowed)throw new _(403,"Reply is not permitted","permission_denied");if(!await d(t,"select id from public.clan_chat_messages where id = $1 and chat_id = $2",[s,o]))throw new _(404,"Reply target was not found","not_found")}let r=n.match(/https?:\/\/\S+/gi)||[];r.length&&await O(t,e,"message.links",M(e,e.params.clanId),r.length);let c=n.match(/@[\p{L}\p{N}_]{2,32}/gu)||[];c.length&&await O(t,e,"message.mentions",M(e,e.params.clanId),c.length);let u=Y(n.toLocaleLowerCase("ru").replace(/\s+/g," ").trim()).slice(0,24);await O(t,e,"message.repeat",M(e,`${e.params.clanId}:${u}`));let l=await S(t,async m=>{let y=await d(m,`insert into public.clan_chat_messages(
         ...
         returning *`,[o,e.userPrincipal.userKey,n,s]);return s&&await m.query(`insert into public.clan_chat_message_replies(message_id, parent_message_id)
           values ($1,$2)`,[y.id,s]),y});a.status(201).json({message:Yt({...l,author_name:e.userPrincipal.name})})})),i.delete("/:clanId/messages/:messageId",p(async(e,a)=>{let n=await ke(t,e.userPrincipal,e.params.clanId,"message.delete_own");if(!n.membership?.chat_id)throw new _(403,"Clan access is denied","permission_denied");let s=P(e.params.messageId,"messageId"),o=await d(t,"select * from public.clan_chat_messages where id = $1 and chat_id = $2",[s,n.membership.chat_id]);if(!o)throw new _(404,"Message was not found","not_found");if(o.deleted_at)return a.status(204).end();let r=o.author_user_key===e.userPrincipal.userKey,c=(Date.now()-new Date(o.created_at).getTime())/1e3,u=n;if((!r||c>Number(n.chat?.own_delete_window_seconds||0))&&(u=await ke(t,e.userPrincipal,e.params.clanId,"message.delete_any")),!u.allowed)throw new _(403,"Message deletion is not permitted","permission_denied");let l=g(e.body?.reason,500),m=ne(u),y=m==="leader"||m==="delegate"?"\u0421\u043E\u043E\u0431\u0449\u0435\u043D\u0438\u0435 \u0443\u0434\u0430\u043B\u0435\u043D\u043E \u0440\u0443\u043A\u043E\u0432\u043E\u0434\u0438\u0442\u0435\u043B\u0435\u043C \u043A\u043B\u0430\u043D\u0430":"\u0421\u043E\u043E\u0431\u0449\u0435\u043D\u0438\u0435 \u0443\u0434\u0430\u043B\u0435\u043D\u043E \u0430\u0432\u0442\u043E\u0440\u043E\u043C";await t.query(`update public.clan_chat_messages
...
${s}`:s]),c});await ee(t,e,{actorType:ne(e.permissionDecision),actorId:e.userPrincipal.userKey,permissionKey:"announcement.create",action:"announcement.create",targetType:"announcement",targetId:o.id,clanId:e.params.clanId,after:{title:n,body:s}}),a.status(201).json({announcement:o})})),i.post("/:clanId/pins",p(async(e,a)=>{let n=f(e.body?.targetType,"targetType",30),s=P(e.body?.targetId,"targetId"),o=n==="message"?"message.pin":n==="poll"?"poll.pin":n==="event"?"event.pin":"announcement.create",r=await ke(t,e.userPrincipal,e.params.clanId,o);if(e.permissionDecision=r,!r.allowed)throw new _(403,"Pin is not permitted","permission_denied");await za(t,n,s,String(r.chat.chat_id));let c=await d(t,`insert into public.clan_chat_pins(chat_id, target_type, target_id, pinned_by_user_key)
       ...
       returning *`,[F(e),e.userPrincipal.userKey,n,A(e.body?.announcementsOnly)]);a.json({preference:s})})),i.get("/:clanId/audit",G(t,"audit.read"),p(async(e,a)=>{let n=k(e.query.limit,50,1,100),s=await w(t,`select * from public.clan_chat_audit_log
        ...
        order by created_at desc limit $3`,[e.params.clanId,e.userPrincipal.userKey,n]);a.json({audit:s})})),i}var Xt=D(()=>{"use strict";B();L();z();Vt();ye();Z();ve();_t();me()});import{randomUUID as Va}from"node:crypto";import{Router as Ga}from"express";function te(...t){return(i,e,a)=>{if(!i.adminPrincipal||!t.includes(i.adminPrincipal.role))return a(new _(403,"Administrator role does not permit this action","admin_permission_denied"));a()}}function Fa(t){let i=String(t||"").trim().toLowerCase();if(i!=="user"&&i!=="corporate")throw new _(400,"clanType must be user or corporate","validation_error");return i}async function _e(t,i){let e=await d(t,`select c.id as clan_id, c.name as clan_name, c.clan_type, c.status as clan_status,
            c.rating_points,
            c.leader_user_key, ch.*
       from public.clans c
       join public.clan_chats ch on ch.clan_id = c.id
      where c.id = $1`,[i]);if(!e)throw new _(404,"Clan chat was not found","not_found");return e}async function ae(t,i,e){return ee(t,i,{actorType:"admin",actorId:i.adminPrincipal.adminId,...e})}function Wa(t){return`"${(t==null?"":typeof t=="object"?JSON.stringify(t):String(t)).replaceAll('"','""')}"`}function qt(t){let i=Ga();return i.use(X),i.get("/permissions",p(async(e,a)=>{let n=await w(t,"select * from public.clan_chat_permissions order by permission_key");a.json({permissions:n})})),i.get("/users",p(async(e,a)=>{let n=String(e.query.search||"").trim(),s=await w(t,`select u.user_key, u.name, u.username,
        ...
        limit 200`,[n]);a.json({users:s})})),i.post("/clans",te("admin","superadmin"),p(async(e,a)=>{let n=f(e.body?.name,"name",120),s=Fa(e.body?.clanType),o=$(e.body?.leaderUserKey,"leaderUserKey"),r=k(e.body?.ratingPoints,0,0,1e9),c=g(e.body?.reason,1e3),u=`clan-${Va()}`,l=await d(t,`select user_key, name, username from public.app_users
...
${o}`:o]),u});await ae(t,e,{permissionKey:"announcement.create",action:"announcement.create",targetType:"announcement",targetId:r.id,clanId:e.params.clanId,after:r}),a.status(201).json({announcement:r})})),i.delete("/clans/:clanId/polls/:pollId",te("admin","superadmin","moderator"),p(async(e,a)=>{let n=await _e(t,e.params.clanId),s=P(e.params.pollId,"pollId"),o=await d(t,`update public.clan_chat_polls set status = 'deleted'
        ...
        order by created_at desc limit ${n}`,u);if(e.query.format==="csv"){let y=["id","created_at","actor_type","actor_id","actor_telegram_id","actor_user_key","permission_key","action","target_type","target_id","clan_id","chat_id","request_id","reason","before_value","after_value"],b=[y.join(","),...m.map(v=>y.map(h=>Wa(v[h])).join(","))].join(`
`);a.type("text/csv").attachment("bali-clan-audit.csv").send(b);return}a.json({audit:m})})),i.get("/rate-limits",p(async(e,a)=>{let n=await w(t,"select * from public.rate_limit_settings order by bucket");a.json({settings:n})})),i.put("/rate-limits/:bucket",te("admin","superadmin"),p(async(e,a)=>{let n=String(e.params.bucket||"");if(!/^[a-z][a-z0-9.-]{1,80}$/.test(n))throw new _(400,"Invalid rate-limit bucket","validation_error");let s=await d(t,"select * from public.rate_limit_settings where bucket = $1",[n]);if(!s)throw new _(404,"Rate-limit setting was not found","not_found");let o=await d(t,`update public.rate_limit_settings
          ...
          where bucket = $5 returning *`,[k(e.body?.limitCount,Number(s.limit_count),1,1e5),k(e.body?.windowSeconds,Number(s.window_seconds),1,86400),e.body?.enabled===void 0?s.enabled:A(e.body.enabled),e.adminPrincipal.adminId,n]);await ae(t,e,{action:"rate_limit.update",targetType:"rate_limit_setting",targetId:n,before:s,after:o}),a.json({setting:o})})),i}var en=D(()=>{"use strict";B();L();z();ve();Z()});import{Router as Ya}from"express";async function Re(t,i,e=!1){let a=await d(t,`select * from public.hall_layouts
      where id = $1
        ...
        where layout_id = $1 and active = true
        order by sort_order, id`,[i])]);return{layout:a,tables:n,elements:s}}function tn(t){let i=Ya();return i.use(H),i.get("/",p(async(e,a)=>{let n=await w(t,`select id, layout_family_key, name, internal_description,
              canvas_width, canvas_height, background_url, version, published_at
         from public.hall_layouts
        where status = 'published'
        order by name, version desc`);a.json({layouts:n})})),i.get("/:layoutId",p(async(e,a)=>{let n=$(e.params.layoutId,"layoutId");a.json(await Re(t,n))})),i}var Je=D(()=>{"use strict";B();L();z();Z()});import{randomUUID as ze}from"node:crypto";import{Router as Za}from"express";async function Se(t,i){let e=await d(t,"select * from public.hall_layouts where id = $1",[i]);if(!e)throw new _(404,"Layout was not found","not_found");if(e.status!=="draft")throw new _(409,"Only a draft layout can be edited; clone this version first","layout_not_editable");return e}function cn(t){let i=Za();return i.use(X),i.get("/layouts",p(async(e,a)=>{let n=e.query.status?K(e.query.status,"status",Xa):null,s=await w(t,`select layout.*,
       ...
       returning *`,[s,o,n,g(e.body?.internalDescription,2e3),k(e.body?.canvasWidth,1e3,240,1e4),k(e.body?.canvasHeight,1400,240,1e4),g(e.body?.backgroundUrl,2e3),e.adminPrincipal.adminId]);await T(t,e,{action:"layout.create",targetType:"hall_layout",targetId:s,after:r}),a.status(201).json({layout:r})})),i.post("/layouts/:layoutId/clone",p(async(e,a)=>{let n=$(e.params.layoutId,"layoutId"),s=await S(t,async o=>{let r=await d(o,"select * from public.hall_layouts where id = $1 for update",[n]);if(!r)throw new _(404,"Layout was not found","not_found");let c=await d(o,`select coalesce(max(version), 0)::integer + 1 as version
           from public.hall_layouts where layout_family_key = $1`,[r.layout_family_key]),u=`layout-${ze()}`,l=await d(o,`insert into public.hall_layouts(
           id, layout_family_key, name, internal_description, canvas_width,
       ...
       returning *`,[s,n,f(e.body?.tableNumber,"tableNumber",80),g(e.body?.name,160),Q(e.body?.x,0,-1e4,1e4),Q(e.body?.y,0,-1e4,1e4),Q(e.body?.width,8,.1,1e4),Q(e.body?.height,8,.1,1e4),Q(e.body?.rotation,0,-3600,3600),K(e.body?.shape||"round","shape",nn),o,r,Q(e.body?.minimumDeposit,0,0,1e9),K(e.body?.tableType||"regular","tableType",an),g(e.body?.description,2e3),K(e.body?.status||"available","status",sn),k(e.body?.sortOrder,0,-1e6,1e6),A(e.body?.active,!0)]);await T(t,e,{action:"layout.table.create",targetType:"layout_table",targetId:s,after:c}),a.status(201).json({table:c})})),i.patch("/layouts/:layoutId/tables/:tableId",p(async(e,a)=>{let n=$(e.params.layoutId,"layoutId"),s=$(e.params.tableId,"tableId");await Se(t,n);let o=await d(t,"select * from public.layout_tables where id = $1 and layout_id = $2",[s,n]);if(!o)throw new _(404,"Table was not found","not_found");let r=k(e.body?.capacity,Number(o.capacity),1,100),c=k(e.body?.recommendedGuests,Math.min(Number(o.recommended_guests),r),1,r),u=await d(t,`update public.layout_tables
       ...
       returning *`,[n,K(e.body?.elementType,"elementType",on),g(e.body?.label,160),Q(e.body?.x,0,-1e4,1e4),Q(e.body?.y,0,-1e4,1e4),Q(e.body?.width,10,.1,1e4),Q(e.body?.height,10,.1,1e4),Q(e.body?.rotation,0,-3600,3600),JSON.stringify(s),k(e.body?.sortOrder,0,-1e6,1e6),A(e.body?.active,!0)]);await T(t,e,{action:"layout.element.create",targetType:"layout_element",targetId:o.id,after:o}),a.status(201).json({element:o})})),i.patch("/layouts/:layoutId/elements/:elementId",p(async(e,a)=>{let n=$(e.params.layoutId,"layoutId"),s=P(e.params.elementId,"elementId");await Se(t,n);let o=await d(t,"select * from public.hall_layout_elements where id = $1 and layout_id = $2",[s,n]);if(!o)throw new _(404,"Layout element was not found","not_found");let r=e.body?.style===void 0?o.style:e.body.style&&typeof e.body.style=="object"&&!Array.isArray(e.body.style)?e.body.style:(()=>{throw new _(400,"style must be an object","validation_error")})(),c=await d(t,`update public.hall_layout_elements
        ...
        returning *`,[k(e.body?.holdSeconds,Number(n.hold_seconds),60,3600),e.body?.allowCapacityOverride===void 0?n.allow_capacity_override:A(e.body.allowCapacityOverride),e.body?.autoConfirm===void 0?n.auto_confirm:A(e.body.autoConfirm),e.adminPrincipal.adminId]);await T(t,e,{action:"booking.settings.update",targetType:"booking_settings",targetId:"singleton",reason:g(e.body?.reason,1e3),before:n,after:s}),a.json({settings:s})})),i}var Xa,nn,an,sn,on,rn,un=D(()=>{"use strict";ve();B();L();z();Z();Je();Xa=["draft","published","archived"],nn=["round","square","rectangle","sofa","custom"],an=["regular","vip","bar","sofa","clan","service"],sn=["available","unavailable","vip_only","clan_only"],on=["stage","dance_floor","bar","entrance","exit","cloakroom","restroom","dj_zone","stairs","partition","decoration","label"],rn=["new","pending","confirmed","cancelled","checked_in","no_show","completed"]});async function re(t,i){if(!Number.isSafeInteger(i.amount)||i.amount===0)throw new _(400,"Point amount must be a non-zero safe integer","validation_error");return S(t,async e=>{let a=await d(e,"select * from public.point_ledger where idempotency_key = $1",[i.idempotencyKey]);if(a){if(a.user_key!==i.userKey||Number(a.amount)!==i.amount)throw new _(409,"Idempotency key was already used for another operation","idempotency_conflict");return{ledger:a,replayed:!0}}await e.query(`insert into public.point_accounts(user_key)
       values ($1)
       ...
       returning *`,[i.userKey,i.amount,s,o,i.operationType,i.sourceType,i.sourceId||"",i.reason||"",i.administratorId||null,i.idempotencyKey,JSON.stringify(i.metadata||{})]),replayed:!1}})}var De=D(()=>{"use strict";B();L()});async function pt(t,i,e=null){return S(t,async a=>{let n=await d(a,"select * from public.game_seasons where id = $1 for update",[i]);if(!n)throw new _(404,"Game season was not found","not_found");if(n.status==="completed"){let c=await w(a,`select user_key, position, reward_payload, status
        ...
        where id = $1 returning *`,[i]),winners:s,replayed:!1}})}async function dn(t){let i=await w(t,`select id from public.game_seasons
      where status = 'active' and ends_at <= now()
      order by ends_at limit 20`);for(let e of i)await pt(t,e.id)}var mt=D(()=>{"use strict";B();De();L()});import{randomUUID as Ve}from"node:crypto";import{Router as qa}from"express";function pe(t,i){if(!t||Array.isArray(t)||typeof t!="object")throw new _(400,`${i} must be an object`,"validation_error");return t}function be(t,i){if(!Array.isArray(t))throw new _(400,`${i} must be an array`,"validation_error");return t}async function Me(t,i,e){if(!new Set(["reward_definitions","gift_catalog","vip_plans","shop_items","game_seasons"]).has(i))throw new Error("Unsupported administrator catalog table");let n=await d(t,`select * from public.${i} where id = $1`,[e]);if(!n)throw new _(404,"Catalog item was not found","not_found");return n}function mn(t){let i=qa();return i.use(X),i.get("/economy",p(async(e,a)=>{let[n,s,o,r,c,u,l,m]=await Promise.all([d(t,"select * from public.economy_settings where singleton = true"),w(t,"select * from public.reward_definitions order by updated_at desc"),w(t,"select * from public.gift_catalog order by sort_order, name"),w(t,"select * from public.vip_plans order by sort_order, points_cost"),w(t,"select * from public.shop_items order by sort_order, name"),d(t,"select * from public.game_settings where singleton = true"),w(t,"select * from public.game_seasons order by starts_at desc"),w(t,"select * from public.game_symbol_versions order by created_at desc limit 200")]);a.json({settings:n,rewards:s,gifts:o,vipPlans:r,shopItems:c,gameSettings:u,seasons:l,gameSymbolVersions:m})})),i.patch("/economy/settings",p(async(e,a)=>{let n=await d(t,"select * from public.economy_settings where singleton = true");if(!n)throw new _(500,"Economy settings are missing","economy_settings_missing");let s=await d(t,`update public.economy_settings
        ...
        order by ledger.created_at desc limit 1000`,[n]);a.json({ledger:s})})),i.post("/points/adjustments",p(async(e,a)=>{let n=$(e.body?.userKey,"userKey"),s=k(e.body?.amount,0,-1e9,1e9);if(!s)throw new _(400,"amount must not be zero","validation_error");let o=f(e.body?.reason,"reason",1e3),r=f(e.body?.idempotencyKey,"idempotencyKey",160),c=await re(t,{userKey:n,amount:s,operationType:"adjustment",sourceType:"admin",sourceId:e.adminPrincipal.adminId,reason:o,administratorId:e.adminPrincipal.adminId,idempotencyKey:`admin-adjustment:${r}`});await T(t,e,{action:"points.adjust",targetType:"app_user",targetId:n,reason:o,after:c.ledger}),a.status(c.replayed?200:201).json(c)})),i.post("/rewards",p(async(e,a)=>{let n=e.body?.id?$(e.body.id,"id"):`reward-${Ve()}`,s=e.body?.conditionConfig===void 0?{}:pe(e.body.conditionConfig,"conditionConfig"),o=await d(t,`insert into public.reward_definitions(
       ...
       returning *`,[n,f(e.body?.name,"name",160),g(e.body?.iconUrl,2e3),g(e.body?.description,2e3),k(e.body?.points,0,0,1e9),k(e.body?.xp,0,0,1e9),K(e.body?.rarity||"common","rarity",ln),f(e.body?.conditionType||"manual","conditionType",100),JSON.stringify(s),e.body?.eventId?$(e.body.eventId,"eventId"):null,e.body?.clanId?$(e.body.clanId,"clanId"):null,E(e.body?.validFrom),E(e.body?.validUntil),A(e.body?.repeatable),k(e.body?.maxGrantsPerUser,1,1,1e6),A(e.body?.active,!0),e.adminPrincipal.adminId]);await T(t,e,{action:"reward.create",targetType:"reward_definition",targetId:n,after:o}),a.status(201).json({reward:o})})),i.patch("/rewards/:rewardId",p(async(e,a)=>{let n=$(e.params.rewardId,"rewardId"),s=await Me(t,"reward_definitions",n),o=e.body?.conditionConfig===void 0?s.condition_config:pe(e.body.conditionConfig,"conditionConfig"),r=await d(t,`update public.reward_definitions
       ...
       returning *`,[n,f(e.body?.name,"name",160),g(e.body?.description,2e3),g(e.body?.imageUrl,2e3),f(e.body?.category||"other","category",100),k(e.body?.pointsCost,0,0,1e9),e.body?.stock===null?null:k(e.body?.stock,0,0,1e9),E(e.body?.validFrom),E(e.body?.validUntil),K(e.body?.status||"draft","status",pn),e.body?.perUserLimit===null?null:k(e.body?.perUserLimit,1,1,1e6),A(e.body?.requiresRedemption),k(e.body?.sortOrder,0,-1e6,1e6),JSON.stringify(s),e.adminPrincipal.adminId]);await T(t,e,{action:"shop.item.create",targetType:"shop_item",targetId:n,after:o}),a.status(201).json({shopItem:o})})),i.patch("/shop/items/:itemId",p(async(e,a)=>{let n=$(e.params.itemId,"itemId"),s=await Me(t,"shop_items",n),o=e.body?.metadata===void 0?s.metadata:pe(e.body.metadata,"metadata"),r=await d(t,`update public.shop_items
        ...
        where id = $1 returning *`,[n,e.body?.name===void 0?s.name:f(e.body.name,"name",160),e.body?.description===void 0?s.description:g(e.body.description,2e3),e.body?.imageUrl===void 0?s.image_url:g(e.body.imageUrl,2e3),e.body?.category===void 0?s.category:f(e.body.category,"category",100),k(e.body?.pointsCost,Number(s.points_cost),0,1e9),e.body?.stock===void 0?s.stock:e.body.stock===null?null:k(e.body.stock,0,0,1e9),e.body?.validFrom===void 0?s.valid_from:E(e.body.validFrom),e.body?.validUntil===void 0?s.valid_until:E(e.body.validUntil),e.body?.status===void 0?s.status:K(e.body.status,"status",pn),e.body?.perUserLimit===void 0?s.per_user_limit:e.body.perUserLimit===null?null:k(e.body.perUserLimit,1,1,1e6),e.body?.requiresRedemption===void 0?s.requires_redemption:A(e.body.requiresRedemption),k(e.body?.sortOrder,Number(s.sort_order),-1e6,1e6),JSON.stringify(o)]);await T(t,e,{action:"shop.item.update",targetType:"shop_item",targetId:n,reason:g(e.body?.reason,1e3),before:s,after:r}),a.json({shopItem:r})})),i.patch("/game/settings",p(async(e,a)=>{let n=await d(t,"select * from public.game_settings where singleton = true");if(!n)throw new _(500,"Game settings are missing","game_settings_missing");let s=A(e.body?.resetSymbols),o=A(e.body?.resetPrizes),r=A(e.body?.resetGameRules),c=s?n.original_symbols:e.body?.symbols===void 0?n.symbols:be(e.body.symbols,"symbols"),u=await d(t,`update public.game_settings
       ...
       returning *`,[f(e.body?.name,"name",160),g(e.body?.description,1e3),n,s,K(e.body?.status||"scheduled","status",es),JSON.stringify(be(e.body?.rewards||[],"rewards")),JSON.stringify(e.body?.configuration===void 0?{}:pe(e.body.configuration,"configuration")),K(e.body?.progressMode||"account_keep_season_reset","progressMode",["account_keep_season_reset","carry_all","reset_all"]),e.adminPrincipal.adminId]);await T(t,e,{action:"game.season.create",targetType:"game_season",targetId:r.id,after:r}),a.status(201).json({season:r})})),i.post("/game/seasons/:seasonId/finalize",p(async(e,a)=>{let n=P(e.params.seasonId,"seasonId"),s=f(e.body?.reason,"reason",1e3),o=await pt(t,n,e.adminPrincipal.adminId);await T(t,e,{action:"game.season.finalize",targetType:"game_season",targetId:n,reason:s,after:o}),a.json(o)})),i.post("/game/sessions/:sessionId/exclude",p(async(e,a)=>{let n=P(e.params.sessionId,"sessionId"),s=f(e.body?.reason,"reason",1e3),o=await d(t,"select * from public.game_sessions where id = $1",[n]);if(!o)throw new _(404,"Game session was not found","not_found");let r=await d(t,`update public.game_sessions
          set status = 'excluded', suspicious = true,
              excluded_by_admin_id = $2, exclusion_reason = $3, updated_at = now()
        where id = $1 returning *`,[n,e.adminPrincipal.adminId,s]);await T(t,e,{action:"game.session.exclude",targetType:"game_session",targetId:n,reason:s,before:o,after:r}),a.json({session:r})})),i}var ln,_n,pn,es,yn=D(()=>{"use strict";ve();B();De();L();mt();z();Z();ln=["common","rare","epic","legendary"],_n=["virtual","physical"],pn=["draft","active","sold_out","archived"],es=["scheduled","active","completed","archived"]});import{randomUUID as ts}from"node:crypto";import{writeFile as ns}from"node:fs/promises";import as from"node:path";import ss from"express";import{Router as os}from"express";function us(t,i){return i==="image/png"?t.length>=24&&t.subarray(0,8).equals(Buffer.from([137,80,78,71,13,10,26,10])):i==="image/jpeg"?t.length>=4&&t[0]===255&&t[1]===216:i==="image/webp"?t.length>=12&&t.toString("ascii",0,4)==="RIFF"&&t.toString("ascii",8,12)==="WEBP":!1}function Ge(t,i){if(!t||Array.isArray(t)||typeof t!="object")throw new _(400,`${i} must be an object`,"validation_error");return t}function ce(t,i){return t==null||t===""?null:k(t,0,1,1e5)}function wn(t){let i=t===void 0?{}:Ge(t,"segment");return{userKeys:i.userKeys===void 0||i.userKeys===null?null:Array.isArray(i.userKeys)?[...new Set(i.userKeys.map(a=>$(a,"userKey")))].slice(0,1e4):(()=>{throw new _(400,"segment.userKeys must be an array","validation_error")})(),marketingOnly:A(i.marketingOnly),clanId:i.clanId?$(i.clanId,"clanId"):null,hasVip:i.hasVip===void 0||i.hasVip===null?null:A(i.hasVip)}}async function fn(t,i){return w(t,`select user_row.user_key, user_row.name, account.telegram_user_id,
      ...
      order by user_row.user_key`,[i.userKeys,i.marketingOnly,i.clanId,i.hasVip])}function gn(t,i){let e=os();return e.use(X),e.post("/content/uploads",ss.raw({type:["image/png","image/jpeg","image/webp"],limit:"12mb"}),p(async(a,n)=>{await O(t,a,"content.upload",M(a));let s=String(a.get("content-type")||"").split(";")[0].trim().toLowerCase(),o=cs.get(s),r=Buffer.isBuffer(a.body)?a.body:Buffer.alloc(0);if(!o||!us(r,s))throw new _(400,"Only valid PNG, JPG and WEBP images are accepted","invalid_image");let c=`${ts()}.${o}`;await ns(as.join(i,c),r,{flag:"wx",mode:416});let u=`/uploads/${c}`;await T(t,a,{action:"content.upload.create",targetType:"uploaded_asset",targetId:c,after:{url:u,mimeType:s,bytes:r.length}}),n.status(201).json({upload:{url:u,mimeType:s,bytes:r.length}})})),e.get("/content",p(async(a,n)=>{let[s,o,r]=await Promise.all([w(t,"select * from public.admin_assets order by asset_key"),w(t,"select * from public.ui_content_blocks order by scope, sort_order, block_key"),w(t,"select * from public.ui_navigation_items order by app_type, sort_order, item_key")]);n.json({assets:s,blocks:o,navigation:r})})),e.post("/content/assets",p(async(a,n)=>{let s=$(a.body?.assetKey,"assetKey"),o=f(a.body?.url,"url",4e3),r=await d(t,`insert into public.admin_assets(
       ...
       returning *`,[s,f(a.body?.name,"name",200),o,g(a.body?.defaultUrl,4e3)||o,K(a.body?.mediaType||"image","mediaType",bn),g(a.body?.mimeType,200),ce(a.body?.width,"width"),ce(a.body?.height,"height"),ce(a.body?.recommendedWidth,"recommendedWidth"),ce(a.body?.recommendedHeight,"recommendedHeight"),a.body?.maxBytes===void 0||a.body.maxBytes===null?null:k(a.body.maxBytes,0,1,1e8),g(a.body?.altText,500),a.adminPrincipal.adminId]);await T(t,a,{action:"content.asset.create",targetType:"admin_asset",targetId:s,after:r}),n.status(201).json({asset:r})})),e.patch("/content/assets/:assetKey",p(async(a,n)=>{let s=$(a.params.assetKey,"assetKey"),o=await d(t,"select * from public.admin_assets where asset_key = $1",[s]);if(!o)throw new _(404,"Asset was not found","not_found");let r=A(a.body?.reset),c=await d(t,`update public.admin_assets
        ...
        where asset_key = $1 returning *`,[s,r?o.default_name:a.body?.name===void 0?o.name:f(a.body.name,"name",200),r?o.default_url:a.body?.url===void 0?o.url:f(a.body.url,"url",4e3),a.body?.mediaType===void 0?o.media_type:K(a.body.mediaType,"mediaType",bn),a.body?.mimeType===void 0?o.mime_type:g(a.body.mimeType,200),a.body?.width===void 0?o.width:ce(a.body.width,"width"),a.body?.height===void 0?o.height:ce(a.body.height,"height"),a.body?.recommendedWidth===void 0?o.recommended_width:ce(a.body.recommendedWidth,"recommendedWidth"),a.body?.recommendedHeight===void 0?o.recommended_height:ce(a.body.recommendedHeight,"recommendedHeight"),a.body?.maxBytes===void 0?o.max_bytes:a.body.maxBytes===null?null:k(a.body.maxBytes,0,1,1e8),a.body?.altText===void 0?o.alt_text:g(a.body.altText,500),a.adminPrincipal.adminId]);await T(t,a,{action:r?"content.asset.reset":"content.asset.update",targetType:"admin_asset",targetId:s,reason:g(a.body?.reason,1e3),before:o,after:c}),n.json({asset:c})})),e.post("/content/blocks",p(async(a,n)=>{let s=K(a.body?.scope,"scope",is),o=$(a.body?.blockKey,"blockKey"),r=a.body?.configuration===void 0?{}:Ge(a.body.configuration,"configuration"),c=a.body?.defaultValue===void 0?{name:f(a.body?.name,"name",200),title:g(a.body?.title,500),subtitle:g(a.body?.subtitle,1e3),assetKey:a.body?.assetKey||null,configuration:r}:Ge(a.body.defaultValue,"defaultValue"),u=await d(t,`insert into public.ui_content_blocks(
       ...
       returning *`,[f(a.body?.name,"name",200),JSON.stringify(s),f(a.body?.messageText,"messageText",4e3),c.length,o,a.adminPrincipal.adminId]);await T(t,a,{action:"campaign.preview",targetType:"crm_campaign",targetId:u.id,after:{campaign:u,sample:c.slice(0,20)}}),n.status(201).json({campaign:u,sample:c.slice(0,20),replayed:!1})})),e.post("/campaigns/:campaignId/confirm",p(async(a,n)=>{let s=String(a.params.campaignId||"");if(!/^[0-9a-f-]{36}$/i.test(s))throw new _(400,"campaignId is invalid","validation_error");let o=f(a.body?.reason,"reason",1e3),r=await S(t,async c=>{let u=await d(c,"select * from public.crm_campaigns where id = $1 for update",[s]);if(!u)throw new _(404,"Campaign was not found","not_found");if(u.status==="sending"||u.status==="completed")return{campaign:u,queued:Number(u.recipient_count),replayed:!0};if(!["draft","previewed"].includes(u.status))throw new _(409,"Campaign cannot be confirmed in its current state","campaign_not_confirmable");let l=wn(u.segment),m=await fn(c,l);for(let b of m){let v=await d(c,`insert into public.crm_campaign_recipients(campaign_id, user_key, status)
         ...
         on conflict (idempotency_key) do nothing`,[s,JSON.stringify({campaignId:s}),`campaign-outbox:${s}`]),{campaign:await d(c,`update public.crm_campaigns
          ...
          where id = $1 returning *`,[s,m.length,a.adminPrincipal.adminId]),queued:m.length,replayed:!1}});await T(t,a,{action:"campaign.confirm",targetType:"crm_campaign",targetId:s,reason:o,after:r}),n.json(r)})),e}var is,bn,rs,cs,hn=D(()=>{"use strict";ve();B();L();z();ye();Z();is=["app","admin","shared","game"],bn=["image","video","audio","icon"],rs=["app","admin"],cs=new Map([["image/png","png"],["image/jpeg","jpg"],["image/webp","webp"]])});import{randomUUID as ds}from"node:crypto";import{Router as ls}from"express";function jn(t,i){let e=f(t,i,10);if(!/^\d{4}-\d{2}-\d{2}$/.test(e)||Number.isNaN(new Date(`${e}T00:00:00Z`).getTime()))throw new _(400,`${i} must use YYYY-MM-DD`,"validation_error");return e}function xn(t,i){let e=f(t,i,8);if(!/^(?:[01]\d|2[0-3]):[0-5]\d(?::[0-5]\d)?$/.test(e))throw new _(400,`${i} must use HH:MM`,"validation_error");return e}async function Pe(t,i){let e=await d(t,`select customer.*, user_row.name, user_row.username, user_row.avatar,
      ...
      where customer.user_key = $1`,[i]);if(!e)throw new _(404,"CRM customer was not found","not_found");return e}function Sn(t){let i=ls();return i.use(X),i.get("/dashboard",p(async(e,a)=>{let[n,s,o,r,c,u,l]=await Promise.all([d(t,"select count(*)::integer as value from public.app_users where account_status = 'active'"),d(t,`select count(*)::integer as value
        ...
        limit $4`,[n,s,o,r]);a.json({users:c})})),i.get("/crm/users/:userKey",p(async(e,a)=>{let n=$(e.params.userKey,"userKey"),s=await Pe(t,n),[o,r,c,u,l,m,y,b,v,h]=await Promise.all([w(t,`select tag.* from public.crm_tags tag
         ...
         where link.customer_id = $1 order by tag.name`,[s.id]),w(t,`select note.*, admin.email as admin_email
          ...
          where user_key = $1`,[n,r]),r!=="active"&&await l.query(`update public.user_sessions set revoked_at = coalesce(revoked_at, now())
            where app_user_key = $1`,[n]),Pe(l,n)));await T(t,e,{action:"crm.customer.update",targetType:"crm_customer",targetId:n,reason:c,before:s,after:u}),a.json({customer:u})})),i.post("/crm/users/:userKey/notes",p(async(e,a)=>{let n=$(e.params.userKey,"userKey"),s=await Pe(t,n),o=await d(t,`insert into public.crm_notes(customer_id, body, created_by_admin_id)
       ...
       values ($1,$2,$3) on conflict do nothing`,[o.id,s,e.adminPrincipal.adminId]),await T(t,e,{action:"crm.tag.assign",targetType:"crm_customer",targetId:n,after:{tagId:s}}),a.status(204).end()})),i.delete("/crm/users/:userKey/tags/:tagId",p(async(e,a)=>{let n=$(e.params.userKey,"userKey"),s=P(e.params.tagId,"tagId"),o=await Pe(t,n);await t.query("delete from public.crm_customer_tags where customer_id = $1 and tag_id = $2",[o.id,s]),await T(t,e,{action:"crm.tag.remove",targetType:"crm_customer",targetId:n,before:{tagId:s}}),a.status(204).end()})),i.get("/events",p(async(e,a)=>{let n=await w(t,`select event.*, runtime.status, runtime.starts_at, runtime.ends_at,
        ...
        order by coalesce(runtime.starts_at, event.event_date::timestamptz) desc`);a.json({events:n})})),i.post("/events",p(async(e,a)=>{let n=e.body?.id?$(e.body.id,"id"):`event-${ds()}`,s=jn(e.body?.eventDate,"eventDate"),o=xn(e.body?.eventTime||"23:00","eventTime"),r=K(e.body?.status||"draft","status",kn),c=E(e.body?.startsAt)||new Date(`${s}T${o}`).toISOString(),u=E(e.body?.endsAt),l=await S(t,async m=>{let y=await d(m,`insert into public.events(
         ...
         from public.events event
         left join public.event_runtime runtime on runtime.event_id = event.id
        where event.id = $1`,[n]);if(!s)throw new _(404,"Event was not found","not_found");let o=e.body?.status===void 0?s.status:K(e.body.status,"status",kn),r=await S(t,async c=>{let u=await d(c,`update public.events
            set title = $2, event_date = $3, event_time = $4,
                description = $5, image_url = $6, active = $7, sort_order = $8
          where id = $1 returning *`,[n,e.body?.title===void 0?s.title:f(e.body.title,"title",200),e.body?.eventDate===void 0?s.event_date:jn(e.body.eventDate,"eventDate"),e.body?.eventTime===void 0?s.event_time:xn(e.body.eventTime,"eventTime"),e.body?.description===void 0?s.description:g(e.body.description,6e3),e.body?.imageUrl===void 0?s.image_url:g(e.body.imageUrl,4e3),!["draft","archived","cancelled"].includes(o),k(e.body?.sortOrder,Number(s.sort_order||0),-1e6,1e6)]),l=await d(c,`update public.event_runtime
        ...
        where ($1 = '' or action = $1)
        order by created_at desc limit $2`,[s,n]);a.json({audit:o})})),i}var vn,$n,kn,In,_s,Pn=D(()=>{"use strict";ve();B();L();z();Z();vn=["trusted","normal","watch","restricted"],$n=["active","blocked","deleted"],kn=["draft","published","active","completed","archived","cancelled"],In=["open","reviewing","actioned","dismissed","closed"],_s=["low","normal","high","critical"]});import{Router as ps}from"express";function ms(t){return f(String(t.get("idempotency-key")||"").trim()||t.body?.idempotencyKey,"idempotencyKey",160)}function Tn(t){let i=ps();return i.use(X),i.get("/check-ins",p(async(e,a)=>{let n=String(e.query.eventId||"").trim(),s=await w(t,`select checkin.*, user_row.name, user_row.username,
        ...
        limit 500`,[n]);a.json({checkIns:s})})),i.post("/check-ins",p(async(e,a)=>{let n=f(e.body?.token,"token",1e3),s=ms(e),o=g(e.body?.reason,1e3),r;try{r=await S(t,async c=>{let u=await d(c,`select checkin.* from public.event_checkins checkin
         ...
         on conflict (idempotency_key) do nothing`,[l.user_key,b?`\u041D\u0430\u0447\u0438\u0441\u043B\u0435\u043D\u043E ${y.checkin_points} BALI Points.`:"\u0414\u043E\u0431\u0440\u043E \u043F\u043E\u0436\u0430\u043B\u043E\u0432\u0430\u0442\u044C \u0432 BALI.",JSON.stringify({eventId:l.event_id,bookingId:l.booking_id}),`checkin-notification:${l.event_id}:${l.user_key}`]),{checkIn:m,replayed:!1,points:b}})}catch(c){throw c?.code==="23505"&&await d(t,`select checkin.*
          ...
          where id = $1 returning *`,[c.id,e.adminPrincipal.adminId])});await T(t,e,{action:"shop.redeem",targetType:"shop_order",targetId:String(o.id),reason:s,after:o}),a.json({order:o})})),i.post("/redemptions/rewards/:rewardGrantId",p(async(e,a)=>{let n=P(e.params.rewardGrantId,"rewardGrantId"),s=g(e.body?.reason,1e3),o=await S(t,async r=>{let c=await d(r,"select * from public.user_rewards where id = $1 for update",[n]);if(!c)throw new _(404,"Reward was not found","not_found");if(c.status==="redeemed")return c;if(c.status!=="active")throw new _(409,"Reward cannot be redeemed","reward_redemption_unavailable");if(c.expires_at&&new Date(c.expires_at).getTime()<=Date.now())throw new _(409,"Reward has expired","reward_expired");return d(r,`update public.user_rewards
            set status = 'redeemed', redeemed_at = now()
          where id = $1 returning *`,[n])});await T(t,e,{action:"reward.redeem",targetType:"user_reward",targetId:n,reason:s,after:o}),a.json({reward:o})})),i}var Nn=D(()=>{"use strict";ve();B();De();L();z();me();Z()});import{randomBytes as ys,randomUUID as bs}from"node:crypto";import{Router as ws}from"express";function fs(){return`Bali-${ys(7).toString("base64url")}9!`}function gs(t){return`https://t.me/${String(t||"").replace(/^@+/,"")}`}async function hs(t,i,e){let a=`mobile:${bs()}`,n=String(i.display_name||"\u041F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044C BALI").trim(),s=n.split(/\s+/),o=s.shift()||n,r=s.join(" ");await t.query(`insert into public.app_users(
     ...
     values ($1,$2,$3,now(),1) on conflict (user_key) do nothing`,[a,o,r]),await t.query("insert into public.point_accounts(user_key) values ($1) on conflict (user_key) do nothing",[a]),await t.query("insert into public.game_profiles(user_key) values ($1) on conflict (user_key) do nothing",[a]),await t.query("insert into public.notification_preferences(user_key) values ($1) on conflict (user_key) do nothing",[a]);let c=await d(t,"select registration_points as amount from public.economy_settings where singleton = true"),u=Number(c?.amount||0);if(u>0){let l=`registration:${a}`,m=await d(t,"select balance from public.point_accounts where user_key = $1 for update",[a]),y=Number(m?.balance||0),b=y+u;await t.query(`update public.point_accounts
       ...
       on conflict (idempotency_key) do nothing`,[a,u,y,b,l])}return a}function An(t){let i=ws();return i.use(X),i.get("/mobile-access",p(async(e,a)=>{let n=e.query.status?K(e.query.status,"status",["pending","issued","completed","rejected","cancelled"]):"pending",s=await w(t,`select r.id, r.request_type, r.phone, r.telegram_username, r.display_name,
       ...
       from public.mobile_access_requests`);a.json({requests:s,counts:o||{pending:0,issued:0,completed:0}})})),i.post("/mobile-access/:requestId/issue",p(async(e,a)=>{let n=P(e.params.requestId,"requestId"),s=fs(),o=await Le(s),r=await S(t,async c=>{let u=await d(c,"select * from public.mobile_access_requests where id = $1 for update",[n]);if(!u)throw new _(404,"\u0417\u0430\u044F\u0432\u043A\u0430 \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D\u0430","mobile_access_request_not_found");if(u.status!=="pending")throw new _(409,"\u0417\u0430\u044F\u0432\u043A\u0430 \u0443\u0436\u0435 \u043E\u0431\u0440\u0430\u0431\u043E\u0442\u0430\u043D\u0430","mobile_access_already_processed");let l=u.app_user_key;if(u.request_type==="registration"){if(await d(c,"select app_user_key from public.mobile_credentials where phone = $1",[u.phone]))throw new _(409,"\u0414\u043B\u044F \u044D\u0442\u043E\u0433\u043E \u0442\u0435\u043B\u0435\u0444\u043E\u043D\u0430 \u0430\u043A\u043A\u0430\u0443\u043D\u0442 \u0443\u0436\u0435 \u0441\u043E\u0437\u0434\u0430\u043D","mobile_account_exists");l=await hs(c,u,o)}else{let m=await d(c,"select app_user_key from public.mobile_credentials where app_user_key = $1 or phone = $2 limit 1",[l,u.phone]);if(!m)throw new _(404,"\u041C\u043E\u0431\u0438\u043B\u044C\u043D\u044B\u0439 \u0430\u043A\u043A\u0430\u0443\u043D\u0442 \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D","mobile_account_not_found");l=m.app_user_key,await c.query(`update public.mobile_credentials
              set password_hash = $2,
                  telegram_username = $3,
      ...
      where status = 'active' and expires_at <= now()`)}async function ks(t,i,e){let a=await d(t,`select booking.*, event.title as event_title, event.event_date, event.event_time,
       ...
       join public.layout_tables layout_table on layout_table.id = booking.table_id
      where booking.id = $1 and booking.user_key = $2`,[i,e]);if(!a)throw new _(404,"Booking was not found","not_found");return a}async function Mn(t,i,e){let a=await d(t,`select membership.*, clan.name as clan_name, clan.clan_type
       from public.clan_memberships membership
       join public.clans clan on clan.id = membership.clan_id
      where membership.user_key = $1
        ...
        and clan.status = 'active'`,[i,e]);if(!a)throw new _(403,"Only the active clan leader can create a clan booking","clan_leader_required");return a}async function On(t,i,e,a){if(!i.active||i.status==="unavailable")throw new _(409,"Table is unavailable","table_unavailable");if(i.status==="clan_only"||i.table_type==="clan"){if(!a)throw new _(403,"This table can be booked only by a clan leader","clan_booking_required");await Mn(t,e,a)}else a&&await Mn(t,e,a);if((i.status==="vip_only"||i.table_type==="vip")&&!await d(t,`select subscription.id
         from public.user_vip_subscriptions subscription
        where subscription.user_key = $1
          and subscription.status = 'active'
          and subscription.starts_at <= now()
          and subscription.ends_at > now()
        limit 1`,[e]))throw new _(403,"An active VIP status is required for this table","vip_required")}function Cn(t){let i=vs();return i.use(H),i.get("/my",p(async(e,a)=>{let n=await w(t,`select booking.*, event.title as event_title, event.event_date, event.event_time,
        ...
        order by booking.created_at desc`,[e.userPrincipal.userKey]);a.json({bookings:n})})),i.post("/holds",p(async(e,a)=>{await O(t,e,"booking.hold",M(e));let n=$(e.body?.eventId,"eventId"),s=$(e.body?.tableId,"tableId"),o=e.body?.clanId?$(e.body.clanId,"clanId"):null;await Dn(t);let r=await S(t,async c=>{let u=await d(c,`select * from public.booking_holds
          where user_key = $1 and status = 'active'
          ...
          for update of layout_table`,[n,s]);if(!l)throw new _(404,"Table is not part of the event's published layout","table_not_found");if(await On(c,l,e.userPrincipal.userKey,o),await d(c,`select id, status
           from public.booking_records
        ...
        where idempotency_key = $1 and user_key = $2`,[s,e.userPrincipal.userKey]);if(y)return a.json({booking:y,replayed:!0});try{let b=await S(t,async v=>{await Dn(v);let h=await d(v,`select hold.*, layout_table.capacity, layout_table.minimum_deposit,
           ...
           on conflict (user_key) do update
             set phone = excluded.phone,
                 ...
                 last_activity_at = now(),
                 updated_at = now()
           returning *`,[e.userPrincipal.userKey,c,r]),N=`booking-${Rn()}`,I=`BALI-${Rn().replaceAll("-","").slice(0,10).toUpperCase()}`,x=h.auto_confirm?"confirmed":"pending",R=await d(v,`insert into public.booking_records(
         ...
         returning id, booking_id, expires_at, created_at, updated_at`,[n,e.userPrincipal.userKey,Y(s),y])}),r=await $s.toDataURL(s,{errorCorrectionLevel:"M",margin:2,width:640,color:{dark:"#080a08",light:"#ffffff"}});a.json({qr:o,token:s,qrDataUrl:r})})),i.post("/:bookingId/cancel",p(async(e,a)=>{let n=$(e.params.bookingId,"bookingId"),s=g(e.body?.reason,1e3),o=await S(t,async r=>{let c=await d(r,`select * from public.booking_records
          ...
          returning *`,[n,e.userPrincipal.userKey]);return await r.query(`insert into public.booking_status_history(
           booking_id, previous_status, next_status, actor_type, actor_id,
           reason, before_value, after_value
         ) values ($1,$2,'cancelled','user',$3,$4,$5::jsonb,$6::jsonb)`,[n,c.status,e.userPrincipal.userKey,s,JSON.stringify(c),JSON.stringify(u)]),u});a.json({booking:o})})),i}var En=D(()=>{"use strict";B();L();z();ye();me();Z()});import{Router as Is}from"express";function Un(t){let i=Is();return i.use(H),i.get("/",p(async(e,a)=>{let[n,s,o]=await Promise.all([w(t,`select * from public.menu_catalog_items
       ...
       values ($1,$2,$3)
       returning *`,[e.userPrincipal.userKey,n,s]);a.status(201).json({review:o})})),i}var Bn=D(()=>{"use strict";B();L();z();Z()});import{Router as js}from"express";import xs from"qrcode";function yt(t){return f(String(t.get("idempotency-key")||"").trim()||t.body?.idempotencyKey,"idempotencyKey",160)}async function Ss(t,i,e,a,n,s,o){await t.query(`insert into public.notifications(
       user_key, notification_type, title, body, data, idempotency_key
     ) values ($1,$2,$3,$4,$5::jsonb,$6)
     on conflict (idempotency_key) do nothing`,[i,e,a,n,JSON.stringify(s),o])}function bt(t){return xs.toDataURL(t,{errorCorrectionLevel:"M",margin:2,width:640,color:{dark:"#080a08",light:"#ffffff"}})}function Ln(t){let i=js();return i.use(H),i.get("/points",p(async(e,a)=>{let[n,s]=await Promise.all([d(t,"select * from public.point_accounts where user_key = $1",[e.userPrincipal.userKey]),w(t,`select * from public.point_ledger
        ...
        where idempotency_key = $1 and sender_user_key = $2`,[n,c]);if(u)return a.json({gift:u,replayed:!0});let l=ie();try{let m=await S(t,async y=>{let[b,v,h]=await Promise.all([d(y,"select * from public.gift_catalog where id = $1 and active = true for update",[s]),d(y,`select user_row.user_key, user_row.name,
           ...
           returning *`,[s,c,o,j,N?.ledger?.id||null,r,b.gift_type==="physical"?Y(l):null,b.validity_days,n])});await Ss(t,o,"gift_received","\u041D\u043E\u0432\u044B\u0439 \u043F\u043E\u0434\u0430\u0440\u043E\u043A",`${e.userPrincipal.name} \u043E\u0442\u043F\u0440\u0430\u0432\u0438\u043B \u0432\u0430\u043C \u043F\u043E\u0434\u0430\u0440\u043E\u043A.`,{giftId:m.id},`gift-received:${m.id}`),a.status(201).json({gift:m,replayed:!1})}catch(m){if(m?.code==="23505"){let y=await d(t,"select * from public.gifts where idempotency_key = $1 and sender_user_key = $2",[n,c]);if(y)return a.json({gift:y,replayed:!0})}throw m}})),i.post("/gifts/:giftId/qr",p(async(e,a)=>{let n=P(e.params.giftId,"giftId"),s=ie(),o=await d(t,`update public.gifts gift
        ...
        returning gift.id, gift.status, gift.expires_at, catalog.name`,[n,e.userPrincipal.userKey,Y(s)]);if(!o)throw new _(409,"This gift cannot be redeemed","gift_redemption_unavailable");a.json({gift:o,token:s,qrDataUrl:await bt(s)})})),i.get("/vip",p(async(e,a)=>{let[n,s]=await Promise.all([w(t,"select * from public.vip_plans where active = true order by sort_order, points_cost"),w(t,`select subscription.*, plan.name, plan.benefits, plan.badge_url,
           ...
           join public.vip_plans plan on plan.id = subscription.plan_id
          where subscription.user_key = $1
          order by subscription.ends_at desc`,[e.userPrincipal.userKey])]);a.json({plans:n,subscriptions:s})})),i.post("/vip/purchase",p(async(e,a)=>{let n=yt(e),s=$(e.body?.planId,"planId"),o=e.userPrincipal.userKey,r=await d(t,`select * from public.user_vip_subscriptions
        where idempotency_key = $1 and user_key = $2`,[n,o]);if(r)return a.json({subscription:r,replayed:!0});let c;try{c=await S(t,async u=>{let l=await d(u,"select * from public.vip_plans where id = $1 and active = true for update",[s]);if(!l)throw new _(404,"VIP plan was not found","not_found");let m=await d(u,`select * from public.user_vip_subscriptions
         ...
         returning *`,[s,b,v?.ledger?.id||null,Y(u),n]);for(let j of y)await m.query(`insert into public.shop_order_items(
            ...
            where idempotency_key = $1 and user_key = $2`,[n,s]);if(y)return a.json({order:y,replayed:!0})}throw m}a.status(201).json({order:l.order,qrToken:l.requiresRedemption?u:void 0,qrDataUrl:l.requiresRedemption?await bt(u):void 0,replayed:!1})})),i.post("/shop/orders/:orderId/qr",p(async(e,a)=>{let n=P(e.params.orderId,"orderId"),s=ie(),o=await d(t,`update public.shop_orders shop_order
          set qr_token_hash = $3, updated_at = now()
     ...
     on conflict (idempotency_key) do nothing`,[i.userKey,i.type,i.title,i.body,JSON.stringify(i.data||{}),i.idempotencyKey])}function zn(t){let i=Ps();return i.use(H),i.get("/",p(async(e,a)=>{let n=await w(t,`select e.id, e.title, e.event_date, e.event_time, e.description, e.image_url,
        ...
        order by case when invitation.status = 'pending' then 0 else 1 end,
                 invitation.created_at desc`,[e.userPrincipal.userKey]);a.json({invitations:n})})),i.patch("/invitations/:invitationId",p(async(e,a)=>{let n=P(e.params.invitationId,"invitationId"),s=K(e.body?.status,"status",Ns),o=await S(t,async r=>{let c=await d(r,`select invitation.*, event.title as event_title
           from public.event_invitations invitation
           join public.events event on event.id = invitation.event_id
        ...
        where layout_table.layout_id = $3 and layout_table.active = true`,[n,e.userPrincipal.userKey,s.layout_id]),c=new Map(r.map(u=>[u.id,u]));a.json({eventId:n,assignment:s,layout:o.layout,elements:o.elements,tables:o.tables.map(u=>({...u,...c.get(u.id)||{availability_status:"available"}}))})})),i.get("/:eventId",p(async(e,a)=>{let n=$(e.params.eventId,"eventId"),s=await Oe(t,n),[o,r,c]=await Promise.all([w(t,`select attendance.status, attendance.updated_at,
          ...
          where checkin.event_id = $1
            and coalesce(profile.discoverable, true) = true
          order by checkin.checked_in_at desc`,[n])]);a.json({event:s,participants:o,checkedIn:c,clans:r})})),i.put("/:eventId/attendance",p(async(e,a)=>{let n=$(e.params.eventId,"eventId"),s=K(e.body?.status,"status",Ts),o=await Oe(t,n);if(!Qn(o)&&s!=="cancelled")throw new _(409,"Attendance can be changed only for an active future event","event_not_active");let r=await d(t,`insert into public.event_attendance(event_id, user_key, status, source_type)
       values ($1,$2,$3,'self')
       on conflict (event_id, user_key) do update
         set status = excluded.status,
             source_type = 'self',
       ...
       returning *`,[n,e.userPrincipal.userKey,s]);a.json({attendance:r})})),i.post("/:eventId/invitations",p(async(e,a)=>{await O(t,e,"event_invitation.create",M(e));let n=$(e.params.eventId,"eventId"),s=$(e.body?.recipientUserKey,"recipientUserKey"),o=g(e.body?.message,500);if(s===e.userPrincipal.userKey)throw new _(400,"A user cannot invite themselves","validation_error");let r=await Oe(t,n);if(!Qn(r))throw new _(409,"Only active future events can be invited to","event_not_active");let c=await d(t,`select user_row.user_key, user_row.name,
        ...
        where user_row.user_key = $2 and user_row.account_status = 'active'`,[e.userPrincipal.userKey,s]);if(!c)throw new _(404,"Recipient was not found","not_found");if(c.blocked)throw new _(403,"Invitation is unavailable because one of the users blocked the other","user_blocked");if(!c.allow_event_invites)throw new _(403,"Recipient disabled event invitations","event_invitations_disabled");try{let u=await S(t,async l=>{let m=await d(l,`insert into public.event_invitations(
           ...
           returning *`,[n,e.userPrincipal.userKey,s,o]);return await Jn(l,{userKey:s,type:"event_invitation",title:"\u041F\u0440\u0438\u0433\u043B\u0430\u0448\u0435\u043D\u0438\u0435 \u043D\u0430 \u043C\u0435\u0440\u043E\u043F\u0440\u0438\u044F\u0442\u0438\u0435",body:`${e.userPrincipal.name} \u043F\u0440\u0438\u0433\u043B\u0430\u0448\u0430\u0435\u0442 \u0432\u0430\u0441 \u043D\u0430 \xAB${r.title}\xBB.`,data:{invitationId:m.id,eventId:n},idempotencyKey:`event-invitation:${m.id}`}),m});a.status(201).json({invitation:u})}catch(u){throw u?.code==="23505"?new _(409,"An unanswered invitation already exists","invitation_already_pending"):u}})),i.post("/:eventId/archive-invitations",p(async(e,a)=>{let n=$(e.params.eventId,"eventId");await Oe(t,n);let s=await t.query(`update public.event_invitations
          set status = 'archived', archived_at = now(), updated_at = now()
        where event_id = $1
          and (sender_user_key = $2 or recipient_user_key = $2)
          and status <> 'archived'`,[n,e.userPrincipal.userKey]);a.json({archived:s.rowCount||0})})),i}var Ts,Ns,Vn=D(()=>{"use strict";B();L();z();ye();Z();Je();Ts=["going","maybe","not_going","cancelled"],Ns=["going","maybe","declined"]});import{createHash as As}from"node:crypto";function Ze(t){return t&&!Array.isArray(t)&&typeof t=="object"?t:{}}function ue(t,i,e,a){let n=Number(t);return Number.isFinite(n)?Math.max(e,Math.min(a,Math.round(n))):i}function Ce(t,i,e,a){let n=Number(t);return Number.isFinite(n)?Math.max(e,Math.min(a,n)):i}function wt(t){return Array.isArray(t)?t.map(wt):t&&typeof t=="object"?Object.fromEntries(Object.entries(t).filter(([,i])=>i!==void 0).sort(([i],[e])=>i.localeCompare(e)).map(([i,e])=>[i,wt(e)])):t}function Ie(t){return As("sha256").update(JSON.stringify(wt(t))).digest("hex")}function Ds(t){return Number.parseInt(Ie(t).slice(0,8),16)>>>0}function Gn(t){let i=Ds(t)||1;return()=>{i+=1831565813;let e=i;return e=Math.imul(e^e>>>15,e|1),e^=e+Math.imul(e^e>>>7,e|61),((e^e>>>14)>>>0)/4294967296}}function Ms(t,i={}){let e={...Ye,...Ze(i)},a=Math.max(1,Math.floor(t));return 1+Ce(e.sqrtDifficulty,Ye.sqrtDifficulty,0,10)*Math.sqrt(a-1)+Ce(e.linearDifficulty,Ye.linearDifficulty,0,10)*(a-1)}function Os(t,i,e,a){let n=Math.min(ue(a.maxGoals,3,1,5),1+Math.floor(t/12)),s=t%7,o=[{type:"score",target:e}];return n>1&&(s===0||s===3?o.push({type:"collect",tileIndex:t%5,target:Math.round(12+i*5)}):t>=ue(a.obstacleStartLevel,8,1,1e6)?o.push({type:"obstacles",target:Math.round(4+i*2)}):o.push({type:"createSpecial",special:"line",target:Math.max(1,Math.round(i))})),n>2&&o.push(t%2?{type:"activateSpecial",special:"any",target:Math.max(1,Math.round(i))}:{type:"collect",tileIndex:(t+2)%5,target:Math.round(10+i*4)}),o}function Fn(t,i,e){let a={...Ye,...Ze(i.level_rules??i.levelRules)},n={...Ks,...Ze(i.scoring_rules??i.scoringRules)},s={...Rs,...Ze(i.rating_rules??i.ratingRules)},o=Math.max(1,Math.floor(t)),r=Ms(o,a),c=ue(a.rows,6,5,10),u=ue(a.columns,6,5,10),l=ue(a.minTileTypes,5,5,8),m=ue(a.maxTileTypes,8,l,8),y=Math.min(m,l+Math.floor((o-1)/18)),b=Math.max(500,Math.round(Ce(a.baseTargetScore,1e4,500,1e9)*r/100)*100),v=Math.max(ue(a.minMoves,12,5,99),ue(a.baseMoves,25,5,99)-Math.floor(Math.log2(o+1))),h=Math.max(0,o-ue(a.obstacleStartLevel,8,1,1e6)),j=ue(a.checkpointEvery,10,1,1e4),N=ue(a.milestoneEvery,25,1,1e4);return{level:o,seed:`${e}:${o}`,difficulty:Number(r.toFixed(5)),rows:c,columns:u,tileTypes:y,moves:v,targetScore:b,goals:Os(o,r,b,a),checkpoint:o%j===0,multistage:o%N===0,obstacleChance:o<ue(a.obstacleStartLevel,8,1,1e6)?0:Math.min(Ce(a.obstacleChanceMax,.28,0,.8),.04+h*.006),blockedChance:o<N?0:Math.min(Ce(a.blockedChanceMax,.12,0,.5),.02+o*5e-4),scoring:n,rating:s}}function Fe(t){return t&&!t.blocked?t.tile:""}function ft(t,i,e){let a=[];for(let n=0;n<i;n+=1){let s=0;for(let o=1;o<=e;o+=1){let r=Fe(t[n*e+s]),c=o<e?Fe(t[n*e+o]):"";(!r||c!==r)&&(r&&o-s>=3&&a.push(Array.from({length:o-s},(u,l)=>n*e+s+l)),s=o)}}for(let n=0;n<e;n+=1){let s=0;for(let o=1;o<=i;o+=1){let r=Fe(t[s*e+n]),c=o<i?Fe(t[o*e+n]):"";(!r||c!==r)&&(r&&o-s>=3&&a.push(Array.from({length:o-s},(u,l)=>(s+l)*e+n)),s=o)}}return a}function Wn(t,i,e){let a=Math.floor(t/e),n=Math.floor(i/e);return Math.abs(a-n)+Math.abs(t%e-i%e)===1}function Xe(t,i,e){[t[i],t[e]]=[t[e],t[i]]}function Yn(t,i,e){for(let a=0;a<t.length;a+=1)for(let n of[a+1,a+e]){if(n>=t.length||!Wn(a,n,e)||t[a]?.blocked||t[n]?.blocked)continue;Xe(t,a,n);let s=ft(t,i,e).length>0;if(Xe(t,a,n),s)return[a,n]}return null}function Zn(t,i,e){return!!Yn(t,i,e)}function qe(t,i){let e=i.slice(0,t.tileTypes);if(e.length<5)throw new Error("At least five active Match-3 symbols are required");for(let a=0;a<80;a+=1){let n=Gn(`${t.seed}:board:${a}`),s=[];for(let o=0;o<t.rows*t.columns;o+=1){let r=Math.floor(o/t.columns),c=o%t.columns,u=new Set;c>=2&&s[o-1]?.tile===s[o-2]?.tile&&u.add(s[o-1].tile),r>=2&&s[o-t.columns]?.tile===s[o-t.columns*2]?.tile&&u.add(s[o-t.columns].tile);let l=e.filter(b=>!u.has(b)),m=t.blockedChance>0&&n()<t.blockedChance,y=!m&&t.obstacleChance>0&&n()<t.obstacleChance?n()<.2?2:1:0;s.push({tile:l[Math.floor(n()*l.length)]||e[0],special:"",obstacle:y,blocked:m})}if(Zn(s,t.rows,t.columns))return s}throw new Error("Unable to generate a playable Match-3 board")}function Xn(t,i,e,a,n,s){let o=Gn(`${e.seed}:move:${n}:cascade:${s}:${Ie(t)}`),r=t.map(c=>({...c}));for(let c=0;c<e.columns;c+=1){let u=[];for(let m=e.rows-1;m>=0;m-=1){let y=m*e.columns+c;r[y]?.blocked||i.has(y)||u.push(r[y])}let l=0;for(let m=e.rows-1;m>=0;m-=1){let y=m*e.columns+c;r[y]?.blocked||(r[y]=u[l++]||{tile:a[Math.floor(o()*Math.min(a.length,e.tileTypes))],special:"",obstacle:0,blocked:!1})}}return r}function We(t,i,e,a,n){let s=t[i];if(!s?.special)return[];let o=Math.floor(i/n),r=i%n;if(s.special==="line-h")return Array.from({length:n},(u,l)=>o*n+l);if(s.special==="line-v")return Array.from({length:a},(u,l)=>l*n+r);if(s.special==="bomb"){let u=[];for(let l=-1;l<=1;l+=1)for(let m=-1;m<=1;m+=1){let y=o+l,b=r+m;y>=0&&y<a&&b>=0&&b<n&&u.push(y*n+b)}return u}let c=t[e]?.tile;return t.flatMap((u,l)=>!u.blocked&&(!c||u.tile===c)?[l]:[])}function qn(t,i,e,a,n,s){let o=t.map(I=>({...I}));if(!Number.isInteger(i)||!Number.isInteger(e)||!Wn(i,e,a.columns))return{valid:!1,reason:"not_adjacent",board:o,scoreDelta:0,progressDelta:{},breakdown:{},cascades:0};if(!o[i]||!o[e]||o[i].blocked||o[e].blocked)return{valid:!1,reason:"blocked",board:o,scoreDelta:0,progressDelta:{},breakdown:{},cascades:0};Xe(o,i,e);let r=o[i]?.special,c=o[e]?.special,u=new Set;if(r&&c?r==="rainbow"||c==="rainbow"?o.forEach((I,x)=>{I.blocked||u.add(x)}):(We(o,i,e,a.rows,a.columns).forEach(I=>u.add(I)),We(o,e,i,a.rows,a.columns).forEach(I=>u.add(I))):(r||c)&&We(o,r?i:e,r?e:i,a.rows,a.columns).forEach(R=>u.add(R)),!u.size&&!ft(o,a.rows,a.columns).length)return Xe(o,i,e),{valid:!1,reason:"no_match",board:o,scoreDelta:0,progressDelta:{},breakdown:{},cascades:0};let l=o,m=0,y=0,b={},v={line:0,bomb:0,rainbow:0},h={line:0,bomb:0,rainbow:0,any:0},j={combinations:0,cascades:0,specials:0,obstacles:0},N=0;for(let I=1;I<=12;I+=1){let x=ft(l,a.rows,a.columns);if(!x.length&&!u.size)break;N=I;let R=new Set(u.size?u:x.flat()),J=[...R],U=new Set;for(;J.length;){let V=J.shift(),q=l[V]?.special;if(!q||U.has(V))continue;U.add(V);let at=V===i?e:i;for(let st of We(l,V,at,a.rows,a.columns))R.has(st)||J.push(st),R.add(st);let Ae=q.startsWith("line")?"line":q;h[Ae]=Number(h[Ae]||0)+1,h.any+=1;let jt=Number(a.scoring[`${Ae}Activate`]||0);j.specials+=jt,m+=jt}let C="",W=x.flatMap((V,q)=>x.slice(q+1).flatMap(at=>V.filter(Ae=>at.includes(Ae)))),oe=x.find(V=>V.includes(e))||x[0]||[...R];!u.size&&W.length?C="bomb":!u.size&&oe.length>=5?C="rainbow":!u.size&&oe.length===4&&(C=oe.every(q=>Math.floor(q/a.columns)===Math.floor(oe[0]/a.columns))?"line-h":"line-v");let se=W[0]??(oe.includes(e)?e:oe[0]);if(C){R.delete(se);let V=C==="rainbow"?"rainbow":C==="bomb"?"bomb":"line";v[V]+=1;let q=Number(a.scoring[`${V}Create`]||0);j.specials+=q,m+=q}let de=0,we=0;for(let V of R){let q=l[V];!q||q.blocked||(b[q.tile]=Number(b[q.tile]||0)+1,q.obstacle>0&&(y+=1,we+=a.scoring.obstacleLayer),de+=1)}let Be=Math.min(a.scoring.maxCascade,1+a.scoring.cascadeStep*(I-1)),fe=x.length?Math.max(...x.map(V=>V.length>=6?a.scoring.combo6:V.length===5?a.scoring.combo5:V.length===4?a.scoring.combo4:a.scoring.combo3)):1,je=Math.round(de*a.scoring.baseTile*fe*Be);j.obstacles+=we,m+=je+we,I===1?j.combinations+=je:j.cascades+=je,l=Xn(l,R,a,n,s,I),C&&l[se]&&!l[se].blocked&&(l[se].special=C),u=new Set}return Zn(l,a.rows,a.columns)||(l=qe({...a,seed:`${a.seed}:reshuffle:${s}`},n)),{valid:!0,board:l,scoreDelta:m,progressDelta:{collected:b,obstaclesDestroyed:y,specialsCreated:v,specialsActivated:h},breakdown:j,cascades:N}}function ea(t,i,e,a,n,s){let o=t.map(m=>({...m}));if(i==="hint"){let m=Yn(o,a.rows,a.columns);return{valid:!!m,reason:m?void 0:"no_hint",board:o,scoreDelta:0,progressDelta:{},hint:m||[]}}if(i==="shuffle")return{valid:!0,board:qe({...a,seed:`${a.seed}:booster:shuffle:${s}`},n),scoreDelta:0,progressDelta:{}};if(!Number.isInteger(e)||e===null||e<0||e>=o.length)return{valid:!1,reason:"invalid_target",board:o,scoreDelta:0,progressDelta:{}};if(o[e]?.blocked)return{valid:!1,reason:"blocked",board:o,scoreDelta:0,progressDelta:{}};let r=new Set;if(i==="remove"&&r.add(e),i==="removeType"){let m=o[e]?.tile;o.forEach((y,b)=>{!y.blocked&&y.tile===m&&r.add(b)})}if(i==="bomb"){let m=Math.floor(e/a.columns),y=e%a.columns;for(let b=-1;b<=1;b+=1)for(let v=-1;v<=1;v+=1){let h=m+b,j=y+v;if(h>=0&&h<a.rows&&j>=0&&j<a.columns){let N=h*a.columns+j;o[N]?.blocked||r.add(N)}}}let c={},u=0;for(let m of r){let y=o[m];y&&(c[y.tile]=Number(c[y.tile]||0)+1,y.obstacle>0&&(u+=1))}let l=Math.round(r.size*Number(a.scoring.baseTile||100));return{valid:!0,board:Xn(o,r,a,n,1e5+s,1),scoreDelta:l,progressDelta:{collected:c,obstaclesDestroyed:u},cleared:r.size}}function Ee(){return{score:0,collected:{},obstaclesDestroyed:0,specialsCreated:{line:0,bomb:0,rainbow:0},specialsActivated:{line:0,bomb:0,rainbow:0,any:0}}}function Cs(t,i,e){return t.type==="score"?Number(i.score||0):t.type==="collect"?Number(i.collected[e[t.tileIndex||0]]||0):t.type==="obstacles"?Number(i.obstaclesDestroyed||0):t.type==="createSpecial"?Number(i.specialsCreated[t.special||"line"]||0):Number(i.specialsActivated[t.special||"any"]||0)}function ta(t,i,e){return t.goals.every(a=>Cs(a,i,e)>=a.target)}function na(t,i,e,a){return e?t>=i*a.star3?3:t>=i*a.star2?2:1:0}function aa(t,i,e,a){return i<1?0:Math.round(a.base*(1+a.levelLog*Math.log(Math.max(1,t)))*a[`star${i}`]*a[`continue${Math.min(2,e)}`])}var Ye,Ks,Rs,sa=D(()=>{"use strict";Ye={rows:6,columns:6,minTileTypes:5,maxTileTypes:8,baseMoves:25,minMoves:12,baseTargetScore:1e4,sqrtDifficulty:.06,linearDifficulty:.004,maxGoals:3,checkpointEvery:10,milestoneEvery:25,specialStartLevel:4,obstacleStartLevel:8,blockedChanceMax:.12,obstacleChanceMax:.28},Ks={baseTile:100,combo3:1,combo4:1.25,combo5:1.6,combo6:2,comboTL:1.75,cascadeStep:.35,maxCascade:3,lineCreate:250,bombCreate:400,rainbowCreate:650,lineActivate:350,bombActivate:550,rainbowActivate:900,obstacleLayer:150,goalComplete:1e3,allGoalsBase:2500,remainingMove:200,cleanMultiplier:.1,star2:1.2,star3:1.5},Rs={base:1e3,levelLog:.1,star1:1,star2:1.15,star3:1.35,continue0:1,continue1:.85,continue2:.65}});import{Router as Es}from"express";function Te(t){return f(String(t.get("idempotency-key")||"").trim()||t.body?.idempotencyKey,"idempotencyKey",160)}function Us(t){return(Array.isArray(t?.symbols)?t.symbols:[]).filter(e=>e?.active!==!1&&e?.key).map(e=>({key:String(e.key),label:String(e.label||e.key),imageUrl:String(e.imageUrl||e.defaultImageUrl||"")}))}function Ue(t,i){Object.entries(i||{}).forEach(([e,a])=>{t[e]=Number(t[e]||0)+Number(a||0)})}async function et(t){await dn(t);let i=await d(t,`select * from public.game_seasons
      where status in ('scheduled','active')
     ...
     on conflict do nothing`,[`BALI Match-3 \xB7 ${o.toISOString().slice(0,10)}`,o.toISOString(),r.toISOString(),JSON.stringify(Array.isArray(e.default_prizes)?e.default_prizes:[])]),d(t,`select * from public.game_seasons
      ...
      order by starts_at desc limit 1`)}async function Bs(t,i,e){let a=new Date,n=new Date(a);n.setUTCHours(0,0,0,0);let s=n.getUTCDay();n.setUTCDate(n.getUTCDate()-(s+6)%7);let o=new Date(Math.max(new Date(i.starts_at).getTime(),n.getTime())),r=new Date(Math.min(new Date(i.ends_at).getTime(),o.getTime()+7*864e5)),c=await d(t,"select clan_rules from public.game_settings where singleton = true"),u=await d(t,`insert into public.game_clan_rounds(
     ...
     on conflict (round_id, clan_id, metric) do nothing`,[u.id,Math.max(1,Number(c?.clan_rules?.minimumLevelsForChest||3))]),u}function oa(t){let i=Es();return i.use(H),i.get("/",p(async(e,a)=>{let[n,s,o]=await Promise.all([d(t,"select * from public.game_settings where singleton = true"),d(t,"select * from public.game_profiles where user_key = $1",[e.userPrincipal.userKey]),et(t)]),r=s;if(n&&r){let c=Math.max(1,Number(n.lives_rules?.maximum||n.base_lives||5)),u=Math.max(1,Number(n.lives_rules?.restoreMinutes||30)),l=r.last_life_at?Date.now()-new Date(r.last_life_at).getTime():0,m=Math.floor(l/(u*6e4));m>0&&Number(r.lives||0)<c&&(r=await d(t,`update public.game_profiles
              set lives = least($2, lives + $3),
                  ...
                  updated_at = now()
            where user_key = $1 returning *`,[e.userPrincipal.userKey,c,m,u]))}a.json({settings:n,profile:r,season:o})})),i.get("/leaderboard",p(async(e,a)=>{let n=e.query.seasonId?P(e.query.seasonId,"seasonId"):null,s=n?await d(t,"select * from public.game_seasons where id = $1",[n]):await et(t),o=await w(t,`select ranked.position, ranked.user_key, ranked.score, ranked.level,
        ...
        limit 100`,[s?.id||null]),r=o.find(c=>c.user_key===e.userPrincipal.userKey)||null;a.json({season:s,leaderboard:o,me:r})})),i.post("/sessions",p(async(e,a)=>{await O(t,e,"game.session",M(e));let n=Te(e),s=e.userPrincipal.userKey,o=await d(t,"select * from public.game_sessions where idempotency_key = $1 and user_key = $2",[n,s]);if(o)return a.json({session:o,replayed:!0});let r;try{r=await S(t,async c=>{let u=await d(c,`select * from public.game_sessions
         ...
         returning *`,[n,e.userPrincipal.userKey,c,o,r,N,J,I.scoreDelta,JSON.stringify(I),s]),session:U,replayed:!1}});a.status(u.replayed?200:201).json(u)})),i.post("/sessions/:sessionId/boosters",p(async(e,a)=>{await O(t,e,"game.booster",M(e));let n=P(e.params.sessionId,"sessionId"),s=Te(e),o=String(e.body?.type||"");if(!["shuffle","hint","bomb","remove","removeType"].includes(o))throw new _(400,"Unsupported game booster","validation_error");let r=e.body?.index===void 0||e.body?.index===null?null:k(e.body.index,0,0,99),c=await S(t,async u=>{let l=await d(u,`select * from public.game_booster_uses
          ...
          where game_session_id = $1`,[n]),x=ea(N,o,r,v,j,Number(I?.count||0)+1);if(!x.valid)throw new _(409,"Game booster cannot be applied","game_booster_invalid",{reason:x.reason});let R={...y.booster_inventory||{}},J=Number(R[o]||0)>0,U=0;if(J)R[o]=Number(R[o]||0)-1,await u.query(`update public.game_profiles
         ...
         returning *`,[n,e.userPrincipal.userKey,o,r,J,U,JSON.stringify(x),s]),session:oe,result:x,replayed:!1}});a.status(c.replayed?200:201).json(c)})),i.post("/sessions/:sessionId/finish",p(async(e,a)=>{let n=P(e.params.sessionId,"sessionId"),s=Te(e),o=await S(t,async r=>{let c=await d(r,`select * from public.idempotency_records
          ...
          where scope = 'game.finish' and idempotency_key = $1`,[s]);if(l?.response_body)return{session:l.response_body,replayed:!0};if(u.status!=="active")throw new _(409,"Game session has already ended","game_already_ended");let m=await d(r,"select * from public.game_settings where singleton = true");if(!m)throw new _(500,"Game settings are missing","game_settings_missing");let y=Math.max(1,Math.floor((Date.now()-new Date(u.started_at).getTime())/1e3)),b=u.level_config||{},v=Array.isArray(b.symbols)?b.symbols:[],h=v.map(V=>String(V.key)),j=Ie({generated:{...b,symbols:void 0},symbols:v})===String(u.config_signature),N=[];j||N.push("config_signature_mismatch"),Number(u.move_sequence||0)>Number(b.moves||0)+Number(u.continues_used||0)*Number(m.economy_rules?.continueMoves||5)&&N.push("move_limit_exceeded");let I=N.length>0,x={...Ee(),...u.goal_progress||{}},R=!I&&ta(b,x,h),J=b.scoring||{},U={...u.score_breakdown||{}},C=Number(u.level_score||0);R&&(U.goals=b.goals.length*Number(J.goalComplete||1e3)+Math.round(Number(J.allGoalsBase||2500)*Number(b.difficulty||1)),U.remainingMoves=Math.round(Number(u.moves_remaining||0)*Number(J.remainingMove||200)*Number(b.difficulty||1)),C+=Number(U.goals||0)+Number(U.remainingMoves||0),Number(u.continues_used||0)===0&&(U.clean=Math.round(C*Number(J.cleanMultiplier||.1)),C+=Number(U.clean||0)));let W=na(C,Number(b.targetScore||0),R,J),oe=R?aa(Number(u.level_number),W,Number(u.continues_used||0),b.rating||{}):0,se=R?await d(r,`select * from public.game_level_results
            where season_id = $1 and user_key = $2 and level_number = $3
            for update`,[u.season_id,e.userPrincipal.userKey,u.level_number]):null,de=Math.max(0,oe-Number(se?.best_rating||0)),we=m.economy_rules||{},Be=Array.isArray(we.starRewards)?we.starRewards:[0,5,10,20],fe=0;R&&(se||(fe+=Number(we.firstCompletion||20)),fe+=Math.max(0,Number(Be[W]||0)-Number(Be[se?.best_stars||0]||0)),Number(u.continues_used||0)===0&&!se?.clean_completed&&(fe+=Number(we.cleanCompletion||10)));let je=await d(r,`update public.game_sessions
         ...
         ) values ('game.finish',$1,$2,200,$3::jsonb,now())`,[s,e.userPrincipal.userKey,JSON.stringify(je)]),{session:je,replayed:!1,result:{success:R,levelScore:C,stars:W,seasonalResult:oe,seasonalPoints:de,ballyAwarded:fe,breakdown:U}}});a.json(o)})),i.post("/sessions/:sessionId/continue",p(async(e,a)=>{let n=P(e.params.sessionId,"sessionId"),s=Te(e),o=await d(t,`select * from public.game_continues
        where idempotency_key = $1 and user_key = $2`,[s,e.userPrincipal.userKey]);if(o)return a.json({continue:o,replayed:!0});let r=await S(t,async c=>{let u=await d(c,`select * from public.game_sessions
         ...
         returning *`,[n,e.userPrincipal.userKey,b,s]);return await c.query(`update public.game_sessions
          ...
          returning *`,[n,e.userPrincipal.userKey]);return r&&await o.query(`update public.game_profiles
            ...
            where user_key = $1`,[e.userPrincipal.userKey]),r});if(!s)throw new _(404,"Active game session was not found","not_found");a.json({session:s})})),i.post("/lives/restore",p(async(e,a)=>{let n=Te(e),s=!!e.body?.full,o=await S(t,async r=>{let c=await d(r,`select * from public.idempotency_records
          ...
          returning *`,[e.userPrincipal.userKey,s,m,y]);if(!b)throw new _(409,"Not enough Bally","insufficient_bally");let v={profile:b,cost:y,full:s};return await r.query(`insert into public.idempotency_records(
           scope, idempotency_key, actor_key, response_code, response_body, completed_at
         ) values ('game.lives.restore',$1,$2,200,$3::jsonb,now())`,[n,e.userPrincipal.userKey,JSON.stringify(v)]),{...v,replayed:!1}});a.json(o)})),i.get("/clans/leaderboard",p(async(e,a)=>{let n=String(e.query.clanType||"user");if(!["user","corporate"].includes(n))throw new _(400,"Unsupported clan category","validation_error");let[s,o]=await Promise.all([et(t),d(t,"select clan_rules from public.game_settings where singleton = true")]),r=await Bs(t,s,n),c=Math.max(2,Number(o?.clan_rules?.minimumMembers||5)),u=await w(t,`with member_scores as (
       ...
       select score.*,
        ...
        where user_key = $1 returning *`,[e.userPrincipal.userKey,e.body?.inAppEnabled===void 0?n.in_app_enabled:A(e.body.inAppEnabled),e.body?.telegramEnabled===void 0?n.telegram_enabled:A(e.body.telegramEnabled),e.body?.marketingEnabled===void 0?n.marketing_enabled:A(e.body.marketingEnabled),s,o,r]);a.json({preferences:c})})),i}var ca=D(()=>{"use strict";B();L();z();Z()});import{Router as Hs}from"express";function ua(t){let i=Hs();return i.use(H),i.get("/",p(async(e,a)=>{let n=k(e.query.limit,30,1,100),s=String(e.query.search||"").trim(),o=await w(t,`select user_row.user_key
         from public.app_users user_row
         left join public.user_profiles profile on profile.user_key = user_row.user_key
        ...
        order by user_row.last_seen_at desc limit $3`,[e.userPrincipal.userKey,s,n]),r=await Qe(t,e.userPrincipal.userKey,o.map(c=>c.user_key));a.json({people:r})})),i.get("/me",p(async(e,a)=>{let[n,s,o,r,c]=await Promise.all([lt(t,e.userPrincipal.userKey,e.userPrincipal.userKey),d(t,"select * from public.user_profiles where user_key = $1",[e.userPrincipal.userKey]),d(t,"select * from public.user_consents where user_key = $1",[e.userPrincipal.userKey]),w(t,`select clan.id, clan.name, clan.clan_type, clan.logo_url
           from (
       ...
       on conflict (user_key) do update
         set marketing_enabled = excluded.marketing_enabled,
             updated_at = now()`,[e.userPrincipal.userKey,r]),a.json({consents:c})})),i.patch("/me/privacy",p(async(e,a)=>{let n=await d(t,"select profile_privacy from public.app_users where user_key = $1",[e.userPrincipal.userKey]);if(!n)throw new _(404,"BALI profile was not found","not_found");let s={...n.profile_privacy||{}};for(let o of Gt){if(e.body?.[o]===void 0)continue;let r=String(e.body[o]);if(!dt.has(r))throw new _(400,`Invalid privacy mode for ${o}`,"validation_error");s[o]=r}await t.query("update public.app_users set profile_privacy = $1::jsonb where user_key = $2",[JSON.stringify(s),e.userPrincipal.userKey]),a.json({privacy:s})})),i.get("/me/export",p(async(e,a)=>{let n=e.userPrincipal.userKey,[s,o,r,c,u,l,m,y,b,v,h,j,N,I,x,R]=await Promise.all([d(t,"select * from public.app_users where user_key = $1",[n]),d(t,"select * from public.user_profiles where user_key = $1",[n]),d(t,"select * from public.user_consents where user_key = $1",[n]),w(t,"select * from public.clan_memberships where user_key = $1 order by created_at",[n]),w(t,"select * from public.event_attendance where user_key = $1 order by created_at",[n]),w(t,"select * from public.booking_records where user_key = $1 order by created_at",[n]),w(t,"select * from public.point_ledger where user_key = $1 order by created_at",[n]),w(t,"select * from public.user_rewards where user_key = $1 order by granted_at",[n]),w(t,`select * from public.gifts
         ...
         returning *`,[o,s,JSON.stringify({previousTelegramId:u.telegram_id,previousName:u.name,previousUsername:u.username})]);return await c.query(`update public.app_users
          ...
          where app_user_key = $1 and revoked_at is null`,[o]),m});a.clearCookie(he,{path:"/"}),a.json({deletion:r})})),i.get("/:userKey",p(async(e,a)=>{let n=$(e.params.userKey,"userKey");a.json({profile:await lt(t,e.userPrincipal.userKey,n)})})),i}var da=D(()=>{"use strict";B();L();z();_t();Z()});import{Router as Qs}from"express";function la(t){let i=Qs();return i.use(H),i.get("/",p(async(e,a)=>{let[n,s,o]=await Promise.all([w(t,`select scope, block_key, name, title, subtitle, asset_key,
          ...
          order by app_type, sort_order, item_key`),w(t,`select asset_key, name, url, media_type, width, height,
           ...
           from public.admin_assets`)]);a.json({blocks:n,navigation:s,assets:o})})),i}var _a=D(()=>{"use strict";B();L();z()});import{Router as Js}from"express";function pa(t,i){return t<i?[t,i]:[i,t]}async function vt(t,i,e){return!!await d(t,`select 1
       from public.user_blocks
      where (blocker_user_key = $1 and blocked_user_key = $2)
         or (blocker_user_key = $2 and blocked_user_key = $1)
      limit 1`,[i,e])}async function gt(t,i,e){let a=await d(t,`select conversation.*, connection.status as connection_status
     ...
     on conflict (idempotency_key) do nothing`,[i,e,a,n,JSON.stringify(s),o])}function ma(t){let i=Js();return i.use(H),i.get("/connections",p(async(e,a)=>{let n=await w(t,`select connection.*,
        ...
        order by case when connection.status = 'pending' then 0 else 1 end,
                 connection.updated_at desc`,[e.userPrincipal.userKey]);a.json({connections:n})})),i.post("/connections",p(async(e,a)=>{await O(t,e,"connection.create",M(e));let n=$(e.body?.recipientUserKey,"recipientUserKey"),s=g(e.body?.message,500),o=e.userPrincipal.userKey;if(n===o)throw new _(400,"A user cannot send a connection request to themselves","validation_error");if(await vt(t,o,n))throw new _(403,"Connection request is unavailable because one user blocked the other","user_blocked");let r=await d(t,`select user_row.user_key, user_row.name,
              coalesce(profile.allow_connections, true) as allow_connections
         from public.app_users user_row
         left join public.user_profiles profile on profile.user_key = user_row.user_key
        ...
        where requester_user_key = $1
          and created_at > now() - interval '24 hours'`,[o]);if(Number(c?.count||0)>=10)throw new _(429,"Daily connection request limit has been reached","connection_daily_limit",{retryAfter:3600});let[u,l]=pa(o,n),m=await S(t,async y=>{let b=await d(y,`select * from public.user_connections
         ...
         returning *`,[o,n,u,l,s])});await ht(t,n,"connection_request","\u041D\u043E\u0432\u0430\u044F \u0437\u0430\u044F\u0432\u043A\u0430 \u043D\u0430 \u0437\u043D\u0430\u043A\u043E\u043C\u0441\u0442\u0432\u043E",`${e.userPrincipal.name} \u0445\u043E\u0447\u0435\u0442 \u043F\u043E\u0437\u043D\u0430\u043A\u043E\u043C\u0438\u0442\u044C\u0441\u044F.`,{connectionId:m.id,requesterUserKey:o},`connection-request:${m.id}:${m.created_at}`),a.status(201).json({connection:m})})),i.patch("/connections/:connectionId",p(async(e,a)=>{let n=P(e.params.connectionId,"connectionId"),s=K(e.body?.status,"status",zs),o=await S(t,async r=>{let c=await d(r,`select * from public.user_connections
          where id = $1 and recipient_user_key = $2
          for update`,[n,e.userPrincipal.userKey]);if(!c)throw new _(404,"Connection request was not found","not_found");if(c.status!=="pending")throw new _(409,"Connection request has already been answered","connection_already_answered");if(await vt(r,c.requester_user_key,c.recipient_user_key))throw new _(403,"Connection is unavailable because one user blocked the other","user_blocked");let u=await d(r,`update public.user_connections
            set status = $2,
                responded_at = now(),
       ...
       returning *`,[n,e.userPrincipal.userKey,s,o]);await t.query("update public.direct_conversations set updated_at = now() where id = $1",[n]),await ht(t,r.peerKey,"direct_message",`\u0421\u043E\u043E\u0431\u0449\u0435\u043D\u0438\u0435 \u043E\u0442 ${e.userPrincipal.name}`,s.slice(0,300),{conversationId:n,messageId:c.id},`direct-message:${c.id}`),a.status(201).json({message:c})})),i.post("/blocks",p(async(e,a)=>{let n=$(e.body?.blockedUserKey,"blockedUserKey"),s=g(e.body?.reason,1e3);if(n===e.userPrincipal.userKey)throw new _(400,"A user cannot block themselves","validation_error");if(!await d(t,"select user_key from public.app_users where user_key = $1",[n]))throw new _(404,"User was not found","not_found");let[r,c]=pa(e.userPrincipal.userKey,n),u=await S(t,async l=>{let m=await d(l,`insert into public.user_blocks(blocker_user_key, blocked_user_key, reason)
        ...
        where blocker_user_key = $1 and blocked_user_key = $2`,[e.userPrincipal.userKey,n])).rowCount)throw new _(404,"Block was not found","not_found");a.status(204).end()})),i.post("/reports",p(async(e,a)=>{await O(t,e,"user_report.create",M(e));let n=$(e.body?.reportedUserKey,"reportedUserKey"),s=f(e.body?.reasonCode,"reasonCode",100),o=g(e.body?.details,2e3),r=e.body?.conversationId?P(e.body.conversationId,"conversationId"):null,c=e.body?.messageId?P(e.body.messageId,"messageId"):null;if(n===e.userPrincipal.userKey)throw new _(400,"A user cannot report themselves","validation_error");r&&await gt(t,r,e.userPrincipal.userKey);let u=await S(t,async l=>{let m=await d(l,`insert into public.user_reports(
         ...
         ) values ('user_report','user_report',$1,$2,'normal')`,[m.id,n]),m});a.status(201).json({report:u})})),i}var zs,ya=D(()=>{"use strict";B();L();z();ye();Z();zs=["accepted","declined"]});var wa={};ot(wa,{createApp:()=>Xs});import{randomUUID as Vs}from"node:crypto";import{mkdirSync as Gs}from"node:fs";import Ne from"node:path";import Fs from"cookie-parser";import tt from"express";import Ws from"helmet";function Xs(t,i){ba||Gs($t,{recursive:!0});let e=tt();return e.disable("x-powered-by"),i.trustProxy&&e.set("trust proxy",1),e.use((a,n,s)=>{let o=String(a.get("origin")||"");if(o&&Zs.has(o)&&(n.setHeader("Access-Control-Allow-Origin",o),n.setHeader("Access-Control-Allow-Credentials","true"),n.setHeader("Access-Control-Allow-Methods","GET,POST,PATCH,PUT,DELETE,OPTIONS"),n.setHeader("Access-Control-Allow-Headers","Content-Type,Authorization,Idempotency-Key,X-Request-ID"),n.setHeader("Access-Control-Max-Age","600"),n.append("Vary","Origin"),a.method==="OPTIONS"))return n.status(204).end();s()}),e.use((a,n,s)=>{a.requestId=String(a.get("x-request-id")||Vs()).slice(0,160),n.setHeader("x-request-id",a.requestId),n.setHeader("x-bali-environment",i.environment),s()}),e.use(Ws({contentSecurityPolicy:{directives:{defaultSrc:["'self'"],scriptSrc:["'self'","'unsafe-inline'"],styleSrc:["'self'","'unsafe-inline'","https://fonts.googleapis.com"],fontSrc:["'self'","https://fonts.gstatic.com","data:"],imgSrc:["'self'","data:","blob:","https:"],connectSrc:["'self'"],frameAncestors:["'self'"]}},crossOriginEmbedderPolicy:!1})),e.use(tt.json({limit:"256kb"})),e.use(Fs()),e.use(Mt(t,i)),e.use(Ot(t,i)),e.use("/api/v1",(a,n,s)=>{n.setHeader("Cache-Control","private, no-store, max-age=0"),n.setHeader("Pragma","no-cache"),s()}),e.get("/api/v1/health",(a,n)=>{n.json({ok:!0,environment:i.environment})}),e.get("/api/v1/config/public",(a,n)=>{n.json({environment:i.environment,demoAvailable:!["production","staging"].includes(i.environment),authentication:"mobile-password"})}),e.use("/api/v1/auth",Et(t,i)),e.use("/api/v1/auth",Lt(t,i)),e.use("/api/v1/clans",Zt(t)),e.use("/api/v1/people",ua(t)),e.use("/api/v1/events",zn(t)),e.use("/api/v1/layouts",tn(t)),e.use("/api/v1/bookings",Cn(t)),e.use("/api/v1/catalog",Un(t)),e.use("/api/v1/economy",Ln(t)),e.use("/api/v1/game",oa(t)),e.use("/api/v1/notifications",ra(t)),e.use("/api/v1/platform-config",la(t)),e.use("/api/v1/social",ma(t)),e.use("/api/v1/admin",qt(t)),e.use("/api/v1/admin",cn(t)),e.use("/api/v1/admin",mn(t)),e.use("/api/v1/admin",gn(t,$t)),e.use("/api/v1/admin",Sn(t)),e.use("/api/v1/admin",Tn(t)),e.use("/api/v1/admin",An(t)),e.use("/site",tt.static(nt,{etag:!0,maxAge:i.environment==="production"?"1h":0,index:!1})),e.use("/uploads",tt.static($t,{etag:!0,immutable:i.environment==="production",maxAge:i.environment==="production"?"1y":0,index:!1,fallthrough:!1})),e.get("/app",(a,n)=>{n.setHeader("Cache-Control","no-store, max-age=0"),n.sendFile(Ne.join(nt,"app-production.html"))}),e.get("/admin",(a,n)=>{n.setHeader("Cache-Control","no-store, max-age=0"),n.sendFile(Ne.join(nt,"admin-production.html"))}),["production","staging"].includes(i.environment)||e.get("/demo",(a,n)=>{n.sendFile(Ne.join(nt,"index.html"))}),e.get("/",(a,n)=>{n.redirect(i.environment==="production"?"/app":"/demo")}),e.use(St),e.use(Pt),e}var nt,ba,Ys,$t,Zs,fa=D(()=>{"use strict";Ut();Ht();Xt();en();un();yn();hn();Pn();Nn();Kn();En();Bn();Hn();Vn();ia();Je();ca();da();_a();ya();L();z();nt=Ne.resolve(process.cwd(),"site"),ba=!!globalThis.Deno,Ys=process.env.VERCEL||ba?"/tmp/bali-uploads":Ne.join(process.cwd(),"var","uploads"),$t=Ne.resolve(process.env.BALI_UPLOAD_DIR||Ys),Zs=new Set(["capacitor://localhost","ionic://localhost","http://localhost","https://localhost","https://nik13599.github.io"])});var ga={};ot(ga,{loadConfig:()=>eo});function kt(t,i){let e=Number.parseInt(t||"",10);return Number.isFinite(e)&&e>0?e:i}function eo(t=process.env){let i=t.BALI_ENV||t.NODE_ENV||"development",e=qs.has(i)?i:"development",a=e==="production"||e==="staging",n={environment:e,port:kt(t.PORT,8080),databaseUrl:t.DATABASE_URL||"",telegramBotToken:t.TELEGRAM_BOT_TOKEN||"",telegramBotUrl:t.TELEGRAM_BOT_URL||"",sessionSecret:t.SESSION_SECRET||"",sessionTtlSeconds:kt(t.SESSION_TTL_SECONDS,720*60*60),telegramAuthMaxAgeSeconds:kt(t.TELEGRAM_AUTH_MAX_AGE_SECONDS,300),adminBootstrapEmail:(t.ADMIN_BOOTSTRAP_EMAIL||"").trim().toLowerCase(),adminBootstrapPassword:t.ADMIN_BOOTSTRAP_PASSWORD||"",trustProxy:t.TRUST_PROXY==="1",secureCookies:a};if(a){let s=[!n.databaseUrl&&"DATABASE_URL",n.sessionSecret.length<32&&"SESSION_SECRET (minimum 32 characters)"].filter(Boolean);if(s.length)throw new Error(`Missing production configuration: ${s.join(", ")}`)}return n}var qs,ha=D(()=>{"use strict";qs=new Set(["demo","development","staging","production","test"])});import It from"node:process";import to from"express";var va=globalThis.Deno?.env,no={...It.env,BALI_ENV:"production",DATABASE_URL:va?.get("SUPABASE_DB_URL")||It.env.SUPABASE_DB_URL||"",SESSION_SECRET:va?.get("SUPABASE_SERVICE_ROLE_KEY")||It.env.SUPABASE_SERVICE_ROLE_KEY||"",TRUST_PROXY:"1",PORT:"8000"},[{createApp:ao},{loadConfig:so},{createPool:oo}]=await Promise.all([Promise.resolve().then(()=>(fa(),wa)),Promise.resolve().then(()=>(ha(),ga)),Promise.resolve().then(()=>(B(),xt))]),$a=so(no),ka=oo($a.databaseUrl,1);await ka.query("select 1");var io=ao(ka,$a),Ia=to();Ia.use((t,i)=>{let e=["/functions/v1/bali-api","/bali-api"];for(let a of e)t.url===a?t.url="/":t.url.startsWith(`${a}/`)&&(t.url=t.url.slice(a.length));return io(t,i)});Ia.listen(8e3,()=>console.log("BALI Edge API listening on 8000"));
