...
`),r=rt("sha256","WebAppData").update(i).digest(),c=rt("sha256",r).update(o).digest(),d=Buffer.from(s,"hex");if(d.length!==c.length||!Tt(d,c))throw new le("Invalid Telegram signature");let l=Number(n.get("auth_date")||0);if(!Number.isInteger(l)||l<=0)throw new le("Invalid Telegram auth date");let m=a-l;if(m<-30||m>e)throw new le("Telegram authentication expired");let y;try{y=JSON.parse(n.get("user")||"")}catch{throw new le("Invalid Telegram user payload")}if(!Number.isSafeInteger(y?.id)||y.id<=0||!String(y.first_name||"").trim())throw new le("Invalid Telegram user payload");return{user:y,authDate:l,queryId:n.get("query_id")||void 0,startParam:n.get("start_param")||void 0}}async function Qe(t){if(t.length<12)throw new Error("Administrator password must contain at least 12 characters");let i=Pt(16),e=await Nt(t,i,64);return`scrypt:${i.toString("base64url")}:${e.toString("base64url")}`}async function Se(t,i){let[e,a,n]=String(i||"").split(":");if(e!=="scrypt"||!a||!n)return!1;let s=Buffer.from(n,"base64url"),o=await Nt(t,Buffer.from(a,"base64url"),s.length);return s.length===o.length&&Tt(s,o)}function Y(t){return Ia("sha256").update(t).digest("hex")}var Nt,le,me=D(()=>{"use strict";Nt=xa(ja),le=class extends Error{status=401}});function Kt(t,i){return p(async(e,a,n)=>{let s=e.cookies?.[ve];if(!s)return n();let o=await u(t,`select s.id as session_id, s.app_user_key, s.last_seen_at,
     ...
     returning request_count`,[e,a,y.toISOString(),m,b.toISOString()]);if(Number(v?.request_count||1)>r){let h=Math.max(1,Math.ceil((b.getTime()-d)/1e3));throw new _(429,"Too many requests","rate_limit_exceeded",{bucket:e,retryAfter:h,requestId:i.requestId})}}var Sa,ye=D(()=>{"use strict";B();L();Sa={"auth.telegram":{limit:10,windowSeconds:60},"auth.admin":{limit:8,windowSeconds:300},"message.create":{limit:20,windowSeconds:60},"message.repeat":{limit:3,windowSeconds:300},"message.mentions":{limit:8,windowSeconds:60},"message.links":{limit:5,windowSeconds:60},"poll.create":{limit:5,windowSeconds:3600},"poll.vote":{limit:30,windowSeconds:60},"event.attach":{limit:10,windowSeconds:3600},"report.create":{limit:5,windowSeconds:3600},"notification.broadcast":{limit:3,windowSeconds:3600},"connection.create":{limit:10,windowSeconds:86400},"invitation.create":{limit:20,windowSeconds:86400},"event_invitation.create":{limit:20,windowSeconds:86400},"direct_message.create":{limit:60,windowSeconds:60},"user_report.create":{limit:5,windowSeconds:86400},"gift.create":{limit:20,windowSeconds:3600},"booking.hold":{limit:10,windowSeconds:60},"game.session":{limit:30,windowSeconds:3600},"content.upload":{limit:30,windowSeconds:3600}}});function f(t,i,e,a=1){let n=String(t??"").trim();if(n.length<a||n.length>e)throw new _(400,`${i} must contain ${a}-${e} characters`,"validation_error");return n}function g(t,i){let e=String(t??"").trim();if(e.length>i)throw new _(400,`Text is longer than ${i} characters`,"validation_error");return e}function P(t,i="id"){let e=String(t||"");if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(e))throw new _(400,`${i} is invalid`,"validation_error");return e}function $(t,i="id"){let e=String(t||"").trim();if(!/^[a-zA-Z0-9:_-]{1,160}$/.test(e))throw new _(400,`${i} is invalid`,"validation_error");return e}function k(t,i,e,a){if(t==null||t==="")return i;let n=Number(t);if(!Number.isInteger(n)||n<e||n>a)throw new _(400,`Value must be an integer from ${e} to ${a}`,"validation_error");return n}function A(t,i=!1){if(t===void 0)return i;if(t===!0||t===!1)return t;throw new _(400,"Value must be a boolean","validation_error")}function K(t,i,e){let a=String(t??"").trim();if(!e.includes(a))throw new _(400,`${i} must be one of: ${e.join(", ")}`,"validation_error");return a}function H(t,i,e,a){if(t==null||t==="")return i;let n=Number(t);if(!Number.isFinite(n)||n<e||n>a)throw new _(400,`Value must be a number from ${e} to ${a}`,"validation_error");return n}function ke(t,i,e,a,n=200){if(!Array.isArray(t))throw new _(400,`${i} must be an array`,"validation_error");let s=[...new Set(t.map(o=>f(o,i,n)))];if(s.length<e||s.length>a)throw new _(400,`${i} must contain ${e}-${a} unique values`,"validation_error");return s}function E(t){if(!t)return null;let i=new Date(String(t));if(Number.isNaN(i.getTime()))throw new _(400,"Invalid date","validation_error");return i.toISOString()}var Z=D(()=>{"use strict";L()});import{Router as Pa}from"express";function Dt(t){return{ipHash:Y(String(t.ip||"")),userAgent:String(t.get("user-agent")||"").slice(0,500)}}function Mt(t,i){let e=Pa();return e.post("/telegram",p(async(a,n)=>{await O(t,a,"auth.telegram",M(a));let s;try{s=At(String(a.body?.initData||""),i.telegramBotToken,i.telegramAuthMaxAgeSeconds)}catch(h){throw h instanceof le?new _(401,h.message,"telegram_auth_failed"):h}let o=s.user,r=`tg:${o.id}`,c=`${o.first_name||""} ${o.last_name||""}`.trim()||"\u0413\u043E\u0441\u0442\u044C BALI",d=await u(t,"select app_user_key from public.telegram_accounts where telegram_user_id = $1",[o.id]);if(!d){let h=await u(t,`select user_key
         ...
         ) values ($1,$2,$3,$4,$5,$6)`,[l,he(m,i.sessionSecret),new Date(s.authDate*1e3).toISOString(),y.toISOString(),b.ipHash,b.userAgent]),await h.query(`insert into public.analytics_events(
           user_key, event_name, source, entity_type, entity_id, properties
         ) values ($1,'app_open','telegram','user',$1,$2::jsonb)`,[l,JSON.stringify({firstOpen:!d})]),j});n.cookie(ve,m,Re(i,i.sessionTtlSeconds*1e3)),n.status(201).json({user:{id:v.user_key,name:v.name,username:v.username,avatar:v.avatar},environment:i.environment,expiresAt:y.toISOString()})})),e.get("/session",Q,p(async(a,n)=>{n.json({user:{id:a.userPrincipal.userKey,name:a.userPrincipal.name,username:a.userPrincipal.username},environment:i.environment})})),e.post("/logout",Q,p(async(a,n)=>{await t.query("update public.user_sessions set revoked_at = now() where id = $1",[a.userPrincipal.sessionId]),n.clearCookie(ve,{path:"/"}),n.status(204).end()})),e.post("/admin/login",p(async(a,n)=>{await O(t,a,"auth.admin",M(a));let s=f(a.body?.email,"email",320).toLowerCase(),o=f(a.body?.password,"password",1e3),r=await u(t,"select id, email, password_hash, role, status from public.admin_users where email = $1",[s]);if(!r||r.status!=="active"||!await Se(o,r.password_hash))throw new _(401,"Invalid administrator credentials","admin_login_failed");let c=ie(),d=new Date(Date.now()+720*60*1e3),l=Dt(a);await t.query(`insert into public.admin_sessions(
         admin_user_id, token_hash, expires_at, ip_hash, user_agent
       ) values ($1,$2,$3,$4,$5)`,[r.id,he(c,i.sessionSecret),d.toISOString(),l.ipHash,l.userAgent]),n.cookie(He,c,Re(i,720*60*1e3)),n.json({admin:{id:r.id,email:r.email,role:r.role},expiresAt:d})})),e.get("/admin/session",X,p(async(a,n)=>{n.json({admin:a.adminPrincipal})})),e.post("/admin/logout",X,p(async(a,n)=>{await t.query("update public.admin_sessions set revoked_at = now() where id = $1",[a.adminPrincipal.sessionId]),n.clearCookie(He,{path:"/"}),n.status(204).end()})),e}var Ot=D(()=>{"use strict";B();L();z();ye();me();Z()});import{Router as Ta}from"express";function ct(t){let e=f(t,"phone",40).replace(/\D/g,"");if(e.length<9||e.length>15)throw new _(400,"\u0412\u0432\u0435\u0434\u0438\u0442\u0435 \u043A\u043E\u0440\u0440\u0435\u043A\u0442\u043D\u044B\u0439 \u043D\u043E\u043C\u0435\u0440 \u0442\u0435\u043B\u0435\u0444\u043E\u043D\u0430","invalid_phone");return`+${e}`}function Ct(t){let i=f(t,"telegramUsername",64).replace(/^@+/,"");if(!/^[A-Za-z0-9_]{5,32}$/.test(i))throw new _(400,"\u0412\u0432\u0435\u0434\u0438\u0442\u0435 Telegram username \u0432 \u0444\u043E\u0440\u043C\u0430\u0442\u0435 @username","invalid_telegram_username");return i}function Na(t){return{ipHash:Y(String(t.ip||"")),userAgent:String(t.get("user-agent")||"").slice(0,500)}}async function Aa(t,i,e,a,n){let s=ie(),o=new Date(Date.now()+i.sessionTtlSeconds*1e3),r=Na(e);return await t.query(`insert into public.user_sessions(
       app_user_key, token_hash, telegram_auth_date, expires_at, ip_hash, user_agent, auth_method
     ) values ($1,$2,now(),$3,$4,$5,'mobile')`,[n.user_key,he(s,i.sessionSecret),o.toISOString(),r.ipHash,r.userAgent]),a.cookie(ve,s,Re(i,i.sessionTtlSeconds*1e3)),o}function Et(t,i){let e=Ta();return e.get("/mobile/session",Q,p(async(a,n)=>{n.json({user:{id:a.userPrincipal.userKey,name:a.userPrincipal.name,username:a.userPrincipal.username},authMethod:a.userPrincipal.authMethod,mustChangePassword:a.userPrincipal.mustChangePassword})})),e.post("/mobile/register-request",p(async(a,n)=>{await O(t,a,"auth.mobile_request",M(a));let s=ct(a.body?.phone),o=Ct(a.body?.telegramUsername),r=f(a.body?.displayName,"displayName",120,2);if(await u(t,"select app_user_key from public.mobile_credentials where phone = $1",[s]))throw new _(409,"\u0410\u043A\u043A\u0430\u0443\u043D\u0442 \u0441 \u044D\u0442\u0438\u043C \u043D\u043E\u043C\u0435\u0440\u043E\u043C \u0443\u0436\u0435 \u0441\u0443\u0449\u0435\u0441\u0442\u0432\u0443\u0435\u0442","account_exists");let d=await u(t,`insert into public.mobile_access_requests(
       ...
       do update set telegram_username = excluded.telegram_username,
                     ...
                     requested_at = now(),
                     updated_at = now()
       returning id, status, requested_at`,[s,o,r]);n.status(202).json({request:d,message:"\u0417\u0430\u044F\u0432\u043A\u0430 \u043E\u0442\u043F\u0440\u0430\u0432\u043B\u0435\u043D\u0430 \u0430\u0434\u043C\u0438\u043D\u0438\u0441\u0442\u0440\u0430\u0442\u043E\u0440\u0443"})})),e.post("/mobile/reset-request",p(async(a,n)=>{await O(t,a,"auth.mobile_reset",M(a));let s=ct(a.body?.phone),o=Ct(a.body?.telegramUsername),r=await u(t,`select app_user_key from public.mobile_credentials
        where phone = $1 and lower(telegram_username) = lower($2)`,[s,o]);r&&await t.query(`insert into public.mobile_access_requests(
         ...
         do update set telegram_username = excluded.telegram_username,
                       ...
                       requested_at = now(),
                       updated_at = now()`,[s,o,r.app_user_key]),n.status(202).json({message:"\u0415\u0441\u043B\u0438 \u0434\u0430\u043D\u043D\u044B\u0435 \u0441\u043E\u0432\u043F\u0430\u043B\u0438, \u0430\u0434\u043C\u0438\u043D\u0438\u0441\u0442\u0440\u0430\u0442\u043E\u0440 \u043F\u043E\u043B\u0443\u0447\u0438\u043B \u0437\u0430\u043F\u0440\u043E\u0441 \u043D\u0430 \u0432\u043E\u0441\u0441\u0442\u0430\u043D\u043E\u0432\u043B\u0435\u043D\u0438\u0435"})})),e.post("/mobile/login",p(async(a,n)=>{await O(t,a,"auth.mobile_login",M(a));let s=ct(a.body?.phone),o=f(a.body?.password,"password",256),r=await u(t,`select c.app_user_key, c.password_hash, c.must_change_password, c.locked_until,
       ...
       values ($1,'app_open','mobile','user',$1,$2::jsonb)`,[r.app_user_key,JSON.stringify({authMethod:"mobile"})]),n.json({user:{id:r.user_key,name:r.name,username:r.username,avatar:r.avatar},mustChangePassword:!!r.must_change_password,expiresAt:c.toISOString()})})),e.post("/mobile/change-password",Q,p(async(a,n)=>{await O(t,a,"auth.mobile_password",M(a));let s=f(a.body?.currentPassword,"currentPassword",256),o=f(a.body?.newPassword,"newPassword",128,12),r=await u(t,"select password_hash from public.mobile_credentials where app_user_key = $1",[a.userPrincipal.userKey]);if(!r||!await Se(s,r.password_hash))throw new _(401,"\u0422\u0435\u043A\u0443\u0449\u0438\u0439 \u043F\u0430\u0440\u043E\u043B\u044C \u0432\u0432\u0435\u0434\u0451\u043D \u043D\u0435\u0432\u0435\u0440\u043D\u043E","current_password_invalid");if(await Se(o,r.password_hash))throw new _(400,"\u041D\u043E\u0432\u044B\u0439 \u043F\u0430\u0440\u043E\u043B\u044C \u0434\u043E\u043B\u0436\u0435\u043D \u043E\u0442\u043B\u0438\u0447\u0430\u0442\u044C\u0441\u044F \u043E\u0442 \u0432\u0440\u0435\u043C\u0435\u043D\u043D\u043E\u0433\u043E","password_not_changed");let c=await Qe(o);await t.query(`update public.mobile_credentials
        ...
        where app_user_key = $1 and id <> $2 and revoked_at is null`,[a.userPrincipal.userKey,a.userPrincipal.sessionId]),n.json({ok:!0,mustChangePassword:!1})})),e}var Ut=D(()=>{"use strict";B();L();z();ye();me();Z()});async function Ie(t,i,e,a){let n=await u(t,`select
     ...
     where m.clan_id = $1 and m.user_key = $2`,[e,i.userKey]);if(!n||n.membership_status!=="active"||n.clan_status!=="active")return{allowed:!1,source:"none"};let s=await u(t,`select * from public.clan_chat_restrictions
     ...
     ) values ($1,$2,$3,$4,$5,$6,$7,$8::jsonb,$9::jsonb,$10,$11)`,[i.adminPrincipal.adminId,i.adminPrincipal.email,e.action,e.targetType,e.targetId||"",i.requestId,e.reason||"",e.before===void 0?null:JSON.stringify(e.before),e.after===void 0?null:JSON.stringify(e.after),Y(String(i.ip||"")),String(i.get("user-agent")||"").slice(0,500)])}var $e=D(()=>{"use strict";B();me()});function zt(t,i){let e=["avatar","status","clan"].includes(i)?"public":i==="events"?"clan":"private",a=String(t?.[i]||e);return dt.has(a)?a:"private"}async function lt(t,i,e){let a=await u(t,`select user_row.user_key,
      ...
      limit 1`,[i,e]))throw new _(404,"BALI profile was not found","not_found");if(!a.discoverable&&!n&&!c&&!d)throw new _(404,"BALI profile was not found","not_found");let m=b=>{if(n)return!0;let v=zt(a.profile_privacy,b);return v==="public"?!0:v==="clan"?c:!1},y={id:a.user_key,name:a.name,bio:a.bio||"",interests:a.interests||[],gender:a.gender||"unspecified",actions:{canConnect:!n&&!!a.allow_connections,canInvite:!n&&!!a.allow_event_invites,canGift:!n&&!!a.allow_gifts},privacy:n?a.profile_privacy:void 0};return m("avatar")&&a.avatar&&(y.avatar=a.avatar),m("username")&&a.username&&(y.username=a.username),m("phone")&&a.phone&&(y.phone=a.phone),m("birth_date")&&a.birth_date&&(y.birthDate=a.birth_date),m("status")&&a.status_text&&(y.status=a.status_text),m("clan")&&(y.clans=await(async()=>(await t.query(`select clan.id, clan.name, clan.clan_type,
        ...
        limit 1`,[e])),y}async function Je(t,i,e){let a=[...new Set(e)].filter(Boolean).slice(0,100);if(!a.length)return[];if(!await u(t,`select user_key from public.app_users
      ...
      order by created_at desc limit 30`,[i]);if(!a.length)return[];let n=[];for(let s of a){let o=await w(t,`select o.id, o.label, o.sort_order, count(v.id)::integer as votes
        ...
        order by event_date, event_time limit 100`);a.json({events:n})})),i.post("/:clanId/messages",G(t,"message.create"),p(async(e,a)=>{await O(t,e,"message.create",M(e,e.params.clanId));let n=f(e.body?.body,"body",4e3),s=e.body?.replyToId?P(e.body.replyToId,"replyToId"):null,o=F(e);if(s){if(!(await Ie(t,e.userPrincipal,e.params.clanId,"message.reply")).allowed)throw new _(403,"Reply is not permitted","permission_denied");if(!await u(t,"select id from public.clan_chat_messages where id = $1 and chat_id = $2",[s,o]))throw new _(404,"Reply target was not found","not_found")}let r=n.match(/https?:\/\/\S+/gi)||[];r.length&&await O(t,e,"message.links",M(e,e.params.clanId),r.length);let c=n.match(/@[\p{L}\p{N}_]{2,32}/gu)||[];c.length&&await O(t,e,"message.mentions",M(e,e.params.clanId),c.length);let d=Y(n.toLocaleLowerCase("ru").replace(/\s+/g," ").trim()).slice(0,24);await O(t,e,"message.repeat",M(e,`${e.params.clanId}:${d}`));let l=await S(t,async m=>{let y=await u(m,`insert into public.clan_chat_messages(
         ...
         returning *`,[o,e.userPrincipal.userKey,n,s]);return s&&await m.query(`insert into public.clan_chat_message_replies(message_id, parent_message_id)
           values ($1,$2)`,[y.id,s]),y});a.status(201).json({message:Gt({...l,author_name:e.userPrincipal.name})})})),i.delete("/:clanId/messages/:messageId",p(async(e,a)=>{let n=await Ie(t,e.userPrincipal,e.params.clanId,"message.delete_own");if(!n.membership?.chat_id)throw new _(403,"Clan access is denied","permission_denied");let s=P(e.params.messageId,"messageId"),o=await u(t,"select * from public.clan_chat_messages where id = $1 and chat_id = $2",[s,n.membership.chat_id]);if(!o)throw new _(404,"Message was not found","not_found");if(o.deleted_at)return a.status(204).end();let r=o.author_user_key===e.userPrincipal.userKey,c=(Date.now()-new Date(o.created_at).getTime())/1e3,d=n;if((!r||c>Number(n.chat?.own_delete_window_seconds||0))&&(d=await Ie(t,e.userPrincipal,e.params.clanId,"message.delete_any")),!d.allowed)throw new _(403,"Message deletion is not permitted","permission_denied");let l=g(e.body?.reason,500),m=ne(d),y=m==="leader"||m==="delegate"?"\u0421\u043E\u043E\u0431\u0449\u0435\u043D\u0438\u0435 \u0443\u0434\u0430\u043B\u0435\u043D\u043E \u0440\u0443\u043A\u043E\u0432\u043E\u0434\u0438\u0442\u0435\u043B\u0435\u043C \u043A\u043B\u0430\u043D\u0430":"\u0421\u043E\u043E\u0431\u0449\u0435\u043D\u0438\u0435 \u0443\u0434\u0430\u043B\u0435\u043D\u043E \u0430\u0432\u0442\u043E\u0440\u043E\u043C";await t.query(`update public.clan_chat_messages
...
${s}`:s]),c});await ee(t,e,{actorType:ne(e.permissionDecision),actorId:e.userPrincipal.userKey,permissionKey:"announcement.create",action:"announcement.create",targetType:"announcement",targetId:o.id,clanId:e.params.clanId,after:{title:n,body:s}}),a.status(201).json({announcement:o})})),i.post("/:clanId/pins",p(async(e,a)=>{let n=f(e.body?.targetType,"targetType",30),s=P(e.body?.targetId,"targetId"),o=n==="message"?"message.pin":n==="poll"?"poll.pin":n==="event"?"event.pin":"announcement.create",r=await Ie(t,e.userPrincipal,e.params.clanId,o);if(e.permissionDecision=r,!r.allowed)throw new _(403,"Pin is not permitted","permission_denied");await Ma(t,n,s,String(r.chat.chat_id));let c=await u(t,`insert into public.clan_chat_pins(chat_id, target_type, target_id, pinned_by_user_key)
       ...
       returning *`,[F(e),e.userPrincipal.userKey,n,A(e.body?.announcementsOnly)]);a.json({preference:s})})),i.get("/:clanId/audit",G(t,"audit.read"),p(async(e,a)=>{let n=k(e.query.limit,50,1,100),s=await w(t,`select * from public.clan_chat_audit_log
        ...
        order by created_at desc limit $3`,[e.params.clanId,e.userPrincipal.userKey,n]);a.json({audit:s})})),i}var Wt=D(()=>{"use strict";B();L();z();Ht();ye();Z();$e();_t();me()});import{randomUUID as Oa}from"node:crypto";import{Router as Ca}from"express";function te(...t){return(i,e,a)=>{if(!i.adminPrincipal||!t.includes(i.adminPrincipal.role))return a(new _(403,"Administrator role does not permit this action","admin_permission_denied"));a()}}function Ea(t){let i=String(t||"").trim().toLowerCase();if(i!=="user"&&i!=="corporate")throw new _(400,"clanType must be user or corporate","validation_error");return i}async function _e(t,i){let e=await u(t,`select c.id as clan_id, c.name as clan_name, c.clan_type, c.status as clan_status,
            c.rating_points,
...
`);a.type("text/csv").attachment("bali-clan-audit.csv").send(b);return}a.json({audit:m})})),i.get("/rate-limits",p(async(e,a)=>{let n=await w(t,"select * from public.rate_limit_settings order by bucket");a.json({settings:n})})),i.put("/rate-limits/:bucket",te("admin","superadmin"),p(async(e,a)=>{let n=String(e.params.bucket||"");if(!/^[a-z][a-z0-9.-]{1,80}$/.test(n))throw new _(400,"Invalid rate-limit bucket","validation_error");let s=await u(t,"select * from public.rate_limit_settings where bucket = $1",[n]);if(!s)throw new _(404,"Rate-limit setting was not found","not_found");let o=await u(t,`update public.rate_limit_settings
          ...
          where bucket = $5 returning *`,[k(e.body?.limitCount,Number(s.limit_count),1,1e5),k(e.body?.windowSeconds,Number(s.window_seconds),1,86400),e.body?.enabled===void 0?s.enabled:A(e.body.enabled),e.adminPrincipal.adminId,n]);await ae(t,e,{action:"rate_limit.update",targetType:"rate_limit_setting",targetId:n,before:s,after:o}),a.json({setting:o})})),i}var Zt=D(()=>{"use strict";B();L();z();$e();Z()});import{Router as Ba}from"express";async function De(t,i,e=!1){let a=await u(t,`select * from public.hall_layouts
      where id = $1
       ...
       returning *`,[s,o,n,g(e.body?.internalDescription,2e3),k(e.body?.canvasWidth,1e3,240,1e4),k(e.body?.canvasHeight,1400,240,1e4),g(e.body?.backgroundUrl,2e3),e.adminPrincipal.adminId]);await T(t,e,{action:"layout.create",targetType:"hall_layout",targetId:s,after:r}),a.status(201).json({layout:r})})),i.post("/layouts/:layoutId/clone",p(async(e,a)=>{let n=$(e.params.layoutId,"layoutId"),s=await S(t,async o=>{let r=await u(o,"select * from public.hall_layouts where id = $1 for update",[n]);if(!r)throw new _(404,"Layout was not found","not_found");let c=await u(o,`select coalesce(max(version), 0)::integer + 1 as version
           from public.hall_layouts where layout_family_key = $1`,[r.layout_family_key]),d=`layout-${Ve()}`,l=await u(o,`insert into public.hall_layouts(
           id, layout_family_key, name, internal_description, canvas_width,
       ...
       returning *`,[s,n,f(e.body?.tableNumber,"tableNumber",80),g(e.body?.name,160),H(e.body?.x,0,-1e4,1e4),H(e.body?.y,0,-1e4,1e4),H(e.body?.width,8,.1,1e4),H(e.body?.height,8,.1,1e4),H(e.body?.rotation,0,-3600,3600),K(e.body?.shape||"round","shape",qt),o,r,H(e.body?.minimumDeposit,0,0,1e9),K(e.body?.tableType||"regular","tableType",en),g(e.body?.description,2e3),K(e.body?.status||"available","status",tn),k(e.body?.sortOrder,0,-1e6,1e6),A(e.body?.active,!0)]);await T(t,e,{action:"layout.table.create",targetType:"layout_table",targetId:s,after:c}),a.status(201).json({table:c})})),i.patch("/layouts/:layoutId/tables/:tableId",p(async(e,a)=>{let n=$(e.params.layoutId,"layoutId"),s=$(e.params.tableId,"tableId");await Pe(t,n);let o=await u(t,"select * from public.layout_tables where id = $1 and layout_id = $2",[s,n]);if(!o)throw new _(404,"Table was not found","not_found");let r=k(e.body?.capacity,Number(o.capacity),1,100),c=k(e.body?.recommendedGuests,Math.min(Number(o.recommended_guests),r),1,r),d=await u(t,`update public.layout_tables
       ...
       returning *`,[i.userKey,i.amount,s,o,i.operationType,i.sourceType,i.sourceId||"",i.reason||"",i.administratorId||null,i.idempotencyKey,JSON.stringify(i.metadata||{})]),replayed:!1}})}var Me=D(()=>{"use strict";B();L()});async function pt(t,i,e=null){return S(t,async a=>{let n=await u(a,"select * from public.game_seasons where id = $1 for update",[i]);if(!n)throw new _(404,"Game season was not found","not_found");if(n.status==="completed"){let c=await w(a,`select user_key, position, reward_payload, status
      ...
      order by ends_at limit 20`);for(let e of i)await pt(t,e.id)}var mt=D(()=>{"use strict";B();Me();L()});import{randomUUID as Ge}from"node:crypto";import{Router as Ha}from"express";function pe(t,i){if(!t||Array.isArray(t)||typeof t!="object")throw new _(400,`${i} must be an object`,"validation_error");return t}function we(t,i){if(!Array.isArray(t))throw new _(400,`${i} must be an array`,"validation_error");return t}async function Oe(t,i,e){if(!new Set(["reward_definitions","gift_catalog","vip_plans","shop_items","game_seasons"]).has(i))throw new Error("Unsupported administrator catalog table");let n=await u(t,`select * from public.${i} where id = $1`,[e]);if(!n)throw new _(404,"Catalog item was not found","not_found");return n}function ln(t){let i=Ha();return i.use(X),i.get("/economy",p(async(e,a)=>{let[n,s,o,r,c,d,l,m]=await Promise.all([u(t,"select * from public.economy_settings where singleton = true"),w(t,"select * from public.reward_definitions order by updated_at desc"),w(t,"select * from public.gift_catalog order by sort_order, name"),w(t,"select * from public.vip_plans order by sort_order, points_cost"),w(t,"select * from public.shop_items order by sort_order, name"),u(t,"select * from public.game_settings where singleton = true"),w(t,"select * from public.game_seasons order by starts_at desc"),w(t,"select * from public.game_symbol_versions order by created_at desc limit 200")]);a.json({settings:n,rewards:s,gifts:o,vipPlans:r,shopItems:c,gameSettings:d,seasons:l,gameSymbolVersions:m})})),i.patch("/economy/settings",p(async(e,a)=>{let n=await u(t,"select * from public.economy_settings where singleton = true");if(!n)throw new _(500,"Economy settings are missing","economy_settings_missing");let s=await u(t,`update public.economy_settings
        ...
        order by ledger.created_at desc limit 1000`,[n]);a.json({ledger:s})})),i.post("/points/adjustments",p(async(e,a)=>{let n=$(e.body?.userKey,"userKey"),s=k(e.body?.amount,0,-1e9,1e9);if(!s)throw new _(400,"amount must not be zero","validation_error");let o=f(e.body?.reason,"reason",1e3),r=f(e.body?.idempotencyKey,"idempotencyKey",160),c=await re(t,{userKey:n,amount:s,operationType:"adjustment",sourceType:"admin",sourceId:e.adminPrincipal.adminId,reason:o,administratorId:e.adminPrincipal.adminId,idempotencyKey:`admin-adjustment:${r}`});await T(t,e,{action:"points.adjust",targetType:"app_user",targetId:n,reason:o,after:c.ledger}),a.status(c.replayed?200:201).json(c)})),i.post("/rewards",p(async(e,a)=>{let n=e.body?.id?$(e.body.id,"id"):`reward-${Ge()}`,s=e.body?.conditionConfig===void 0?{}:pe(e.body.conditionConfig,"conditionConfig"),o=await u(t,`insert into public.reward_definitions(
       ...
       returning *`,[n,f(e.body?.name,"name",160),k(e.body?.pointsCost,0,0,1e9),k(e.body?.durationDays,30,1,3650),JSON.stringify(we(e.body?.benefits||[],"benefits")),H(e.body?.pointsMultiplier,1,1,100),k(e.body?.extraGameLives,0,0,1e3),JSON.stringify(we(e.body?.eventAccess||[],"eventAccess")),JSON.stringify(we(e.body?.shopAccess||[],"shopAccess")),k(e.body?.bookingPriority,0,-1e3,1e3),g(e.body?.profileFrameUrl,2e3),g(e.body?.badgeUrl,2e3),A(e.body?.active,!0),k(e.body?.sortOrder,0,-1e6,1e6),e.adminPrincipal.adminId]);await T(t,e,{action:"vip.plan.create",targetType:"vip_plan",targetId:n,after:s}),a.status(201).json({vipPlan:s})})),i.patch("/vip/plans/:planId",p(async(e,a)=>{let n=$(e.params.planId,"planId"),s=await Oe(t,"vip_plans",n),o=await u(t,`update public.vip_plans
       ...
       returning *`,[f(e.body?.name,"name",160),g(e.body?.description,1e3),n,s,K(e.body?.status||"scheduled","status",Ja),JSON.stringify(we(e.body?.rewards||[],"rewards")),JSON.stringify(e.body?.configuration===void 0?{}:pe(e.body.configuration,"configuration")),K(e.body?.progressMode||"account_keep_season_reset","progressMode",["account_keep_season_reset","carry_all","reset_all"]),e.adminPrincipal.adminId]);await T(t,e,{action:"game.season.create",targetType:"game_season",targetId:r.id,after:r}),a.status(201).json({season:r})})),i.post("/game/seasons/:seasonId/finalize",p(async(e,a)=>{let n=P(e.params.seasonId,"seasonId"),s=f(e.body?.reason,"reason",1e3),o=await pt(t,n,e.adminPrincipal.adminId);await T(t,e,{action:"game.season.finalize",targetType:"game_season",targetId:n,reason:s,after:o}),a.json(o)})),i.post("/game/sessions/:sessionId/exclude",p(async(e,a)=>{let n=P(e.params.sessionId,"sessionId"),s=f(e.body?.reason,"reason",1e3),o=await u(t,"select * from public.game_sessions where id = $1",[n]);if(!o)throw new _(404,"Game session was not found","not_found");let r=await u(t,`update public.game_sessions
          set status = 'excluded', suspicious = true,
              excluded_by_admin_id = $2, exclusion_reason = $3, updated_at = now()
        where id = $1 returning *`,[n,e.adminPrincipal.adminId,s]);await T(t,e,{action:"game.session.exclude",targetType:"game_session",targetId:n,reason:s,before:o,after:r}),a.json({session:r})})),i}var cn,un,dn,Ja,_n=D(()=>{"use strict";$e();B();Me();L();mt();z();Z();cn=["common","rare","epic","legendary"],un=["virtual","physical"],dn=["draft","active","sold_out","archived"],Ja=["scheduled","active","completed","archived"]});import{randomUUID as za}from"node:crypto";import{writeFile as Va}from"node:fs/promises";import Ga from"node:path";import Fa from"express";import{Router as Wa}from"express";function qa(t,i){return i==="image/png"?t.length>=24&&t.subarray(0,8).equals(Buffer.from([137,80,78,71,13,10,26,10])):i==="image/jpeg"?t.length>=4&&t[0]===255&&t[1]===216:i==="image/webp"?t.length>=12&&t.toString("ascii",0,4)==="RIFF"&&t.toString("ascii",8,12)==="WEBP":!1}function Fe(t,i){if(!t||Array.isArray(t)||typeof t!="object")throw new _(400,`${i} must be an object`,"validation_error");return t}function ce(t,i){return t==null||t===""?null:k(t,0,1,1e5)}function mn(t){let i=t===void 0?{}:Fe(t,"segment");return{userKeys:i.userKeys===void 0||i.userKeys===null?null:Array.isArray(i.userKeys)?[...new Set(i.userKeys.map(a=>$(a,"userKey")))].slice(0,1e4):(()=>{throw new _(400,"segment.userKeys must be an array","validation_error")})(),marketingOnly:A(i.marketingOnly),clanId:i.clanId?$(i.clanId,"clanId"):null,hasVip:i.hasVip===void 0||i.hasVip===null?null:A(i.hasVip)}}async function yn(t,i){return w(t,`select user_row.user_key, user_row.name, account.telegram_user_id,
      ...
      where customer.user_key = $1`,[i]);if(!e)throw new _(404,"CRM customer was not found","not_found");return e}function In(t){let i=ts();return i.use(X),i.get("/dashboard",p(async(e,a)=>{let[n,s,o,r,c,d,l]=await Promise.all([u(t,"select count(*)::integer as value from public.app_users where account_status = 'active'"),u(t,`select count(*)::integer as value
        ...
        limit $4`,[n,s,o,r]);a.json({users:c})})),i.get("/crm/users/:userKey",p(async(e,a)=>{let n=$(e.params.userKey,"userKey"),s=await Te(t,n),[o,r,c,d,l,m,y,b,v,h]=await Promise.all([w(t,`select tag.* from public.crm_tags tag
         ...
         where link.customer_id = $1 order by tag.name`,[s.id]),w(t,`select note.*, admin.email as admin_email
          ...
          where user_key = $1`,[n,r]),r!=="active"&&await l.query(`update public.user_sessions set revoked_at = coalesce(revoked_at, now())
            where app_user_key = $1`,[n]),Te(l,n)));await T(t,e,{action:"crm.customer.update",targetType:"crm_customer",targetId:n,reason:c,before:s,after:d}),a.json({customer:d})})),i.post("/crm/users/:userKey/notes",p(async(e,a)=>{let n=$(e.params.userKey,"userKey"),s=await Te(t,n),o=await u(t,`insert into public.crm_notes(customer_id, body, created_by_admin_id)
     ...
     on conflict (idempotency_key) do nothing`,[i,e,a,n,JSON.stringify(s),o])}function bt(t){return bs.toDataURL(t,{errorCorrectionLevel:"M",margin:2,width:640,color:{dark:"#080a08",light:"#ffffff"}})}function En(t){let i=ys();return i.use(Q),i.get("/points",p(async(e,a)=>{let[n,s]=await Promise.all([u(t,"select * from public.point_accounts where user_key = $1",[e.userPrincipal.userKey]),w(t,`select * from public.point_ledger
        ...
        returning gift.id, gift.status, gift.expires_at, catalog.name`,[n,e.userPrincipal.userKey,Y(s)]);if(!o)throw new _(409,"This gift cannot be redeemed","gift_redemption_unavailable");a.json({gift:o,token:s,qrDataUrl:await bt(s)})})),i.get("/vip",p(async(e,a)=>{let[n,s]=await Promise.all([w(t,"select * from public.vip_plans where active = true order by sort_order, points_cost"),w(t,`select subscription.*, plan.name, plan.benefits, plan.badge_url,
                plan.profile_frame_url, plan.points_multiplier, plan.extra_game_lives
           from public.user_vip_subscriptions subscription
           join public.vip_plans plan on plan.id = subscription.plan_id
          where subscription.user_key = $1
          order by subscription.ends_at desc`,[e.userPrincipal.userKey])]);a.json({plans:n,subscriptions:s})})),i.post("/vip/purchase",p(async(e,a)=>{let n=yt(e),s=$(e.body?.planId,"planId"),o=e.userPrincipal.userKey,r=await u(t,`select * from public.user_vip_subscriptions
        where idempotency_key = $1 and user_key = $2`,[n,o]);if(r)return a.json({subscription:r,replayed:!0});let c;try{c=await S(t,async d=>{let l=await u(d,"select * from public.vip_plans where id = $1 and active = true for update",[s]);if(!l)throw new _(404,"VIP plan was not found","not_found");let m=await u(d,`select * from public.user_vip_subscriptions
         ...
         returning *`,[s,b,v?.ledger?.id||null,Y(d),n]);for(let j of y)await m.query(`insert into public.shop_order_items(
            ...
            where idempotency_key = $1 and user_key = $2`,[n,s]);if(y)return a.json({order:y,replayed:!0})}throw m}a.status(201).json({order:l.order,qrToken:l.requiresRedemption?d:void 0,qrDataUrl:l.requiresRedemption?await bt(d):void 0,replayed:!1})})),i.post("/shop/orders/:orderId/qr",p(async(e,a)=>{let n=P(e.params.orderId,"orderId"),s=ie(),o=await u(t,`update public.shop_orders shop_order
          set qr_token_hash = $3, updated_at = now()
     ...
     on conflict (idempotency_key) do nothing`,[i.userKey,i.type,i.title,i.body,JSON.stringify(i.data||{}),i.idempotencyKey])}function Qn(t){let i=fs();return i.use(Q),i.get("/",p(async(e,a)=>{let n=await w(t,`select e.id, e.title, e.event_date, e.event_time, e.description, e.image_url,
        ...
        where layout_table.layout_id = $3 and layout_table.active = true`,[n,e.userPrincipal.userKey,s.layout_id]),c=new Map(r.map(d=>[d.id,d]));a.json({eventId:n,assignment:s,layout:o.layout,elements:o.elements,tables:o.tables.map(d=>({...d,...c.get(d.id)||{availability_status:"available"}}))})})),i.get("/:eventId",p(async(e,a)=>{let n=$(e.params.eventId,"eventId"),s=await Ce(t,n),[o,r,c]=await Promise.all([w(t,`select attendance.status, attendance.updated_at,
          ...
          where checkin.event_id = $1
            and coalesce(profile.discoverable, true) = true
          order by checkin.checked_in_at desc`,[n])]);a.json({event:s,participants:o,checkedIn:c,clans:r})})),i.put("/:eventId/attendance",p(async(e,a)=>{let n=$(e.params.eventId,"eventId"),s=K(e.body?.status,"status",gs),o=await Ce(t,n);if(!Bn(o)&&s!=="cancelled")throw new _(409,"Attendance can be changed only for an active future event","event_not_active");let r=await u(t,`insert into public.event_attendance(event_id, user_key, status, source_type)
       values ($1,$2,$3,'self')
       on conflict (event_id, user_key) do update
         set status = excluded.status,
             source_type = 'self',
       ...
       returning *`,[n,e.userPrincipal.userKey,s]);a.json({attendance:r})})),i.post("/:eventId/invitations",p(async(e,a)=>{await O(t,e,"event_invitation.create",M(e));let n=$(e.params.eventId,"eventId"),s=$(e.body?.recipientUserKey,"recipientUserKey"),o=g(e.body?.message,500);if(s===e.userPrincipal.userKey)throw new _(400,"A user cannot invite themselves","validation_error");let r=await Ce(t,n);if(!Bn(r))throw new _(409,"Only active future events can be invited to","event_not_active");let c=await u(t,`select user_row.user_key, user_row.name,
        ...
        where user_row.user_key = $2 and user_row.account_status = 'active'`,[e.userPrincipal.userKey,s]);if(!c)throw new _(404,"Recipient was not found","not_found");if(c.blocked)throw new _(403,"Invitation is unavailable because one of the users blocked the other","user_blocked");if(!c.allow_event_invites)throw new _(403,"Recipient disabled event invitations","event_invitations_disabled");try{let d=await S(t,async l=>{let m=await u(l,`insert into public.event_invitations(
           ...
           returning *`,[n,e.userPrincipal.userKey,s,o]);return await Ln(l,{userKey:s,type:"event_invitation",title:"\u041F\u0440\u0438\u0433\u043B\u0430\u0448\u0435\u043D\u0438\u0435 \u043D\u0430 \u043C\u0435\u0440\u043E\u043F\u0440\u0438\u044F\u0442\u0438\u0435",body:`${e.userPrincipal.name} \u043F\u0440\u0438\u0433\u043B\u0430\u0448\u0430\u0435\u0442 \u0432\u0430\u0441 \u043D\u0430 \xAB${r.title}\xBB.`,data:{invitationId:m.id,eventId:n},idempotencyKey:`event-invitation:${m.id}`}),m});a.status(201).json({invitation:d})}catch(d){throw d?.code==="23505"?new _(409,"An unanswered invitation already exists","invitation_already_pending"):d}})),i.post("/:eventId/archive-invitations",p(async(e,a)=>{let n=$(e.params.eventId,"eventId");await Ce(t,n);let s=await t.query(`update public.event_invitations
          set status = 'archived', archived_at = now(), updated_at = now()
        where event_id = $1
          and (sender_user_key = $2 or recipient_user_key = $2)
          and status <> 'archived'`,[n,e.userPrincipal.userKey]);a.json({archived:s.rowCount||0})})),i}var gs,hs,Hn=D(()=>{"use strict";B();L();z();ye();Z();ze();gs=["going","maybe","not_going","cancelled"],hs=["going","maybe","declined"]});import{createHash as vs}from"node:crypto";function Xe(t){return t&&!Array.isArray(t)&&typeof t=="object"?t:{}}function ue(t,i,e,a){let n=Number(t);return Number.isFinite(n)?Math.max(e,Math.min(a,Math.round(n))):i}function Ee(t,i,e,a){let n=Number(t);return Number.isFinite(n)?Math.max(e,Math.min(a,n)):i}function wt(t){return Array.isArray(t)?t.map(wt):t&&typeof t=="object"?Object.fromEntries(Object.entries(t).filter(([,i])=>i!==void 0).sort(([i],[e])=>i.localeCompare(e)).map(([i,e])=>[i,wt(e)])):t}function je(t){return vs("sha256").update(JSON.stringify(wt(t))).digest("hex")}function Is(t){return Number.parseInt(je(t).slice(0,8),16)>>>0}function Jn(t){let i=Is(t)||1;return()=>{i+=1831565813;let e=i;return e=Math.imul(e^e>>>15,e|1),e^=e+Math.imul(e^e>>>7,e|61),((e^e>>>14)>>>0)/4294967296}}function js(t,i={}){let e={...Ze,...Xe(i)},a=Math.max(1,Math.floor(t));return 1+Ee(e.sqrtDifficulty,Ze.sqrtDifficulty,0,10)*Math.sqrt(a-1)+Ee(e.linearDifficulty,Ze.linearDifficulty,0,10)*(a-1)}function xs(t,i,e,a){let n=Math.min(ue(a.maxGoals,3,1,5),1+Math.floor(t/12)),s=t%7,o=[{type:"score",target:e}];return n>1&&(s===0||s===3?o.push({type:"collect",tileIndex:t%5,target:Math.round(12+i*5)}):t>=ue(a.obstacleStartLevel,8,1,1e6)?o.push({type:"obstacles",target:Math.round(4+i*2)}):o.push({type:"createSpecial",special:"line",target:Math.max(1,Math.round(i))})),n>2&&o.push(t%2?{type:"activateSpecial",special:"any",target:Math.max(1,Math.round(i))}:{type:"collect",tileIndex:(t+2)%5,target:Math.round(10+i*4)}),o}function zn(t,i,e){let a={...Ze,...Xe(i.level_rules??i.levelRules)},n={...$s,...Xe(i.scoring_rules??i.scoringRules)},s={...ks,...Xe(i.rating_rules??i.ratingRules)},o=Math.max(1,Math.floor(t)),r=js(o,a),c=ue(a.rows,6,5,10),d=ue(a.columns,6,5,10),l=ue(a.minTileTypes,5,5,8),m=ue(a.maxTileTypes,8,l,8),y=Math.min(m,l+Math.floor((o-1)/18)),b=Math.max(500,Math.round(Ee(a.baseTargetScore,1e4,500,1e9)*r/100)*100),v=Math.max(ue(a.minMoves,12,5,99),ue(a.baseMoves,25,5,99)-Math.floor(Math.log2(o+1))),h=Math.max(0,o-ue(a.obstacleStartLevel,8,1,1e6)),j=ue(a.checkpointEvery,10,1,1e4),N=ue(a.milestoneEvery,25,1,1e4);return{level:o,seed:`${e}:${o}`,difficulty:Number(r.toFixed(5)),rows:c,columns:d,tileTypes:y,moves:v,targetScore:b,goals:xs(o,r,b,a),checkpoint:o%j===0,multistage:o%N===0,obstacleChance:o<ue(a.obstacleStartLevel,8,1,1e6)?0:Math.min(Ee(a.obstacleChanceMax,.28,0,.8),.04+h*.006),blockedChance:o<N?0:Math.min(Ee(a.blockedChanceMax,.12,0,.5),.02+o*5e-4),scoring:n,rating:s}}function We(t){return t&&!t.blocked?t.tile:""}function ft(t,i,e){let a=[];for(let n=0;n<i;n+=1){let s=0;for(let o=1;o<=e;o+=1){let r=We(t[n*e+s]),c=o<e?We(t[n*e+o]):"";(!r||c!==r)&&(r&&o-s>=3&&a.push(Array.from({length:o-s},(d,l)=>n*e+s+l)),s=o)}}for(let n=0;n<e;n+=1){let s=0;for(let o=1;o<=i;o+=1){let r=We(t[s*e+n]),c=o<i?We(t[o*e+n]):"";(!r||c!==r)&&(r&&o-s>=3&&a.push(Array.from({length:o-s},(d,l)=>(s+l)*e+n)),s=o)}}return a}function Vn(t,i,e){let a=Math.floor(t/e),n=Math.floor(i/e);return Math.abs(a-n)+Math.abs(t%e-i%e)===1}function qe(t,i,e){[t[i],t[e]]=[t[e],t[i]]}function Gn(t,i,e){for(let a=0;a<t.length;a+=1)for(let n of[a+1,a+e]){if(n>=t.length||!Vn(a,n,e)||t[a]?.blocked||t[n]?.blocked)continue;qe(t,a,n);let s=ft(t,i,e).length>0;if(qe(t,a,n),s)return[a,n]}return null}function Fn(t,i,e){return!!Gn(t,i,e)}function et(t,i){let e=i.slice(0,t.tileTypes);if(e.length<5)throw new Error("At least five active Match-3 symbols are required");for(let a=0;a<80;a+=1){let n=Jn(`${t.seed}:board:${a}`),s=[];for(let o=0;o<t.rows*t.columns;o+=1){let r=Math.floor(o/t.columns),c=o%t.columns,d=new Set;c>=2&&s[o-1]?.tile===s[o-2]?.tile&&d.add(s[o-1].tile),r>=2&&s[o-t.columns]?.tile===s[o-t.columns*2]?.tile&&d.add(s[o-t.columns].tile);let l=e.filter(b=>!d.has(b)),m=t.blockedChance>0&&n()<t.blockedChance,y=!m&&t.obstacleChance>0&&n()<t.obstacleChance?n()<.2?2:1:0;s.push({tile:l[Math.floor(n()*l.length)]||e[0],special:"",obstacle:y,blocked:m})}if(Fn(s,t.rows,t.columns))return s}throw new Error("Unable to generate a playable Match-3 board")}function Wn(t,i,e,a,n,s){let o=Jn(`${e.seed}:move:${n}:cascade:${s}:${je(t)}`),r=t.map(c=>({...c}));for(let c=0;c<e.columns;c+=1){let d=[];for(let m=e.rows-1;m>=0;m-=1){let y=m*e.columns+c;r[y]?.blocked||i.has(y)||d.push(r[y])}let l=0;for(let m=e.rows-1;m>=0;m-=1){let y=m*e.columns+c;r[y]?.blocked||(r[y]=d[l++]||{tile:a[Math.floor(o()*Math.min(a.length,e.tileTypes))],special:"",obstacle:0,blocked:!1})}}return r}function Ye(t,i,e,a,n){let s=t[i];if(!s?.special)return[];let o=Math.floor(i/n),r=i%n;if(s.special==="line-h")return Array.from({length:n},(d,l)=>o*n+l);if(s.special==="line-v")return Array.from({length:a},(d,l)=>l*n+r);if(s.special==="bomb"){let d=[];for(let l=-1;l<=1;l+=1)for(let m=-1;m<=1;m+=1){let y=o+l,b=r+m;y>=0&&y<a&&b>=0&&b<n&&d.push(y*n+b)}return d}let c=t[e]?.tile;return t.flatMap((d,l)=>!d.blocked&&(!c||d.tile===c)?[l]:[])}function Yn(t,i,e,a,n,s){let o=t.map(I=>({...I}));if(!Number.isInteger(i)||!Number.isInteger(e)||!Vn(i,e,a.columns))return{valid:!1,reason:"not_adjacent",board:o,scoreDelta:0,progressDelta:{},breakdown:{},cascades:0};if(!o[i]||!o[e]||o[i].blocked||o[e].blocked)return{valid:!1,reason:"blocked",board:o,scoreDelta:0,progressDelta:{},breakdown:{},cascades:0};qe(o,i,e);let r=o[i]?.special,c=o[e]?.special,d=new Set;if(r&&c?r==="rainbow"||c==="rainbow"?o.forEach((I,x)=>{I.blocked||d.add(x)}):(Ye(o,i,e,a.rows,a.columns).forEach(I=>d.add(I)),Ye(o,e,i,a.rows,a.columns).forEach(I=>d.add(I))):(r||c)&&Ye(o,r?i:e,r?e:i,a.rows,a.columns).forEach(R=>d.add(R)),!d.size&&!ft(o,a.rows,a.columns).length)return qe(o,i,e),{valid:!1,reason:"no_match",board:o,scoreDelta:0,progressDelta:{},breakdown:{},cascades:0};let l=o,m=0,y=0,b={},v={line:0,bomb:0,rainbow:0},h={line:0,bomb:0,rainbow:0,any:0},j={combinations:0,cascades:0,specials:0,obstacles:0},N=0;for(let I=1;I<=12;I+=1){let x=ft(l,a.rows,a.columns);if(!x.length&&!d.size)break;N=I;let R=new Set(d.size?d:x.flat()),J=[...R],U=new Set;for(;J.length;){let V=J.shift(),q=l[V]?.special;if(!q||U.has(V))continue;U.add(V);let st=V===i?e:i;for(let ot of Ye(l,V,st,a.rows,a.columns))R.has(ot)||J.push(ot),R.add(ot);let Ke=q.startsWith("line")?"line":q;h[Ke]=Number(h[Ke]||0)+1,h.any+=1;let It=Number(a.scoring[`${Ke}Activate`]||0);j.specials+=It,m+=It}let C="",W=x.flatMap((V,q)=>x.slice(q+1).flatMap(st=>V.filter(Ke=>st.includes(Ke)))),oe=x.find(V=>V.includes(e))||x[0]||[...R];!d.size&&W.length?C="bomb":!d.size&&oe.length>=5?C="rainbow":!d.size&&oe.length===4&&(C=oe.every(q=>Math.floor(q/a.columns)===Math.floor(oe[0]/a.columns))?"line-h":"line-v");let se=W[0]??(oe.includes(e)?e:oe[0]);if(C){R.delete(se);let V=C==="rainbow"?"rainbow":C==="bomb"?"bomb":"line";v[V]+=1;let q=Number(a.scoring[`${V}Create`]||0);j.specials+=q,m+=q}let de=0,fe=0;for(let V of R){let q=l[V];!q||q.blocked||(b[q.tile]=Number(b[q.tile]||0)+1,q.obstacle>0&&(y+=1,fe+=a.scoring.obstacleLayer),de+=1)}let Le=Math.min(a.scoring.maxCascade,1+a.scoring.cascadeStep*(I-1)),ge=x.length?Math.max(...x.map(V=>V.length>=6?a.scoring.combo6:V.length===5?a.scoring.combo5:V.length===4?a.scoring.combo4:a.scoring.combo3)):1,xe=Math.round(de*a.scoring.baseTile*ge*Le);j.obstacles+=fe,m+=xe+fe,I===1?j.combinations+=xe:j.cascades+=xe,l=Wn(l,R,a,n,s,I),C&&l[se]&&!l[se].blocked&&(l[se].special=C),d=new Set}return Fn(l,a.rows,a.columns)||(l=et({...a,seed:`${a.seed}:reshuffle:${s}`},n)),{valid:!0,board:l,scoreDelta:m,progressDelta:{collected:b,obstaclesDestroyed:y,specialsCreated:v,specialsActivated:h},breakdown:j,cascades:N}}function Zn(t,i,e,a,n,s){let o=t.map(m=>({...m}));if(i==="hint"){let m=Gn(o,a.rows,a.columns);return{valid:!!m,reason:m?void 0:"no_hint",board:o,scoreDelta:0,progressDelta:{},hint:m||[]}}if(i==="shuffle")return{valid:!0,board:et({...a,seed:`${a.seed}:booster:shuffle:${s}`},n),scoreDelta:0,progressDelta:{}};if(!Number.isInteger(e)||e===null||e<0||e>=o.length)return{valid:!1,reason:"invalid_target",board:o,scoreDelta:0,progressDelta:{}};if(o[e]?.blocked)return{valid:!1,reason:"blocked",board:o,scoreDelta:0,progressDelta:{}};let r=new Set;if(i==="remove"&&r.add(e),i==="removeType"){let m=o[e]?.tile;o.forEach((y,b)=>{!y.blocked&&y.tile===m&&r.add(b)})}if(i==="bomb"){let m=Math.floor(e/a.columns),y=e%a.columns;for(let b=-1;b<=1;b+=1)for(let v=-1;v<=1;v+=1){let h=m+b,j=y+v;if(h>=0&&h<a.rows&&j>=0&&j<a.columns){let N=h*a.columns+j;o[N]?.blocked||r.add(N)}}}let c={},d=0;for(let m of r){let y=o[m];y&&(c[y.tile]=Number(c[y.tile]||0)+1,y.obstacle>0&&(d+=1))}let l=Math.round(r.size*Number(a.scoring.baseTile||100));return{valid:!0,board:Wn(o,r,a,n,1e5+s,1),scoreDelta:l,progressDelta:{collected:c,obstaclesDestroyed:d},cleared:r.size}}function Ue(){return{score:0,collected:{},obstaclesDestroyed:0,specialsCreated:{line:0,bomb:0,rainbow:0},specialsActivated:{line:0,bomb:0,rainbow:0,any:0}}}function Ss(t,i,e){return t.type==="score"?Number(i.score||0):t.type==="collect"?Number(i.collected[e[t.tileIndex||0]]||0):t.type==="obstacles"?Number(i.obstaclesDestroyed||0):t.type==="createSpecial"?Number(i.specialsCreated[t.special||"line"]||0):Number(i.specialsActivated[t.special||"any"]||0)}function Xn(t,i,e){return t.goals.every(a=>Ss(a,i,e)>=a.target)}function qn(t,i,e,a){return e?t>=i*a.star3?3:t>=i*a.star2?2:1:0}function ea(t,i,e,a){return i<1?0:Math.round(a.base*(1+a.levelLog*Math.log(Math.max(1,t)))*a[`star${i}`]*a[`continue${Math.min(2,e)}`])}var Ze,$s,ks,ta=D(()=>{"use strict";Ze={rows:6,columns:6,minTileTypes:5,maxTileTypes:8,baseMoves:25,minMoves:12,baseTargetScore:1e4,sqrtDifficulty:.06,linearDifficulty:.004,maxGoals:3,checkpointEvery:10,milestoneEvery:25,specialStartLevel:4,obstacleStartLevel:8,blockedChanceMax:.12,obstacleChanceMax:.28},$s={baseTile:100,combo3:1,combo4:1.25,combo5:1.6,combo6:2,comboTL:1.75,cascadeStep:.35,maxCascade:3,lineCreate:250,bombCreate:400,rainbowCreate:650,lineActivate:350,bombActivate:550,rainbowActivate:900,obstacleLayer:150,goalComplete:1e3,allGoalsBase:2500,remainingMove:200,cleanMultiplier:.1,star2:1.2,star3:1.5},ks={base:1e3,levelLog:.1,star1:1,star2:1.15,star3:1.35,continue0:1,continue1:.85,continue2:.65}});import{Router as Ps}from"express";function Ne(t){return f(String(t.get("idempotency-key")||"").trim()||t.body?.idempotencyKey,"idempotencyKey",160)}function Ts(t){return(Array.isArray(t?.symbols)?t.symbols:[]).filter(e=>e?.active!==!1&&e?.key).map(e=>({key:String(e.key),label:String(e.label||e.key),imageUrl:String(e.imageUrl||e.defaultImageUrl||"")}))}function Be(t,i){Object.entries(i||{}).forEach(([e,a])=>{t[e]=Number(t[e]||0)+Number(a||0)})}async function tt(t){await rn(t);let i=await u(t,`select * from public.game_seasons
      where status in ('scheduled','active')
     ...
     on conflict (round_id, clan_id, metric) do nothing`,[d.id,Math.max(1,Number(c?.clan_rules?.minimumLevelsForChest||3))]),d}function na(t){let i=Ps();return i.use(Q),i.get("/",p(async(e,a)=>{let[n,s,o]=await Promise.all([u(t,"select * from public.game_settings where singleton = true"),u(t,"select * from public.game_profiles where user_key = $1",[e.userPrincipal.userKey]),tt(t)]),r=s;if(n&&r){let c=Math.max(1,Number(n.lives_rules?.maximum||n.base_lives||5)),d=Math.max(1,Number(n.lives_rules?.restoreMinutes||30)),l=r.last_life_at?Date.now()-new Date(r.last_life_at).getTime():0,m=Math.floor(l/(d*6e4));m>0&&Number(r.lives||0)<c&&(r=await u(t,`update public.game_profiles
              set lives = least($2, lives + $3),
                  ...
                  updated_at = now()
            where user_key = $1 returning *`,[e.userPrincipal.userKey,c,m,d]))}a.json({settings:n,profile:r,season:o})})),i.get("/leaderboard",p(async(e,a)=>{let n=e.query.seasonId?P(e.query.seasonId,"seasonId"):null,s=n?await u(t,"select * from public.game_seasons where id = $1",[n]):await tt(t),o=await w(t,`select ranked.position, ranked.user_key, ranked.score, ranked.level,
        ...
        limit 100`,[s?.id||null]),r=o.find(c=>c.user_key===e.userPrincipal.userKey)||null;a.json({season:s,leaderboard:o,me:r})})),i.post("/sessions",p(async(e,a)=>{await O(t,e,"game.session",M(e));let n=Ne(e),s=e.userPrincipal.userKey,o=await u(t,"select * from public.game_sessions where idempotency_key = $1 and user_key = $2",[n,s]);if(o)return a.json({session:o,replayed:!0});let r;try{r=await S(t,async c=>{let d=await u(c,`select * from public.game_sessions
         ...
         returning *`,[n,e.userPrincipal.userKey,c,o,r,N,J,I.scoreDelta,JSON.stringify(I),s]),session:U,replayed:!1}});a.status(d.replayed?200:201).json(d)})),i.post("/sessions/:sessionId/boosters",p(async(e,a)=>{await O(t,e,"game.booster",M(e));let n=P(e.params.sessionId,"sessionId"),s=Ne(e),o=String(e.body?.type||"");if(!["shuffle","hint","bomb","remove","removeType"].includes(o))throw new _(400,"Unsupported game booster","validation_error");let r=e.body?.index===void 0||e.body?.index===null?null:k(e.body.index,0,0,99),c=await S(t,async d=>{let l=await u(d,`select * from public.game_booster_uses
         ...
         returning *`,[n,e.userPrincipal.userKey,o,r,J,U,JSON.stringify(x),s]),session:oe,result:x,replayed:!1}});a.status(c.replayed?200:201).json(c)})),i.post("/sessions/:sessionId/finish",p(async(e,a)=>{let n=P(e.params.sessionId,"sessionId"),s=Ne(e),o=await S(t,async r=>{let c=await u(r,`select * from public.idempotency_records
          ...
          where scope = 'game.finish' and idempotency_key = $1`,[s]);if(l?.response_body)return{session:l.response_body,replayed:!0};if(d.status!=="active")throw new _(409,"Game session has already ended","game_already_ended");let m=await u(r,"select * from public.game_settings where singleton = true");if(!m)throw new _(500,"Game settings are missing","game_settings_missing");let y=Math.max(1,Math.floor((Date.now()-new Date(d.started_at).getTime())/1e3)),b=d.level_config||{},v=Array.isArray(b.symbols)?b.symbols:[],h=v.map(V=>String(V.key)),j=je({generated:{...b,symbols:void 0},symbols:v})===String(d.config_signature),N=[];j||N.push("config_signature_mismatch"),Number(d.move_sequence||0)>Number(b.moves||0)+Number(d.continues_used||0)*Number(m.economy_rules?.continueMoves||5)&&N.push("move_limit_exceeded");let I=N.length>0,x={...Ue(),...d.goal_progress||{}},R=!I&&Xn(b,x,h),J=b.scoring||{},U={...d.score_breakdown||{}},C=Number(d.level_score||0);R&&(U.goals=b.goals.length*Number(J.goalComplete||1e3)+Math.round(Number(J.allGoalsBase||2500)*Number(b.difficulty||1)),U.remainingMoves=Math.round(Number(d.moves_remaining||0)*Number(J.remainingMove||200)*Number(b.difficulty||1)),C+=Number(U.goals||0)+Number(U.remainingMoves||0),Number(d.continues_used||0)===0&&(U.clean=Math.round(C*Number(J.cleanMultiplier||.1)),C+=Number(U.clean||0)));let W=qn(C,Number(b.targetScore||0),R,J),oe=R?ea(Number(d.level_number),W,Number(d.continues_used||0),b.rating||{}):0,se=R?await u(r,`select * from public.game_level_results
            where season_id = $1 and user_key = $2 and level_number = $3
            for update`,[d.season_id,e.userPrincipal.userKey,d.level_number]):null,de=Math.max(0,oe-Number(se?.best_rating||0)),fe=m.economy_rules||{},Le=Array.isArray(fe.starRewards)?fe.starRewards:[0,5,10,20],ge=0;R&&(se||(ge+=Number(fe.firstCompletion||20)),ge+=Math.max(0,Number(Le[W]||0)-Number(Le[se?.best_stars||0]||0)),Number(d.continues_used||0)===0&&!se?.clean_completed&&(ge+=Number(fe.cleanCompletion||10)));let xe=await u(r,`update public.game_sessions
          ...
          where id = $1
          returning *`,[n,y,C,I,JSON.stringify(N),R?"success":"failed",W,de,ge,JSON.stringify(U),R?0:1,JSON.stringify(e.body||{})]);return R?(await r.query(`insert into public.game_level_results(
           ...
           on conflict (season_id, user_key, level_number) do update
             set best_session_id = case
              ...
              set account_level = greatest(account_level, $2 + 1),
                  ...
                  updated_at = now()
            where user_key = $1`,[e.userPrincipal.userKey,Number(d.level_number),de,ge,!se,W===3&&Number(se?.best_stars||0)<3,Number(d.continues_used||0)===0&&!se?.clean_completed,C])):await r.query(`update public.game_profiles
         ...
         ) values ('game.finish',$1,$2,200,$3::jsonb,now())`,[s,e.userPrincipal.userKey,JSON.stringify(xe)]),{session:xe,replayed:!1,result:{success:R,levelScore:C,stars:W,seasonalResult:oe,seasonalPoints:de,ballyAwarded:ge,breakdown:U}}});a.json(o)})),i.post("/sessions/:sessionId/continue",p(async(e,a)=>{let n=P(e.params.sessionId,"sessionId"),s=Ne(e),o=await u(t,`select * from public.game_continues
        where idempotency_key = $1 and user_key = $2`,[s,e.userPrincipal.userKey]);if(o)return a.json({continue:o,replayed:!0});let r=await S(t,async c=>{let d=await u(c,`select * from public.game_sessions
         ...
         returning *`,[n,e.userPrincipal.userKey,b,s]);return await c.query(`update public.game_sessions
          ...
          returning *`,[n,e.userPrincipal.userKey]);return r&&await o.query(`update public.game_profiles
            ...
            where user_key = $1`,[e.userPrincipal.userKey]),r});if(!s)throw new _(404,"Active game session was not found","not_found");a.json({session:s})})),i.post("/lives/restore",p(async(e,a)=>{let n=Ne(e),s=!!e.body?.full,o=await S(t,async r=>{let c=await u(r,`select * from public.idempotency_records
          ...
          returning *`,[e.userPrincipal.userKey,s,m,y]);if(!b)throw new _(409,"Not enough Bally","insufficient_bally");let v={profile:b,cost:y,full:s};return await r.query(`insert into public.idempotency_records(
           scope, idempotency_key, actor_key, response_code, response_body, completed_at
         ) values ('game.lives.restore',$1,$2,200,$3::jsonb,now())`,[n,e.userPrincipal.userKey,JSON.stringify(v)]),{...v,replayed:!1}});a.json(o)})),i.get("/clans/leaderboard",p(async(e,a)=>{let n=String(e.query.clanType||"user");if(!["user","corporate"].includes(n))throw new _(400,"Unsupported clan category","validation_error");let[s,o]=await Promise.all([tt(t),u(t,"select clan_rules from public.game_settings where singleton = true")]),r=await Ns(t,s,n),c=Math.max(2,Number(o?.clan_rules?.minimumMembers||5)),d=await w(t,`with member_scores as (
       ...
       select score.*,
        ...
        order by user_row.last_seen_at desc limit $3`,[e.userPrincipal.userKey,s,n]),r=await Je(t,e.userPrincipal.userKey,o.map(c=>c.user_key));a.json({people:r})})),i.get("/me",p(async(e,a)=>{let[n,s,o,r,c]=await Promise.all([lt(t,e.userPrincipal.userKey,e.userPrincipal.userKey),u(t,"select * from public.user_profiles where user_key = $1",[e.userPrincipal.userKey]),u(t,"select * from public.user_consents where user_key = $1",[e.userPrincipal.userKey]),w(t,`select clan.id, clan.name, clan.clan_type, clan.logo_url
           from (
       ...
       on conflict (user_key) do update
         ...
         returning *`,[o,s,JSON.stringify({previousTelegramId:d.telegram_id,previousName:d.name,previousUsername:d.username})]);return await c.query(`update public.app_users
          ...
          where app_user_key = $1 and revoked_at is null`,[o]),m});a.clearCookie(ve,{path:"/"}),a.json({deletion:r})})),i.get("/:userKey",p(async(e,a)=>{let n=$(e.params.userKey,"userKey");a.json({profile:await lt(t,e.userPrincipal.userKey,n)})})),i}var ra=D(()=>{"use strict";B();L();z();_t();Z()});import{Router as Rs}from"express";function ca(t){let i=Rs();return i.use(Q),i.get("/",p(async(e,a)=>{let[n,s,o]=await Promise.all([w(t,`select scope, block_key, name, title, subtitle, asset_key,
          ...
          order by app_type, sort_order, item_key`),w(t,`select asset_key, name, url, media_type, width, height,
           ...
           from public.admin_assets`)]);a.json({blocks:n,navigation:s,assets:o})})),i}var ua=D(()=>{"use strict";B();L();z()});import{Router as Ds}from"express";function da(t,i){return t<i?[t,i]:[i,t]}async function vt(t,i,e){return!!await u(t,`select 1
       from public.user_blocks
      where (blocker_user_key = $1 and blocked_user_key = $2)
         or (blocker_user_key = $2 and blocked_user_key = $1)
      limit 1`,[i,e])}async function gt(t,i,e){let a=await u(t,`select conversation.*, connection.status as connection_status
      ...
      where conversation.id = $1
        and $2 in (conversation.pair_low, conversation.pair_high)`,[i,e]);if(!a)throw new _(404,"Conversation was not found","not_found");if(a.connection_status!=="accepted"||a.archived_at)throw new _(403,"Conversation is not active","conversation_unavailable");let n=a.pair_low===e?a.pair_high:a.pair_low;if(await vt(t,e,n))throw new _(403,"Conversation is unavailable because one user blocked the other","user_blocked");return{...a,peerKey:n}}async function ht(t,i,e,a,n,s,o){await t.query(`insert into public.notifications(
       user_key, notification_type, title, body, data, idempotency_key
     ) values ($1,$2,$3,$4,$5::jsonb,$6)
     on conflict (idempotency_key) do nothing`,[i,e,a,n,JSON.stringify(s),o])}function la(t){let i=Ds();return i.use(Q),i.get("/connections",p(async(e,a)=>{let n=await w(t,`select connection.*,
        ...
        order by case when connection.status = 'pending' then 0 else 1 end,
                 connection.updated_at desc`,[e.userPrincipal.userKey]);a.json({connections:n})})),i.post("/connections",p(async(e,a)=>{await O(t,e,"connection.create",M(e));let n=$(e.body?.recipientUserKey,"recipientUserKey"),s=g(e.body?.message,500),o=e.userPrincipal.userKey;if(n===o)throw new _(400,"A user cannot send a connection request to themselves","validation_error");if(await vt(t,o,n))throw new _(403,"Connection request is unavailable because one user blocked the other","user_blocked");let r=await u(t,`select user_row.user_key, user_row.name,
              coalesce(profile.allow_connections, true) as allow_connections
         from public.app_users user_row
         left join public.user_profiles profile on profile.user_key = user_row.user_key
        ...
        where requester_user_key = $1
         ...
         returning *`,[o,n,d,l,s])});await ht(t,n,"connection_request","\u041D\u043E\u0432\u0430\u044F \u0437\u0430\u044F\u0432\u043A\u0430 \u043D\u0430 \u0437\u043D\u0430\u043A\u043E\u043C\u0441\u0442\u0432\u043E",`${e.userPrincipal.name} \u0445\u043E\u0447\u0435\u0442 \u043F\u043E\u0437\u043D\u0430\u043A\u043E\u043C\u0438\u0442\u044C\u0441\u044F.`,{connectionId:m.id,requesterUserKey:o},`connection-request:${m.id}:${m.created_at}`),a.status(201).json({connection:m})})),i.patch("/connections/:connectionId",p(async(e,a)=>{let n=P(e.params.connectionId,"connectionId"),s=K(e.body?.status,"status",Ms),o=await S(t,async r=>{let c=await u(r,`select * from public.user_connections
          where id = $1 and recipient_user_key = $2
          for update`,[n,e.userPrincipal.userKey]);if(!c)throw new _(404,"Connection request was not found","not_found");if(c.status!=="pending")throw new _(409,"Connection request has already been answered","connection_already_answered");if(await vt(r,c.requester_user_key,c.recipient_user_key))throw new _(403,"Connection is unavailable because one user blocked the other","user_blocked");let d=await u(r,`update public.user_connections
            set status = $2,
                responded_at = now(),
        ...
        order by latest.created_at desc nulls last, conversation.updated_at desc`,[e.userPrincipal.userKey]);a.json({conversations:n})})),i.get("/conversations/:conversationId/messages",p(async(e,a)=>{let n=P(e.params.conversationId,"conversationId");await gt(t,n,e.userPrincipal.userKey);let s=await w(t,`select message.*, author.name as author_name, author.avatar as author_avatar
         from public.direct_messages message
       ...
       returning *`,[n,e.userPrincipal.userKey,s,o]);await t.query("update public.direct_conversations set updated_at = now() where id = $1",[n]),await ht(t,r.peerKey,"direct_message",`\u0421\u043E\u043E\u0431\u0449\u0435\u043D\u0438\u0435 \u043E\u0442 ${e.userPrincipal.name}`,s.slice(0,300),{conversationId:n,messageId:c.id},`direct-message:${c.id}`),a.status(201).json({message:c})})),i.post("/blocks",p(async(e,a)=>{let n=$(e.body?.blockedUserKey,"blockedUserKey"),s=g(e.body?.reason,1e3);if(n===e.userPrincipal.userKey)throw new _(400,"A user cannot block themselves","validation_error");if(!await u(t,"select user_key from public.app_users where user_key = $1",[n]))throw new _(404,"User was not found","not_found");let[r,c]=da(e.userPrincipal.userKey,n),d=await S(t,async l=>{let m=await u(l,`insert into public.user_blocks(blocker_user_key, blocked_user_key, reason)
        ...
        where blocker_user_key = $1 and blocked_user_key = $2`,[e.userPrincipal.userKey,n])).rowCount)throw new _(404,"Block was not found","not_found");a.status(204).end()})),i.post("/reports",p(async(e,a)=>{await O(t,e,"user_report.create",M(e));let n=$(e.body?.reportedUserKey,"reportedUserKey"),s=f(e.body?.reasonCode,"reasonCode",100),o=g(e.body?.details,2e3),r=e.body?.conversationId?P(e.body.conversationId,"conversationId"):null,c=e.body?.messageId?P(e.body.messageId,"messageId"):null;if(n===e.userPrincipal.userKey)throw new _(400,"A user cannot report themselves","validation_error");r&&await gt(t,r,e.userPrincipal.userKey);let d=await S(t,async l=>{let m=await u(l,`insert into public.user_reports(
         ...
         ) values ('user_report','user_report',$1,$2,'normal')`,[m.id,n]),m});a.status(201).json({report:d})})),i}var Ms,_a=D(()=>{"use strict";B();L();z();ye();Z();Ms=["accepted","declined"]});var pa={};it(pa,{createApp:()=>Ls});import{randomUUID as Os}from"node:crypto";import{mkdirSync as Cs}from"node:fs";import Ae from"node:path";import Es from"cookie-parser";import nt from"express";import Us from"helmet";function Ls(t,i){Cs($t,{recursive:!0});let e=nt();return e.disable("x-powered-by"),i.trustProxy&&e.set("trust proxy",1),e.use((a,n,s)=>{a.requestId=String(a.get("x-request-id")||Os()).slice(0,160),n.setHeader("x-request-id",a.requestId),n.setHeader("x-bali-environment",i.environment),s()}),e.use(Us({contentSecurityPolicy:{directives:{defaultSrc:["'self'"],scriptSrc:["'self'","'unsafe-inline'"],styleSrc:["'self'","'unsafe-inline'","https://fonts.googleapis.com"],fontSrc:["'self'","https://fonts.gstatic.com","data:"],imgSrc:["'self'","data:","blob:","https:"],connectSrc:["'self'"],frameAncestors:["'self'"]}},crossOriginEmbedderPolicy:!1})),e.use(nt.json({limit:"256kb"})),e.use(Es()),e.use(Kt(t,i)),e.use(Rt(t,i)),e.use("/api/v1",(a,n,s)=>{n.setHeader("Cache-Control","private, no-store, max-age=0"),n.setHeader("Pragma","no-cache"),s()}),e.get("/api/v1/health",(a,n)=>{n.json({ok:!0,environment:i.environment})}),e.get("/api/v1/config/public",(a,n)=>{n.json({environment:i.environment,demoAvailable:!["production","staging"].includes(i.environment),authentication:"mobile-password"})}),e.use("/api/v1/auth",Mt(t,i)),e.use("/api/v1/auth",Et(t,i)),e.use("/api/v1/clans",Ft(t)),e.use("/api/v1/people",ia(t)),e.use("/api/v1/events",Qn(t)),e.use("/api/v1/layouts",Xt(t)),e.use("/api/v1/bookings",Dn(t)),e.use("/api/v1/catalog",On(t)),e.use("/api/v1/economy",En(t)),e.use("/api/v1/game",na(t)),e.use("/api/v1/notifications",sa(t)),e.use("/api/v1/platform-config",ca(t)),e.use("/api/v1/social",la(t)),e.use("/api/v1/admin",Yt(t)),e.use("/api/v1/admin",sn(t)),e.use("/api/v1/admin",ln(t)),e.use("/api/v1/admin",bn(t,$t)),e.use("/api/v1/admin",In(t)),e.use("/api/v1/admin",xn(t)),e.use("/api/v1/admin",Pn(t)),e.use("/site",nt.static(at,{etag:!0,maxAge:i.environment==="production"?"1h":0,index:!1})),e.use("/uploads",nt.static($t,{etag:!0,immutable:i.environment==="production",maxAge:i.environment==="production"?"1y":0,index:!1,fallthrough:!1})),e.get("/app",(a,n)=>{n.setHeader("Cache-Control","no-store, max-age=0"),n.sendFile(Ae.join(at,"app-production.html"))}),e.get("/admin",(a,n)=>{n.setHeader("Cache-Control","no-store, max-age=0"),n.sendFile(Ae.join(at,"admin-production.html"))}),["production","staging"].includes(i.environment)||e.get("/demo",(a,n)=>{n.sendFile(Ae.join(at,"index.html"))}),e.get("/",(a,n)=>{n.redirect(i.environment==="production"?"/app":"/demo")}),e.use(xt),e.use(St),e}var at,Bs,$t,ma=D(()=>{"use strict";Ot();Ut();Wt();Zt();on();_n();wn();jn();Sn();Tn();Mn();Cn();Un();Hn();aa();ze();oa();ra();ua();_a();L();z();at=Ae.resolve(process.cwd(),"site"),Bs=process.env.VERCEL?"/tmp/bali-uploads":Ae.join(process.cwd(),"var","uploads"),$t=Ae.resolve(process.env.BALI_UPLOAD_DIR||Bs)});var ya={};it(ya,{loadConfig:()=>Hs});function kt(t,i){let e=Number.parseInt(t||"",10);return Number.isFinite(e)&&e>0?e:i}function Hs(t=process.env){let i=t.BALI_ENV||t.NODE_ENV||"development",e=Qs.has(i)?i:"development",a=e==="production"||e==="staging",n={environment:e,port:kt(t.PORT,8080),databaseUrl:t.DATABASE_URL||"",telegramBotToken:t.TELEGRAM_BOT_TOKEN||"",telegramBotUrl:t.TELEGRAM_BOT_URL||"",sessionSecret:t.SESSION_SECRET||"",sessionTtlSeconds:kt(t.SESSION_TTL_SECONDS,720*60*60),telegramAuthMaxAgeSeconds:kt(t.TELEGRAM_AUTH_MAX_AGE_SECONDS,300),adminBootstrapEmail:(t.ADMIN_BOOTSTRAP_EMAIL||"").trim().toLowerCase(),adminBootstrapPassword:t.ADMIN_BOOTSTRAP_PASSWORD||"",trustProxy:t.TRUST_PROXY==="1",secureCookies:a};if(a){let s=[!n.databaseUrl&&"DATABASE_URL",n.sessionSecret.length<32&&"SESSION_SECRET (minimum 32 characters)"].filter(Boolean);if(s.length)throw new Error(`Missing production configuration: ${s.join(", ")}`)}return n}var Qs,ba=D(()=>{"use strict";Qs=new Set(["demo","development","staging","production","test"])});import be from"node:process";import Js from"express";var wa=globalThis.Deno?.env;be.env.BALI_ENV="production";be.env.DATABASE_URL=wa?.get("SUPABASE_DB_URL")||be.env.SUPABASE_DB_URL||"";be.env.SESSION_SECRET=wa?.get("SUPABASE_SERVICE_ROLE_KEY")||be.env.SUPABASE_SERVICE_ROLE_KEY||"";be.env.TRUST_PROXY="1";be.env.DB_POOL_MAX="1";be.env.BALI_UPLOAD_DIR="/tmp/bali-uploads";be.env.PORT="8000";var[{createApp:zs},{loadConfig:Vs},{createPool:Gs}]=await Promise.all([Promise.resolve().then(()=>(ma(),pa)),Promise.resolve().then(()=>(ba(),ya)),Promise.resolve().then(()=>(B(),jt))]),fa=Vs(be.env),ga=Gs(fa.databaseUrl);await ga.query("select 1");var Fs=zs(ga,fa),ha=Js();ha.use((t,i)=>{let e=["/functions/v1/bali-api","/bali-api"];for(let a of e)t.url===a?t.url="/":t.url.startsWith(`${a}/`)&&(t.url=t.url.slice(a.length));return Fs(t,i)});ha.listen(8e3,()=>console.log("BALI Edge API listening on 8000"));
