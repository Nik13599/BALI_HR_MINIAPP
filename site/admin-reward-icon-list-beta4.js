(() => {
  ...
  function draw() {
    ...
    panel.innerHTML = `<div class="panel-head"><div><h3>Все награды</h3><small>Стандартные и созданные вручную</small></div></div><div class="panel-body"><p class="muted">PNG, квадрат 1:1, прозрачный фон, сторона 64–2048 px.</p><input id="rewardIconFile" type="file" accept="image/png" hidden><div class="reward-icon-grid">${registry.allRewards().map(item => `<article class="reward-icon-item"><div>${item.image ? `<img src="${esc(item.image)}" alt="">` : `<span>${esc(item.icon || "🏆")}</span>`}</div><strong>${esc(item.title)}</strong><small>${item.source === "standard" ? "Стандартная" : "Созданная"} · +${Number(item.xp || 0)} XP</small><button type="button" class="secondary" data-pick-reward-icon="${esc(item.id)}">Заменить PNG</button></article>`).join("")}</div></div>`;
  }
