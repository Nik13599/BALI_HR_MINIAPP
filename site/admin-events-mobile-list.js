(() => {
  ...
  async function renderLite(root) {
    ...
    const cards = events.map((item) => {
      ...
      return `<article class="event-mobile-card">
        ...
        <div class="event-mobile-main">
          ...
          <div class="event-mobile-tags"><span class="event-mobile-tag ${ready ? "ok" : "warn"}">${ready ? `${layout.tables.length} столов` : "Рассадка не готова"}</span><span class="event-mobile-tag ${item.active !== false ? "ok" : ""}">${item.active !== false ? "Опубликовано" : "Черновик"}</span></div>
          <div class="event-mobile-actions"><button class="ghost" data-mobile-event-edit="${item.id}">Изменить</button><button class="ghost" data-mobile-event-layout="${item.id}">Рассадка</button><button class="${item.active !== false ? "danger" : "primary"}" data-mobile-event-publish="${item.id}" ${item.active === false && !ready ? "disabled" : ""}>${item.active !== false ? "Снять" : "Опубликовать"}</button></div>
        </div>
      </article>`;
    }).join("");
    root.innerHTML = `<section class="panel"><div class="panel-head"><div><h3>Афиши и мероприятия</h3><small>Облегчённый мобильный режим</small></div></div><div class="panel-body"><div class="events-mobile-note">Список событий загружается отдельно от редактора рассадки. Нижнее меню всегда остаётся доступным.</div><div class="events-mobile-list" style="margin-top:11px">${cards || '<div class="empty">Афиш пока нет</div>'}</div></div></section>`;
  }
