(() => {
  ...
  function styles() {
    ...
    style.textContent = `
      ...
      .profile-v2-controls{margin-left:auto;display:flex;gap:7px;align-items:center}
      .profile-v2-controls button{width:43px;height:43px;padding:0;border:1px solid var(--line);border-radius:50%;background:#ffffff08;color:#fff;font-size:18px}
      .profile-v2-quick{display:grid;grid-template-columns:1fr 1fr;gap:10px}
      .profile-v2-tile{position:relative;display:grid;align-content:end;gap:6px;min-height:146px;padding:16px;border:1px solid var(--line);border-radius:21px;background:linear-gradient(145deg,#151a17,#0e110f);color:#fff;text-align:left;overflow:hidden}
      .profile-v2-tile:after{position:absolute;right:12px;top:8px;color:#ffffff0d;font:700 72px Unbounded}
      .profile-v2-tile.shop:after{content:"B"}.profile-v2-tile.rewards:after{content:"★"}
      .profile-v2-tile small{position:relative;z-index:1;color:var(--muted);font-size:8px;font-weight:900;letter-spacing:.12em}
      .profile-v2-tile strong{position:relative;z-index:1;font:600 21px Unbounded;color:var(--lime)}
      ...
      .profile-v2-balance strong{display:block;color:var(--lime);font:600 38px Unbounded}
      .profile-v2-balance small{color:var(--muted)}
      ...
      .profile-v2-row i{width:42px;height:42px;display:grid;place-items:center;overflow:hidden;border-radius:12px;background:#ffffff08;font-style:normal}
      .profile-v2-row i img{width:100%;height:100%;object-fit:contain}
      ...
      @media(max-width:400px){.profile-v2-quick{grid-template-columns:1fr 1fr;gap:7px}.profile-v2-tile{min-height:132px;padding:13px}.profile-v2-tile strong{font-size:17px}.profile-v2-row{grid-template-columns:38px minmax(0,1fr) auto}.profile-v2-row i{width:38px;height:38px}}
    `;
  ...
  function dialogs() {
    ...
    document.body.insertAdjacentHTML("beforeend", ["Points", "Rewards", "Settings", "History"].map(name => `
  ...
  function rewardRows() {
    ...
    return [
      ...standard,
      ...custom.map(row => ({ ...row, earned: earnedCustom.has(row.id), source: "Награда BALI" }))
    ];
  }
  ...
  function mount() {
    ...
    stats.innerHTML = `
      <article><strong>${Number(profile.visits || 0)}</strong><span>ПОСЕЩЕНИЯ</span></article>
      <article><strong>${Number(profile.bookings || 0)}</strong><span>БРОНИРОВАНИЯ</span></article>
      <article><strong>${earned}</strong><span>НАГРАДЫ</span></article>`;
    ...
    controls.innerHTML = `<button type="button" data-open-profile-settings title="Настройки профиля" aria-label="Настройки профиля">⚙</button>`;
    ...
    quick.innerHTML = `
      <button class="profile-v2-tile shop" type="button" data-open-profile-points data-open-profile-vip>
        <small>МАГАЗИН</small>
        <strong>BALI Shop</strong>
        <span>${balance} баллов · VIP, фишки и все варианты покупок →</span>
      </button>
      <button class="profile-v2-tile rewards" type="button" data-open-profile-rewards>
        <small>МОИ НАГРАДЫ</small>
        <strong>${earned} из ${rows.length}</strong>
        <span>Полученные и доступные награды BALI →</span>
      </button>`;
    return true;
  }
  ...
  function renderShop() {
    ...
    if (!root) return;
    root.innerHTML = `
      <div class="profile-v2-balance">
        <span>ВАШ БАЛАНС</span>
        <strong>${balance}</strong>
        <small>BALI-Баллов · ${chips} фишек</small>
      </div>
      <section class="profile-v2-section">
        <h3>VIP-статусы</h3>
        <div class="profile-v2-shop-note">Все доступные статусы BALI, сроки действия и варианты покупки находятся здесь.</div>
        <div id="profileVipBody"></div>
      </section>
      <section class="profile-v2-section">
        <h3>Обмен баллов на фишки</h3>
        <div class="profile-v2-shop-note">Выберите количество фишек. Заявка поступит администратору, а выдача подтверждается в заведении.</div>
      </section>`;
  ...
  function renderRewards() {
    ...
    root.innerHTML = `
      <section class="profile-v2-section">
        <h3>Награды BALI</h3>
        <div class="profile-v2-list">
          ${rows.map(row => `
            <article class="profile-v2-row ${row.earned ? "" : "locked"}">
              <i>${row.image ? `<img src="${esc(row.image)}" alt="">` : esc(row.icon || "🏆")}</i>
              <div>
                <h3>${esc(row.title || "Награда BALI")}</h3>
                <p>${esc(row.description || "Продолжайте посещать BALI, чтобы получить награду")}</p>
              </div>
              <b>${row.earned ? "Получено" : `+${Number(row.xp || 0)} XP`}</b>
            </article>`).join("") || '<div class="empty">Награды скоро появятся</div>'}
        </div>
      </section>`;
  }
    ...
    root.innerHTML = `
      <form class="profile-v2-form" id="profileV2SettingsForm">
        <div class="profile-v2-avatar">
          ${profile.avatar ? `<img src="${esc(profile.avatar)}" alt="">` : '<div class="avatar">B</div>'}
          <div><strong>Фотография профиля</strong><br><button class="secondary" type="button" data-v2-pick-avatar>Загрузить фото</button></div>
          <input id="profileV2AvatarInput" type="file" accept="image/*" hidden>
        </div>
  ...
  async function renderHistory() {
    ...
    let rows = [];
    try {
      rows = attendance ? await attendance.listCheckins() : Object.values(JSON.parse(localStorage.getItem("bali_event_checkins_v1") || "{}"));
    } catch {}
    ...
    if (!root) return;
    root.innerHTML = `<div class="profile-v2-list">${rows.map(row => `
      <article class="profile-v2-row">
        <i>✓</i>
        <div><h3>${esc(row.event_title || "Мероприятие BALI")}</h3><p>${fmt(row.checked_in_at)} · ${Number(row.reward || 0)} баллов · ${Number(row.xp || 0)} XP</p></div>
        <b>${esc(row.level || "")}</b>
      </article>`).join("") || '<div class="empty">Подтверждённых посещений пока нет</div>'}</div>`;
  }
  ...
  document.addEventListener("click", async event => {
    ...
    if (event.target.closest("[data-open-profile-points]")) {
      renderShop();
      open("Points");
    }
    if (event.target.closest("[data-open-profile-rewards]")) {
      renderRewards();
      open("Rewards");
    }
    if (event.target.closest("[data-open-profile-settings]")) {
      renderSettings();
      open("Settings");
    }
    if (event.target.closest("[data-open-profile-history]")) {
      await renderHistory();
      open("History");
    }
  ...
  document.addEventListener("submit", event => {
    ...
    game.saveProfile({
      ...
      publicRanking: form.publicRanking.checked
    });
    social?.saveProfile?.({
      name: String(data.name || "").trim(),
      active: form.socialActive.checked,
      status: form.socialActive.checked ? data.socialStatus : "closed",
  ...
  window.BaliCompactProfile = { mount, renderShop, renderRewards, renderSettings, renderHistory };
})();