(() => {
  ...
  function mount() {
    ...
    if (!inner || document.getElementById("eventQrHomeCard")) return;
    const actions = inner.querySelector(".actions");
    const card = document.createElement("section");
    card.id = "eventQrHomeCard";
    card.className = "event-checkin-card";
    card.innerHTML = `<span class="eyebrow">Я УЖЕ В BALI</span><h3>Подтвердить вход</h3><p>Отсканируйте QR-код мероприятия у хостес, чтобы посещение попало в профиль, рейтинг и систему наград.</p><button class="primary" type="button" data-open-event-qr>Сканировать QR-код</button>`;
  ...
  async function processQuery() {
    ...
    renderResult(await attendance.checkIn(code));
  ...
  document.addEventListener("click", event => {
    ...
    if (event.target.closest("[data-close-event-qr]")) { event.preventDefault(); stopScanner(); document.getElementById("eventQrDialog")?.close(); }
