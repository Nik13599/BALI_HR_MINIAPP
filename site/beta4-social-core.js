(() => {
  ...
  const KEYS = { profile:"bali_social_profile_v1", people:"bali_social_people_v1", requests:"bali_social_requests_v1", gifts:"bali_social_gifts_v1", swipes:"bali_social_swipes_v2" };
  ...
  function people(){const rows=read(KEYS.people,[]);if(!rows.length)localStorage.setItem(KEYS.people,JSON.stringify(demos));const me=profile();return[me,...read(KEYS.people,demos).filter(x=>x.id!==me.id)]}
  function visiblePeople(){return people().filter(x=>x.id!==myId()&&x.active!==false&&x.status!=="closed")}
  const swipes=()=>read(KEYS.swipes,[]);
  function setSwipe(fromId,toId,decision){const rows=swipes().filter(x=>!(x.fromId===fromId&&x.toId===toId));rows.unshift({id:uid("swipe"),fromId,toId,decision,createdAt:now()});write(KEYS.swipes,rows.slice(0,2000));return rows[0]}
  function swipe(targetId,decision){const item=setSwipe(myId(),targetId,decision);return{item,matched:decision==="like"&&isMatch(targetId)}}
  function hasLiked(fromId,toId){return swipes().some(x=>x.fromId===fromId&&x.toId===toId&&x.decision==="like")}
  function isMatch(otherId){return hasLiked(myId(),otherId)&&hasLiked(otherId,myId())}
  function incomingLikes(){return visiblePeople().filter(p=>hasLiked(p.id,myId()))}
  function matches(){return visiblePeople().filter(p=>isMatch(p.id))}
  function passedOrLikedIds(){return new Set(swipes().filter(x=>x.fromId===myId()).map(x=>x.toId))}
  const requests=()=>read(KEYS.requests,[]);
  function sendRequest(targetId,type="invite",event=null){const target=people().find(x=>x.id===targetId);if(!target)return{ok:false,message:"Профиль не найден"};if(requests().some(x=>x.fromId===myId()&&x.toId===targetId&&x.type===type&&x.status==="pending"))return{ok:false,message:"Запрос уже отправлен"};const item={id:uid("req"),fromId:myId(),fromName:profile().name,toId:targetId,toName:target.name,type,status:"pending",eventId:event?.id||"",eventTitle:event?.title||"",eventDate:event?.event_date||"",createdAt:now()};const rows=requests();rows.unshift(item);write(KEYS.requests,rows);return{ok:true,item}}
  ...
  function telegramVisible(person){return Boolean(person?.username&&person.shareTelegram!==false&&isMatch(person.id))}
  ...
  function seedIncomingLikes(){if(swipes().some(x=>x.toId===myId()))return;const rows=swipes();rows.unshift({id:uid("swipe"),fromId:"demo-1",toId:myId(),decision:"like",createdAt:now()},{id:uid("swipe"),fromId:"demo-3",toId:myId(),decision:"like",createdAt:now()});localStorage.setItem(KEYS.swipes,JSON.stringify(rows))}
  seedIncomingLikes();
  window.BaliBeta4Social={KEYS,STATUSES,GIFT_CATALOG,profile,saveProfile,people,visiblePeople,swipes,swipe,hasLiked,isMatch,incomingLikes,matches,passedOrLikedIds,requests,sendRequest,respond,telegramVisible,gifts,recordGift,myId};
})();