(async () => {
  ...
  // Production keeps the stable27 UI, but never enables demo data/auth.
  ...
  // Same visual layer as site/index.html stable27. Production-only CSS is appended last.
  const css = [
    "beta4-app.css",
  ...
  // UI modules from stable27 that are safe with the production adapters.
  const stableUi = [
    "beta4-app.js",
  ...
  // Optional stable27 refinements. They must not block the application if a legacy helper is absent.
  const enhancements = [
  ...
  try {
    ...
    // Production data adapters must exist before any stable27 UI is mounted.
    ...
    // Refresh DOM-only stable27 modules after the real secondary data arrives.
