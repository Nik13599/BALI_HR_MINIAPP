(() => {
  ...
  async function activeVisit() {
    ...
    const mine = checkins.filter(row => !row.left_at && row.presence_status !== "left" && isMine(row, keys, telegramId)).sort((a, b) => String(b.checked_in_at || "").localeCompare(String(a.checked_in_at || "")));
  ...
  function renderResult(result) {
    ...
    node.innerHTML = `<i>✅</i><h3>${result.event.title}</h3><p>${result.reentered ? "Вы снова на мероприятии. Повторные баллы не начислены." : "Посещение успешно записано. До окончания мероприятия на главной странице будет отображаться активный статус."}</p><div class="event-qr-rewards"><span><strong>+${Number(result.points || 0)}</strong>баллов</span><span><strong>+${Number(result.xp || 0)}</strong>XP</span><span><strong>${Number(result.visits || 0)}</strong>посещений</span></div><p>Текущий уровень: <b>${result.level || "—"}</b></p><button class="primary full" type="button" data-close-event-qr>Готово</button>`;
  }
  ...
  document.addEventListener("click", async event => {
    ...
    if (leave) {
      ...
      if (!confirm("Уйти с мероприятия? Активные функции вечеринки станут недоступны, но посещение и начисленные баллы сохранятся.")) return;
      const result = await attendance.leave?.(leave.dataset.leaveEvent);
      toast(result?.ok ? "Вы ушли с мероприятия" : result?.message || "Не удалось завершить посещение");
      await refreshHomeCard();
    }
  }, true);
  ...
  ["bali:data-changed", "bali:beta4-changed", "bali:points-changed", "bali:checkin-left"].forEach(name => window.addEventListener(name, () => setTimeout(refreshHomeCard, 0)));
  ...
  window.BaliEventQrUi = { refreshActiveEvent: refreshHomeCard, activeVisit };
})();